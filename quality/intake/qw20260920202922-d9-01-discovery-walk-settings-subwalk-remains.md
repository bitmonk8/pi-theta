...
verdict: questionable — accounting verified: size-scan map reproduces 615 LOC / band zone (FILE_BANDS zone=600) and all 12 declarations at the cited ranges (concern 1 TreeEntry…resolveSettingsSource :111-426 = 245 LOC, concern 2 discoverThetas :434-581 + collectFromEntries :583-615 = 181 LOC; discoverThetas 1/18, every concern-1 member 0/0 — my grep across src/extensions/tools/tests finds only package-discovery.ts's same-named private walker and prose in test messages/comments, no import); both excerpts byte-match at :273-279 and :434-437; cross-references hold both ways (sed :111-426 has zero discoverThetas/collectFromEntries hits; :434-615 reaches concern 1 only through the one resolveSettingsSource call at :471) and the file declares no module-scope const/let, so the two rows share no state — a real ≥ 2-concern inventory; concern 1's enumerate calls resolve via the :81-87 import (actual 173/326/349/367/370/373, ≤2-line drift), so PTQ-0367's cycle blocker is gone; no overlooked reason (closed enumeration defends only the driver; DISC-5 grammar is wholly inside the moving block; behaviour not data; no @generated marker; git shows discovery-settings-source.ts never existed, so no reverted split; exemptions.json's 4 rows are D8 #enumerateDirectory (moved out), package-discovery.ts D9 (different host) and two unrelated); PTQ-0367's resolved note carries the human ratification verbatim — 'Seam A … NOT ratified this wave … D9 re-files it after, expect ratification then' — and Seam B landed in 46a063e0 (+288 discovery-source-enumerate.ts, −255 host); no other live intake/issues file names discovery-walk or this seam; only slip is prose spec paths omitting docs/spec_topics/discovery/ (DISC-5 at package-and-settings.md:21, thetaPaths :99; DISC-1…4 in discovery-sources.md), non-load-bearing; per D9 policy an accurate zone-band breakdown accounting caps at questionable — seam and home are the human's ruling, never confirmed (triage: claude-fable-5-1)
