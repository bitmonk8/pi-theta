...
verdict: questionable — independently re-verified: all 11 `new StaticTypeInferencePass(` sites (3 src, 8 tests; invoke-static-checks.ts now :1060/:1634, content-identical drift) bind the single `checkCompatible` defined at type-compat.ts:210, no mock/wrapper/second `CheckCompatible`- or `CompatRelation`-typed value exists anywhere in src/tests/extensions/tools, and the seam has held one value since its V20b-T birth (57dd14ef, 2026-07-04), so the brief's "config knob with one instantiation" criterion is mechanically met and no existing issue tracks it (PTQ-0030's triage names it as a distinct sibling); but unlike the confirmed constant-param precedents (PTQ-0011/0044/0053/0130, all inert) `#checkCompatible` is live at three read sites (:607, :642, :652), and the injection is stated design intent in three places — header :14-15, type-compat.ts:749-758 (which names "importing `checkCompatible` into the inference pass" as the rejected alternative) and bug 0158's fix record (docs/bugs/0158 §~998-1001, echoed at :616-624) which chose a private LUB copy specifically to decide over the injected relation — while the deps interface, constructor and per-site literal all survive for `enumNames`, so the fix trades one alias and one field against three rationale blocks to rewrite; whether a deliberately documented DI seam with one binding is cruft or architecture is a human ruling, not a mechanical one (triage: claude-opus-5)
