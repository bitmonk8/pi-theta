...
verdict: questionable — third-pass independent re-verification concurs: reran the construction-site grep myself (11 hits, unchanged) and diffed the pre-drift tree directly (`git show 264dcbd6^`/`96303cc3^`) rather than trusting the prior note's arithmetic — every one of the six frontmatter-cited ranges (static-type-inference.ts :50-54/:71-73/:99-104, type-layer-checks.ts :338-341, invoke-static-checks.ts :943-946/:1386-1389) matches the pre-drift source byte-for-byte, so the drift is fully accounted for by two later, unrelated commits (35df0ce3 doc edits + 96303cc3's +251-line RFC-0009 feature insert), not fabrication; a repo-wide `checkCompatible` grep (every src/ and tests/ hit, not just the filed sites) turns up zero mocks/spies/alternate relations, confirming "eleven binders, one value" as filed. But the anchor is genuinely contested, not merely asserted: type-compat.ts:753-762 names the finding's own suggested fix ("importing `checkCompatible` into the inference pass") and states why it was rejected — a shared decision procedure between the checker and the pass — static-type-inference.ts:624-628 repeats the same reasoning for `#matchArmType`, and docs/bugs/0158 (:993-1001) shows the identical choice remade in a later, dated refactor rather than merely carried forward unexamined. A mechanically-proven constant-valued knob set against a mechanically-proven, twice-reaffirmed anti-duplication rationale is exactly the tension `questionable` exists for (triage: claude-opus-5)
verdict: questionable — fourth independent pass, same conclusion from primary sources alone: confirmed all 11 construction sites still bind one production `checkCompatible` (`new StaticTypeInferencePass(` grep = 11, unchanged; production trio content-matches the frontmatter at drifted lines type-layer-checks.ts:341, invoke-static-checks.ts:1096/:1670); no dedupe hit (PTQ-0127/PTQ-0170 already distinguish this exact pending file from their own findings by name). But the "buys nothing" anchor is directly contradicted by on-the-record design intent, read myself at source: type-compat.ts's `CompatRelation` doc names this finding's own suggested fix verbatim ("rather than importing `checkCompatible` into the inference pass") and gives a reason (checker/pass must share one decision procedure), static-type-inference.ts's `#commonType`/`#matchArmType` docs restate it, and docs/bugs/0158's Fix section remakes the same choice in a dated later refactor rather than merely inheriting it — real, mechanically-verified counter-intent, not a fabricated excuse, so whether that rationale justifies the per-site ceremony is a judgment call the fix stage's human owner should make, not a triage default (triage: claude-opus-5)
