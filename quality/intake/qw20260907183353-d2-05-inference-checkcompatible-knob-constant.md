...
verdict: questionable — independently re-verified: all 11 call sites reproduce (3 src, now :1096/:1670, content-identical drift; 8 tests), `#checkCompatible` is read live at three sites (:614/:649/:659, not dead), and no second `CheckCompatible`/`CompatRelation` value exists anywhere in src/tests/extensions/tools, so the mechanical "one binding" premise holds and this is confirmed distinct from PTQ-0030 and PTQ-0127 (both name this file as the separate sibling); but the seam is documented, load-bearing intent, not silence — type-compat.ts:757 names and rejects "importing `checkCompatible` into the inference pass" so the checker's `commonType` and this pass compute the LUB through the identical decision procedure, and bug 0158's fix log (§~991-1001) shows that exact trade-off being chosen again, on purpose, during a later refactor rather than defaulted past — so a fix would be overturning restated architecture, not removing forgotten cruft, and that weighing is a human call (triage: claude-opus-5)
