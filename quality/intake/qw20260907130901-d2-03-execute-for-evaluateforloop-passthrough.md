---
id: pending                  # PTQ-NNNN minted at acceptance; never self-assigned
title: "executeFor's drive of evaluateForLoop builds a plan array through a two-method host whose both members are inert, adding no behavior over iterating the snapshot directly"
lens: D2                     # the lens that filed this
status: intake               # intake | open | fixed | rejected (store mechanics own transitions)
verdict: pending             # pending | confirmed | questionable | false-positive | duplicate | out-of-scope | malformed
locations:                   # every cited site, repo-relative path:line-range
  - src/runtime/statement-executor.ts:2341-2355
  - src/runtime/control-flow.ts:52-63
sites: 1                     # count of occurrences cited in Evidence
fix_scope: localized         # localized | module | cross-module — mechanical size proxy, NOT a priority
wave: qw20260907130901
reported_by: lens-d2-cruft (anthropic/claude-sonnet-5)
date: 2026-09-07
---

# executeFor's drive of evaluateForLoop builds a plan array through a two-method host whose both members are inert, adding no behavior over iterating the snapshot directly

## Observation

`executeFor` evaluates the loop iterand itself (once, at loop entry) and
captures `snapshot`. It then constructs a `ForLoopHost` whose
`evaluateIterand` returns that already-computed `snapshot` and whose
`runIteration` only pushes `{ element }` into a local `plan` array, calls
`evaluateForLoop(host)`, and iterates `plan`. `evaluateForLoop` is a
seven-line function that reads the iterand once and calls `runIteration` per
element in index order, so at this call site the drive reduces to copying
`snapshot`'s elements, in order, into wrapper objects — `plan[i].element ===
snapshot[i]` by construction. This is `evaluateForLoop`'s only caller in
src/, extensions/, and tools/. The sibling loop `evalParFor`, under the same
CTRL-1 snapshot obligation, iterates its snapshot directly without the drive.

## Evidence

src/runtime/statement-executor.ts:2341-2355 — the drive and its consumption:

```ts
  // Drive `V3c`'s real `evaluateForLoop` to fix the iteration order over the
  // snapshot (iterand evaluated exactly once — CTRL-1). The body's effects are
  // async, so the synchronous loop host captures each element in order; the
  // async body walk below honours `break` / `continue`.
  const plan: { readonly element: ThetaValue }[] = [];
  const host: ForLoopHost = {
    evaluateIterand: () => snapshot,
    runIteration: (element) => {
      plan.push({ element });
    },
  };
  evaluateForLoop(host);

  const site = loopIterSite(stmt, deps);
  for (const { element } of plan) {
```

src/runtime/control-flow.ts:52-63 — the entire driven function:

```ts
export function evaluateForLoop(host: ForLoopHost): void {
  // CTRL-1: evaluate the iterand exactly once at loop entry, committing its
  // effect, and capture the resulting `array<T>` snapshot before iterating. A
  // body-side `let mut` reassignment of the iterand source cannot reach this
  // captured reference, so the iterated sequence stays fixed.
  const snapshot = host.evaluateIterand();
  for (let index = 0; index < snapshot.length; index += 1) {
    host.runIteration(snapshot[index] as ThetaValue, index);
  }
}
```

CTRL-1's evaluate-once obligation is discharged before the drive, at
src/runtime/statement-executor.ts:2329-2339:

```ts
  const iterand = await evalExpr(stmt.iterand, env, deps);
  if (iterand.flow !== "value") {
    return terminalFlow(iterand);
  }
  // Bug 0369 belt: a non-array iterand that evaded the parse refusal by static
  // unresolvability must abort loudly, not silently satisfy the loop with a
  // fabricated empty snapshot.
  if (!Array.isArray(iterand.value)) {
    throw new ForIterandKindDefectError(iterand.value);
  }
  const snapshot: readonly ThetaValue[] = iterand.value;
```

Caller count: `grep -rn "evaluateForLoop" src/ extensions/ tools/` — hits only
in src/runtime/control-flow.ts (declaration and its own comments) and
src/runtime/statement-executor.ts (import at 60, comments at 6/2322/2341/2381,
the one call at 2352). Tests call it directly at
tests/control-flow.test.ts:75,100,129 (the V3c seam's own witnesses).

The sibling `evalParFor` under the same CTRL-1 obligation iterates its
snapshot without the drive — src/runtime/statement-executor.ts:1917 captures
`const snapshot: readonly ThetaValue[] = iterandValue;`, 1989 reads
`const n = snapshot.length;`, and the worker at 2010-2035 indexes
`snapshot[index]` directly; no `ForLoopHost` is built.

## Why this is a problem

A redundant pass-through layer at its sole production instantiation. Both
injected host members are inert at this site: `evaluateIterand` cannot
evaluate anything (it returns a constant computed at line 2329, which is where
the evaluate-once property actually holds), and `runIteration` records rather
than runs an iteration (`index` is not even bound). The net effect of the
composition is an element-wise identity copy of `snapshot` into `plan`, which
the very next statement unwraps back to elements — the comment's stated
purpose, "fix the iteration order over the snapshot", is a property direct
array iteration already has. The indirection misleads a reader into thinking
the V3c seam gates the executor's `for` semantics, while the sibling `par for`
path shows the module itself does not route snapshot iteration through that
seam.

## Suggested direction (non-binding, optional)

Iterate `snapshot` directly in `executeFor` (as `evalParFor` already does),
leaving `control-flow.ts` and its witness tests untouched; if the V3c seam
must remain the executor's iteration authority, the host it is handed would
need to carry the real per-iteration work rather than a recorder.

## False-positive check

- Caller enumeration: every `evaluateForLoop` reference in src/, extensions/,
  tools/, and tests/ cited above; the single production call is
  statement-executor.ts:2352. No dynamic or string-keyed dispatch reaches it
  (grep for `"evaluateForLoop"` as a string literal: only comments).
- Not a deadness claim against `evaluateForLoop` / `ForLoopHost`: the V3c
  module has its own witness tests (tests/control-flow.test.ts) and is not
  claimed removable; the claim is confined to the executor-side drive being a
  behavior-free pass-through at its one production site.
- Behavioral equivalence verified by inspection: `evaluateForLoop` performs no
  work besides the two host calls; `evaluateIterand` closes over the
  already-captured `snapshot`; `plan` is consumed only for `element`
  (statement-executor.ts:2355). No await, no catch, no signal read occurs
  inside the drive, so cancellation/checkpoint order is unchanged by inlining.
- Documented-intent check: the module header (statement-executor.ts:6,
  "driving the real `ForLoopHost` / `evaluateForLoop` from `V3c`") states the
  composition is deliberate; noted rather than disputed — the mechanical fact
  filed here is that at this site the composition contributes no behavior, and
  the sibling `evalParFor` discharges the same CTRL-1 obligation without it.
- Git intent: the drive shape (pre-evaluated iterand + recording host) is
  unchanged since the V19c executor landed; no later commit added behavior to
  the host members.

## Triage

verdict: questionable — drive verified behavior-free (plan is an identity copy of snapshot, index unbound, sole production call at :2352, evalParFor iterates directly), but the shape is the module header's documented original design that never carried behavior, and inlining it leaves evaluateForLoop/ForLoopHost test-only — the dead-seam-duplicate shape this repo tolerates elsewhere; anchor is simplicity taste, so a human should rule (triage: claude-opus-5)
verdict: questionable — independently re-verified in full: every excerpt is content-identical under uniform drift (drive now :2370-2381, `evaluateForLoop` at control-flow.ts:46-55), the drive is provably an ordered identity copy (`evaluateIterand` returns the :2368 constant, `runIteration` drops `index` and records, `evaluateForLoop` does nothing beyond the two host calls; theta arrays are `readonly ThetaValue[]` with no in-place mutation repo-wide so the copy is not even defensive), :2381 is the sole production call (tests/control-flow.test.ts:75/100/129 the only others; no string-keyed access, no re-export barrel), `evalParFor` indexes `snapshot[index]` directly at :2049, and blame shows the shape born in V19c 086c1540 with 2bc691576 the rename only — but unlike the confirmed born-inert findings (PTQ-0076/0081/0102, all accidental constructs) this is the deliberate, four-times-documented V19c→V3c composition (header :6, doc :2349-2352, inline :2370-2373 which openly calls the host a recorder, commit message) whose inlining would leave control-flow.ts production-orphaned under a header claiming to own runtime `for` evaluation (the exact PTQ-0078 shape) or force a cross-module reshaping of the sync seam to carry async body work; the anchor is simplicity taste and the fix cost plausibly exceeds its benefit, so a human should rule (triage: claude-opus-5)
verdict: questionable — third independent pass concurs: all mechanical claims reproduce verbatim (identity copy of snapshot into plan, `index` param genuinely unbound in `runIteration`, sole production call at :2381 confirmed by my own grep across src/extensions/tools/tests, `evalParFor` confirmed indexing `snapshot[index]` directly at :2048-2049 with no host, blame confirms 086c1540 V19c origin) but the header's TDD-declared-seam language (`ForLoopHost` interface authored by V3c-T before V3c's implementation, independently witnessed by tests/control-flow.test.ts) is a legitimate cohesion rationale competing with the simplicity complaint, not refuted by it — anchor remains taste/cost-benefit, not a mechanically forced defect, so a human should rule (triage: claude-opus-5)
verdict: questionable — fourth independent pass, same result: every mechanical fact reproduces (plan is an ordered identity copy of snapshot; `runIteration`'s `index` parameter is genuinely dropped; :2381 is the one production call, confirmed by my own repo-wide grep including dist/ and a check for `"evaluateForLoop"` as a string literal — no hits; `evalParFor` indexes `snapshot[index]` directly with no host at :2048-2049; blame confirms 086c1540 V19c authored the drive verbatim with 2bc69157 a pure Loom→Theta rename) and the seam is genuinely TDD-declared (103802e1 V3c-T stubs `ForLoopHost`/`evaluateForLoop` and reds until 15a3f4c0 V3c implements them, still witnessed today by tests/control-flow.test.ts) — a real, deliberate cohesion rationale, not an accident like the confirmed PTQ-0081/PTQ-0102 shapes, so removing the indirection is an architecture trade-off rather than a forced fix; anchor is taste/cost-benefit, a human should rule (triage: claude-opus-5)
verdict: questionable — fifth independent pass, re-derived from scratch, same conclusion: current drive at statement-executor.ts:2370-2381 and evaluateForLoop at control-flow.ts:46-55 (line drift only, content identical to the filed excerpts); plan is a provable ordered identity copy of snapshot with runIteration's index unused; my own grep of evaluateForLoop across src/, extensions/, tools/, tests/ finds :2381 the sole production call and tests/control-flow.test.ts:75/100/129 the only others, no string-literal or re-export access; evalParFor confirmed indexing snapshot[index] directly at :2049 with no host; `git log -S "runIteration" -- src/runtime/statement-executor.ts` returns exactly one commit (086c1540, V19c) matching the module header/doc-comment/inline-comment's thrice-documented deliberate V3c-seam-driving design rather than decay; dedupe-checked against PTQ-0078 (a different module's stale-ownership-header claim on checkpoint-granularity.ts, not this file's behavior-free-composition claim on control-flow.ts) — not a duplicate; anchor remains taste/cost-benefit between seam-routing consistency and simplification, so a human should rule (triage: claude-opus-5)
verdict: questionable — sixth independent pass, re-derived from scratch: every excerpt reproduces at the drifted lines (statement-executor.ts:2370-2381, control-flow.ts:46-55, content byte-identical to the filing); the drive is a provable ordered identity copy (plan[i].element === snapshot[i], runIteration's `index` param never bound in the arrow at :2372-2374); my own repo-wide grep of `evaluateForLoop` across src/, extensions/, tools/, tests/ finds exactly one production call (:2381, sole importer of `./control-flow` inside src/runtime/) plus the three test call sites already cited, no string-literal or barrel re-export; evalParFor independently confirmed indexing `snapshot[index]` directly at :2049 with no host built; git confirms the lineage as filed — 086c1540 (V19c) is the only commit touching `runIteration`/`plan.push` in statement-executor.ts, 103802e1 (V3c-T) stubs the seam and 15a3f4c0 (V3c) implements it, and 2bc69157's control-flow.ts diff is a pure LoomValue→ThetaValue rename with zero logic change; PTQ-0070 (already resolved) shows this same header once carried stale stub-narration on this exact function and was fixed to accurate past tense, confirming the seam's TDD-declared-and-landed status is real, not asserted, which is what separates this filing from the accidental born-inert shapes at PTQ-0076/0081/0102 (no competing design rationale there) and keeps it distinct from PTQ-0078 (a different module's stale-ownership-claim root cause) — not a duplicate of either; anchor is genuinely taste/cost-benefit between seam-routing cohesion and inlining, so a human should rule (triage: claude-opus-5)

