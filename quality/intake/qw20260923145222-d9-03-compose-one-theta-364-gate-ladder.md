...
verdict: questionable — accounting independently re-verified: size-scan map gives composeOneTheta 1758-2121, 364 LOC, band strong (FN strong ≥ 200), unexported 0/0; the gate ladder (Step 0(f) 1786, RFC-0012 §6 1795, V20a tools, …, binder-model 1980-2042, typed-query warning ~2044-2063, composedInput 2070) is real distinct gates citing distinct authorities, and only toolResult (15 refs), importCheck (13) and binderModelResolution (5) cross phase boundaries (invokeDiagnostics/bypassEligible/isMarkedRootTheta stay in-gate), so no ≥6-shared-locals, spec-table, generated (0 markers) or exemption reason applies; not a duplicate — PTQ-1151/1438 (resolved) carry #runComposePass and no row carries #composeOneTheta; the seam shape needs a human ruling per D9 protocol, never confirmed (triage: claude-opus-5-5)
verdict: questionable — accounting verified; target shape needs a human ruling: size-scan map (one-line manifest) gives composeOneTheta 1758-2121, 364 LOC, band strong (FN strong ≥ 200), unexported 0/0; the 1786-1789 excerpt matches; the gates at the cited anchors are real, separate refusal gates, each citing its own authority and emitting its own diagnostics, and only toolResult/importCheck/binderModelResolution cross gate boundaries (fewer than the 6 shared locals that would argue against a split); no spec table, generated marker, exemptions.json key or revert; 182 non-comment code lines, so it is over the zone bar even without comments; not a duplicate, since PTQ-1151/1438 (resolved) are keyed to #runComposePass and no issues/resolved row is keyed to #composeOneTheta (triage: claude-opus-5-5)
