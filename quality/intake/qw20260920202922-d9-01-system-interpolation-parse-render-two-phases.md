...
verdict: questionable — accounting verified against the post-71af3b5b file with drift noted: size-scan map now reports 705 LOC (not 761) because the D4 dedupes PTQ-1120/PTQ-1124 landed (`scanInterpolationBody` / `interpolationTypeOf` now imported from ../render/query-render, `interpolationTypeOfValue` gone) — still zone band (≥ 600), no exemption key; inventory re-adds from the current map as parse 219 = 12+6+68+3+3+23+90+14 (219-331, 480-531 incl. `toInterpolationType` callers at 460/464 and `located`), render 131 = 6+3+59+43+20 (536-705), model 71, `toInterpolationType` 36, anchors 20 — two ≥ 100-LOC concerns survive; helper call sites confirm zero shared runtime helpers (parse helpers only at 268-464, render helpers only at 573-612, crossing only via the `SystemTemplate`/`SystemTemplatePart`/`SystemUnionArm` types); consumer split reproduces (`checkSystemInterpolation` imported only at src/parser/frontmatter.ts:42-46 and called at 2325, `renderSystemPrompt` only at src/extension/production-theta-producer.ts:282/2501, 20 test files; import-static-checks.ts:109 / production-composition.ts:191 type-only, import-system-template-patch.ts:24 `toInterpolationType` + types; nothing under extensions/ or tools/); reasons-considered stand — type+anchor LOC 127/705 ≈ 18 %, no generator, no ≥ 6 shared locals, not one production, header's "seam" wording is a design statement not a spec-cited co-location invariant, git log shows no reverted split; dedupe: PTQ-1106/1120/1124 are resolved D2/D4 with different root causes, no D9 issue names this host. Seam A now drops the host to ~574 LOC (below zone), so A vs A+B is a design decision for a human ruling. (triage: claude-fable-5-1)
