...
verdict: questionable — re-verified: the trio sits at shape-refusal:200/211-213/271-273 and zero-entry:192-194/197/200-202, byte-identical apart from `bug0104`/`bug0206`, and the grep returns exactly these 2 files. But `parse` is a one-line label binder over the canonical e2e-s1 `parseFrontmatterSource`, left in exactly this shape by PTQ-1390's fix (d3bf3e19), so that part re-files a fix's final form. The real residual is the 3-line joiner plus the 1-line `BODY_NO_CALL`. Both files already import production-load-harness, whose variadic `theta` would cover the joiner (the PTQ-1358/0782 precedent), but with a different signature. The qw20260922211400 reviewer left it unfiled as too weak (REVIEW_LOG:766), and PTQ-0469's fix, which the 2026-09-18 duplicate ruling routed it to, never absorbed it. A human should rule whether this 4-line residual justifies a fix (triage: claude-opus-5-5)
