...
verdict: questionable — fourth independent pass, same result: my own greps reproduce every mechanical claim (3 identifier hits at schema-validator.ts:323/351/376; 0 `export *`; barrel src/seams/index.ts:7-12 omits this name and also its siblings AjvSchemaValidator/SchemaSlug/SchemaSlugFn; 91 `new AjvSchemaValidator(` sites, all untyped literals; `git log -S` shows only the two pre-filing commits, no importer ever added; package.json has no main/types/exports field), and a fresh from-scratch census of the same 43 exported `*Deps` interfaces in src/ finds exactly 19 with zero references outside their declaring file (matching the second pass's count) — so the interface types a live #deps field and constructor parameter of a class 91 sites construct, nothing is dead, and the unreached `export` keyword is this repo's recurring, already-triaged-questionable house convention (same shape as stdlib-anchor-exports-unimported and host-incompatible-kind-export-unimported), not a defect unique to this site — a human should rule wholesale (triage: claude-opus-5)
verdict: questionable — fifth independent pass, re-verified from scratch rather than trusted: excerpts/line numbers reproduce exactly (schema-validator.ts:323/351/376, the 6-line shift matching 359d27ef's PTQ-0009 comment deletion), 0 `export *` in src/tests/tools/extensions, barrel src/seams/index.ts:7-12 omits this name and equally omits siblings AjvSchemaValidator/SchemaSlug/SchemaSlugFn (direct-module import only, so the barrel argument is neutral), 91 `new AjvSchemaValidator(` sites all untyped, SchemaSlug (production-composition.ts:110) and SchemaSlugFn (5 named test files) genuinely have importers so the empty set isn't a search artefact, package.json has no main/types/exports, and repo-wide `git log -S` finds only founding commit a2993196 plus two quality-process commits (bd671ad3, c9a1a45a) touching this text — never a code importer added; my own from-scratch census of the 43 exported `*Deps` interfaces in src/ finds 17 with zero external references (matching the third pass), confirming a live #deps-field/constructor-parameter type sharing its zero-importer condition with ~40% of same-shaped siblings is this repo's recurring convention, not PTQ-0150's proven outlier (there, every sibling of the same shape DID reach a further file) — a human should rule wholesale (triage: claude-opus-5)
