...
verdict: questionable — accounting re-verified at HEAD with one explained drift: size-scan map now places `src/lexer/lexer.ts#scanTokens` at 233-729 (not 310-806) because commit 16614a65 (the PTQ-1113 D4 fix) moved the UTF-8 helpers to src/lexer/encoding.ts above it; the body is untouched, still 497 LOC / band strong (FN strong ≥ 200), and every inventory row reproduces at −77 (template prose 290-329; string arm 370-520 = 151 LOC with arm-local raw/value/closed/hex/braced/braceClosed; stray backslash 522-533; number arm 535-631 = 97 LOC with arm-local value/isFractional/extra; ident 633-648; semicolon 656-670; punct + template-state transitions 671-696; EOF unterminated-template 698-728) with the 8 cross-phase locals (i/line/column, inTemplateProse/interpDepth/templateOpenStart, tokens/diagnostics) declared at 237-263 — the two big arms touch only cursor + sinks, so the rows are real distinct concerns; reasons-considered not overlooked: exemptions.json has no lexer key, `git log --follow -S scanStringLiteral/scanNumberLiteral/scanNumber` is empty (no reverted split), lexical.md has no single-body clause, no measured-cost note in the file; the applicable concrete reasons (closed-enumeration dispatch over lexical.md token classes, ≥ 6 shared locals) are acknowledged and are not strong; not a duplicate — sibling d9-01 is the file-level host (module bundling, different root cause) and no open/resolved PTQ tracks scanTokens' size; per D9 policy the seam shape is a design decision for a human ruling (triage: claude-fable-5-1)
