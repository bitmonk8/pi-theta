...
verdict: questionable — independently re-verified from scratch: both `_settlement`/`_channels` are unread in both route bodies at current invoke:141-159/query:144-162, all 3 in-scope call sites (drifted to production-theta-producer.ts:4096-4110/6565-6578/6648-6651) fabricate `noopSwallowChannels()`, and `InvokeExecutionSideChannels`/`QueryProviderSideChannels`/`*Settlement` have zero references outside their declaring modules and the two test files; but `git show 85a046e1` (V15h-T red stub literally invoked `channels.emitRuntimeEvent`/`emitDiagnostic` to redden cka-33 channels 2-3) and `3c1c0fdc` (the green diff flipping `channels`→`_channels` the same moment `guard` becomes the read field) prove the unread parameter is a deliberate TDD red/green seam, and invoke/query-swallowing-handler.test.ts still plug real recording spies into this exact parameter to assert zero invocation directly — so "no reachable behaviour, only threading" is a contestable framing of a live test-observation seam rather than a clean vestige; a human should rule (triage: claude-opus-5)
