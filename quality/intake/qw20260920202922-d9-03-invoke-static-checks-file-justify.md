...
verdict: questionable — accounting re-verified independently at HEAD: `size-scan map` reproduces 1343 LOC / band justify with all 25 declarations at the cited ranges and five over-threshold function hosts; the only module-level state is the 15-LOC `SCHEMA_REFINEMENT_KEYS` constant (573), so the seven rows are separate top-level function groups with no shared closure state and the orchestrator hands `callSites`/`typeEnv`/`typePass` to each group by parameter (1251-1330); header excerpt matches lines 1-3; back-import is real at invoke-expr-call-surface.ts:22; `grep invoke-static-checks quality/exemptions.json` → 0 hits; no generated marker; no reverted split in git log (prior extractions ba304a0f/c3f8a0ef/f7aae3fc all stuck); reasons-considered list holds (no spec-table switch, no ≥6-shared-local algorithm, ~1% data LOC). Dedupe: PTQ-0321/0351/0413 are `#checkInvokeStaticResolution`-keyed and resolved, PTQ-0370 is a landed misplacement, TRIAGE_LOG.md:64 records the prior file-level candidate as human-deferred "then re-file" with PTQ-0319/0330 and Seam A all in quality/resolved/ — no open file-level root cause tracked, no other intake candidate on this host. Note for the ruling: Seam A's "dissolves the mutual reference" claim is imprecise — `buildInvokeArgSlot` (665) calls `collectProvableArgTypes` from invoke-expr-call-surface.ts, so the move would invert, not remove, the type-level cycle; the target shape is the human's design decision, never confirmed (triage: claude-fable-5-1)
