...
verdict: questionable — independently re-verified from scratch (the file's pre-existing note above was not trusted): all 6 excerpts reproduce verbatim at the cited lines; `parseCalleeTheta` measures 134 LOC via `size-scan.mjs loc --host` and unconditionally calls `resolveThetaToolsAtLoad`→`captureRootClosureHash` before any `tools:`/mode check; a `\.rootClosureHash\b` grep across all of src/ confirms the sole consuming read is production-theta-producer.ts:2538 (`const { theta } = bindInput` inside `spawnSubagentConversation`), and the `callerMode==="prompt" && callee.frontmatter.mode==="prompt"` branch (producer.ts:4929-4998, confirmed by reading the full branch) returns on every path before ever reaching the `spawnSubagentConversation` call that follows — the computed hash is provably discarded on that cell. Confirmed distinct from PTQ-0348 (resolved: that fix added the very per-dispatch `closureSourcesCache`/`closureHashCache` this candidate correctly treats as orthogonal — PTQ-0348 was about reuse across callers, this is about one dispatch's own unread output) and from the wave's sibling d8-02 (different host mechanism, `parseCalleeForTools`'s readdir/realpath probes). subagent.md `#subagent-theta-callable-hash` scopes the hash to the child-process marshalling boundary the in-process attach cell never crosses, so no spec-required behaviour is dropped and no `challenges_spec` is owed. No exemptions.json entry for this host. Per the D8 protocol an accurate heavier-than-scale accounting rests at questionable, never confirmed — whether to skip the hash for this cell is a human design call. (triage: claude-opus-5)
verdict: questionable — independently re-verified a third time from current HEAD (23f847e4), trusting none of the file's prior notes: all 6 excerpts are byte-identical at their content (line numbers drift +10 to +61, fully explained by already-landed unrelated commits — PTQ-0382's tools-entry-gate dedup at 82efa599 removed ~30 lines above `parseCalleeTheta`, plus intervening RFC 0011/0012 commits — not by any fix touching this claim); `parseCalleeTheta` = 134 LOC confirmed via `node tools/quality/size-scan.mjs loc --host`, and unconditionally calls `resolveThetaToolsAtLoad`→`captureRootClosureHash` before any `tools:`/mode check, with `callerMode` unknowable until after that parse completes; an exhaustive `rootClosureHash` grep across all of src/ finds exactly one consuming read, `theta.rootClosureHash` inside `spawnSubagentConversation`, and reading the full `callerMode==="prompt"&&callee.frontmatter.mode==="prompt"` branch body confirms its try/finally (via `runPromptSuspendInvoke`) returns on every path without ever reaching that call. Distinct from PTQ-0348 (resolved: load-time cross-caller reuse, orthogonal to this dispatch-time unconsumed-value claim) and from the wave's sibling d8-02 (`parseCalleeForTools`, a different host/mechanism); docs/spec_topics/pi-integration-contract/subagent.md's `#subagent-theta-callable-hash` and its neighbouring PIC-59 clauses repeatedly scope the hash/envelope machinery to the subagent leg and state outright that "the prompt→prompt attach leg does not serialise ... is unaffected", so skipping the computation for that cell drops no spec-required behaviour and no `challenges_spec` is owed; the heavier-than-scale data-size citation (CTRL-2/CTRL-4, docs/spec_topics/control-flow.md) checks out against the worked `par for`/`invoke` example; no exemptions.json row names this host. Per the D8 protocol an accurate heavier-than-scale accounting rests at questionable, never confirmed. (triage: claude-opus-5)
