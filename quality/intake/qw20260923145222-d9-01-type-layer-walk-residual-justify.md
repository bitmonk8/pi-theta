...
verdict: questionable — accounting verified: re-running size-scan map gives 1639 LOC in the justify band (1000–1999), matching `wc -l`; TypeLayerWalk is 87-1637 = 1551 LOC, and the members sit at the cited map lines; per-row `this.*` tallies show the fn-return, literal and call-arg rows use only the env/file/diagnostics/typeOf context (call-args also uses fnDecls/importedSymbols/shadowedNames); only the statement row reads resultBindings/unprovableBindings, which is the same shape as the TypeWalkContext split already done (type-layer-provable.ts:20-33); there is no type-layer entry in exemptions.json, the 33548129 split was never reverted, and no strong reason was overlooked; not a duplicate, because PTQ-1281 was fixed against the 2693-LOC shape; one inaccuracy that does not change the verdict: the class is exported (:1639), not module-private; the seam shape needs a human ruling (triage: claude-opus-5-5)
