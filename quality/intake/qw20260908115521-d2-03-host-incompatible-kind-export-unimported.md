...
verdict: questionable — independently re-verified every claim (3 in-file hits at :39/:96/:168; no importer/dynamic-access/re-export/barrel across src, extensions, tools, tests; placeholder.ts excerpt now :354-357, content-identical; git founding commit e795597b never gained an importer) and additionally built a `tsc --declaration` fixture under this repo's tsconfig: dropping `export` from the alias while `ProbeFailureDetails` stays exported still compiles clean and the emitted `.d.ts` inlines the full seven-literal union into `ProbeFailureDetails` regardless, so the suggested fix changes nothing observable to any consumer; the carrier (`ProbeFailureDetails`/`ProbeOutcome`) is live in production cross-module, not just tests — `runCapabilityProbe`/`hostIncompatibleDiagnostic` are imported and called at factory.ts:1373/1375; and all ten named sibling `XKind` aliases independently resolve to exactly one file each, confirming pervasive house convention rather than a proven outlier — nothing is dead, the anchor is export-surface taste, a human should rule (triage: claude-opus-5)
