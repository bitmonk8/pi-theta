...
verdict: questionable — every fact reproduces (3 in-file hits at :39/:96/:168; no importer, `export *`, or string-keyed access; placeholder.ts excerpt drifts to :353-360) and the export is provably surplus (a scratch `tsc --declaration` emit keeps a non-exported alias as a local d.ts declaration), but nothing is dead: the alias is alive in-module and was exported from birth (e795597b — no file at any commit ever imported it, so this is not a leftover), its carrier ProbeFailureDetails/ProbeOutcome is imported externally (tests/capability-probe.test.ts:5,7), the renderer cannot take the union without an extension→diagnostics→extension cycle (capability-probe.ts:26 already imports placeholder.ts), and my census finds 258 of 731 exported types in src/ alive-in-file with zero external references, including 10 other `*Kind` string-union discriminators in the identical shape (AssignTargetKind bindings.ts:103/107, CommittedSideEffectKind no-rollback.ts:76/87, PathLiteralKind, ThetaLibDeclarationKind, IndexReceiverKind, NumberKind, SlashCallerKind, PromptTurnKind, CommittedSurfaceKind, ToolCallCalleeKind) — so under the PTQ-0150 outlier test this is house style rather than proven cruft, same ruling as the stdlib-anchor sibling; a human should rule whether a repo-wide no-unimported-exports rule is wanted (triage: claude-opus-5)
