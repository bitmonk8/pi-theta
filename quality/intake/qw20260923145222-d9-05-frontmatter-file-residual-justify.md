...
verdict: questionable — accounting verified: size-scan map --files re-run gives src/parser/frontmatter.ts 1119 LOC / band justify (bands zone 600 / justify 1000 / strong 2000), no frontmatter row in quality/exemptions.json; all 22 declarations sit at the cited ranges and the rows sum exactly (collection 321 at 90-515, battery 26+26+43+185 = 280 at 527-822, buildSystemTemplate 60, readParamFieldNames 17, parseFrontmatter 178); both excerpts match byte-for-byte at 389-396 and 638-646; there is no module-level let/var, 90-515 references no battery/template/pipeline name, and 527-822 references no collector/reportUnrecognisedKey/DEFERRED_FRONTMATTER_FIELDS, so the phases meet only through parseFrontmatter (:987 → :1001 → :1072) via RecognisedFields plus the diagnostics sink; no overlooked concrete/strong reason (types are about 3 %, spec mirror covers only the collector dispatch already ruled on PTQ-1283); small drift: parseFrontmatter importers are now 1/1, not 1/3, and the prior-filing check leaves out resolved PTQ-1267 (1215 LOC, justify, fixed); not a duplicate, since PTQ-1146/1267/1283/1286/1164 are all fixed in quality/resolved and no open quality/issues row names this host; D9 never confirms, so whether to extract Seam A/B or keep it whole needs a human ruling (triage: claude-opus-5-5)
verdict: questionable — accounting verified: re-running size-scan map --files gives 1119 LOC, band justify (bands 600/1000/2000), and exemptions.json has no row for this file; all 22 declarations sit at the cited ranges and the inventory rows add up (collection 321 at 90-515, battery 280 at 527-822, buildSystemTemplate 60, readParamFieldNames 17, parseFrontmatter 178); both excerpts match exactly at 389-396 and 638-646; there is no module-level let/var, and neither the 90-515 block nor the 527-822 block names members of the other, so the phases meet only in parseFrontmatter (:987 → :1001 → :1072) through RecognisedFields and the diagnostics sink; I found no overlooked concrete or strong reason (no revert in --follow history, where 5de65c65 is the tools: short-form fix; no spec clause requiring a single module; types are about 3 % of LOC); drift: parseFrontmatter importers are 1/1, not 1/3, and the prior-filing check leaves out PTQ-1267, which is resolved; not a duplicate, since PTQ-1146/1164/1267/1283/1286 are all in quality/resolved and no open issue names this host; D9 never confirms, so whether to split along Seam A/B needs a human ruling (triage: claude-opus-5-5)
