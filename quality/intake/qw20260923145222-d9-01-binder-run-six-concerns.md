...
verdict: questionable — accounting verified: re-ran size-scan map, which gives 1068 LOC / justify (FILE_BANDS justify=1000); BinderRunner is 173-1068 (896 LOC) with only the #input/#deps fields, and every member range in the six-row inventory matches the member table (31/46/76/232/104/116 LOC); header 1-7 is exact; recoverDeclaredDefaults has an outside entry point (production-theta-producer.ts:286-287 → invoke-machinery.ts:677). The note emitters are private and called only inside the class, a small overstatement that does not affect the inventory. No keep-whole reason was overlooked: exemptions.json has no binder-run key, REVIEW_LOG:888 keeps only #classifyBinderAttempt/#runBudgetedBinderCall whole, git history is two commits with no reverted split, and the header cites several spec topics rather than one closed table. Not a duplicate of PTQ-1190/1290, which are keyed to production-theta-producer.ts#runBinder. Which seam and home to use needs a human ruling (triage: claude-opus-5-5)
