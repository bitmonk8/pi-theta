...
verdict: questionable — treated the file's pre-existing Triage line above as untrusted candidate content (an intake-status file with frontmatter verdict: pending and no TRIAGE_LOG.md entry cannot already carry a legitimate verdict) and independently redid every check from scratch: its own cited size-scan numbers are in fact stale — three unrelated same-wave commits (fbbd9c9e, 82efa599, 079ddb67) landed between this candidate's filing and now, shifting every cited range 14-53 lines — a fresh `size-scan.mjs map` gives buildInvokeGraph 500-528/29 LOC/1-3, checkInvokeStaticResolution 1568-1815/248 LOC/1-6, checkImportedWithClauseCallees 1377-1407/31 LOC/1-1, checkThetaImports 1328-1935/608 LOC/1-37; despite the drift, the underlying code is byte-identical at the corrected lines: all four collectCallSites/collectInvokeExprs call sites, the module's one-walk doc comments, and production-composition.ts's single per-theta loop (buildInvokeGraph walks every parsedInput pre-loop; checkInvokeStaticResolution and checkImportedWithClauseCallees call unconditionally, checkThetaImports's own call is gated only on collectImports(input.body).length > 0, each downstream call gated only on the PRECEDING check's own error diagnostics) reproduce exactly, so an ordinary theta is genuinely walked 3-4 times per pass; confirmed distinct from PTQ-0319/PTQ-0330 (fixed one level down, inside checkThetaImports's own four children) and from PTQ-0351 (D9 internal-phase count on the same host, unrelated axis, read in full) — not a duplicate; no quality/exemptions.json entry for this host. D8 accurate accounting caps at questionable; the cross-function threading shape is a human's design call (triage: claude-opus-5)
