...
verdict: questionable — independently reverified from scratch (this file's pre-existing Triage line above is not treated as authoritative, since it is inconsistent with the frontmatter's own verdict: pending): every citation has drifted 4-43 lines — buildInvokeGraph's two loops are now at invoke-static-checks.ts:500-514/515-526 (29 LOC, 1/3 importers, reconfirmed via `node tools/quality/size-scan.mjs map`), canonicalizePath at invocation.ts:131-136, CachingThetaLibProbe's field/block at import-static-checks.ts:492/506-520, and the call site at production-composition.ts:1295 — because same-wave commit fbbd9c9e ("fix src/extension__p1", 19:07:41) landed after this candidate's own 17:58 filing and inserted an unrelated ~38-line withClausePromptModeRefusal extraction directly above buildInvokeGraph; this is a small, uniform, single-file insertion shift (unlike the discovery-walk.ts cross-file-extraction precedent ruled false-positive), so content still reproduces at/near the cited lines and I do not sink this on citation staleness alone; confirmed canonical()'s byPath is keyed on realpath OUTPUT for node lookup, never on canonical's own pre-canonicalisation INPUT, canonicalizePath -> PiFileSystem.realpath is a raw uncached realpath.native syscall, and no pass-scoped realpath cache exists elsewhere in production-composition.ts; CachingThetaLibProbe.canonicalCache is a real, applicable sibling precedent; no exemptions.json entry for this host (full file checked, 4 entries, none matching); confirmed distinct from PTQ-0295, PTQ-0061, and the same-wave collectCallSites-redundant-walk intake sibling (different mechanism, incidental line overlap only) — matches the PTQ-0348/PTQ-0349 heavier-than-scale precedent exactly: accurate accounting, the cache is a design decision for a human ruling, never confirmed at triage (triage: claude-opus-5)
