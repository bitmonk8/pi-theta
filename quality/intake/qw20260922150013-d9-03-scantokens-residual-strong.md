...
verdict: questionable — accounting re-verified independently at HEAD: size-scan map reproduces `src/lexer/lexer.ts#scanTokens — 471-732 — 262 LOC — band strong` (FN strong ≥ 200; file 732 LOC zone); every inventory row opens at its cited lines (setup + ScannerCursor/ScannerSinks 471-523, template-prose arm 537-575, ident/keyword 636-658, punct + template-state transitions 675-699, EOF unterminated-template 702-731) and the three template-machine locals (inTemplateProse/interpDepth/templateOpenStart, declared 495-501) are written only by the prose arm and the punct transitions and read only by the EOF check while the ident arm touches only cursor + reserved + tokens, so template machine / identifier arm / core dispatch are real distinct concerns; reasons-considered not overlooked — the concrete reasons (lexical.md closed-enumeration dispatch, 6 shared locals) are acknowledged and only strong reasons hold a strong-band host, none exists: quality/exemptions.json has 4 entries, none keyed to lexer.ts or #scanTokens (filing's 'one entry' miscount does not change the no-ruling claim), `git log -S scanTemplateProse` is empty (no reverted split), no measured-cost note in the file, no spec single-body clause; not a duplicate — PTQ-1167 (resolved) covered the pre-split 497-LOC shape and PTQ-1148 (resolved) the file-level host; residual re-files after a landed split follow PTQ-1209/PTQ-1213 practice; per D9 policy the seam shape (TemplateState helper pair vs. scanIdentifier vs. keep-whole) is a design decision for a human ruling (triage: claude-fable-5-1)
verdict: questionable — accounting re-verified at HEAD 801e23a3 with a 2-line drift from the two post-filing lexer commits (0275df1b, f993fb17): size-scan map reproduces `src/lexer/lexer.ts#scanTokens — 469-730 — 262 LOC — band strong` (FN strong ≥ 200; file 730 LOC zone), every inventory row opens at its cited lines −2 (setup incl. ScannerCursor/ScannerSinks objectification, template-prose arm, newline/ws, comments, string + number dispatch through cursor/sinks, ident/keyword arm touching only cursor + reserved + tokens, semicolon, punct + template-state transitions, EOF unterminated-template with invariant throw), and the three template-machine locals are written only by the prose arm and the punct transitions and read only by the EOF check, so template machine / identifier arm / core dispatch are real distinct concerns; reasons-considered not overlooked — the concrete reasons (lexical.md closed-enumeration dispatch, 6 shared mutable locals via cursor closures) are acknowledged in the filing and only a strong reason holds a strong-band host: quality/exemptions.json has 4 entries and none keyed to lexer.ts or #scanTokens (the filing's 'one entry' miscount does not change the no-ruling claim), `git log -S scanTemplateProse -- src/lexer/lexer.ts` is empty (no reverted split), PTQ-1167's ratification is a batch accept with no residual-acceptable note, REVIEW_LOG:637 records the residual as 'already covered' not keep-whole, no measured-cost note in the file, no spec single-body clause; not a duplicate — PTQ-1167 (resolved) covered the pre-split 497-LOC shape and PTQ-1148 the file-level host, and residual re-files after a landed split are established practice (PTQ-1209, PTQ-1213); per D9 policy the seam shape (TemplateState helper pair vs. scanIdentifier vs. keep-whole) is a design decision for a human ruling (triage: claude-fable-5-1)
