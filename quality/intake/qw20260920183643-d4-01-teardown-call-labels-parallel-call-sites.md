...
verdict: questionable — re-verified: all 12 excerpts reproduce byte-exact at the cited lines (roster 61-75, interface 95-101, emitter sites 498/501/531/534/537/566/573/639/662/701); grep confirms exactly 10 `runIsolatedCall`/`teardownStepFailedDiagnostic` emitter sites, both helpers take `call: string` (176-178, 594-598) so 8 free literals are unchecked against the roster, 1 site (566) reads the constant, 1 (573) is typed only by the `ForwardingSignalSource.label` union, which is itself a hand-mirrored copy of the step-5 row (and re-spelled as literals at production-theta-producer.ts:2188/2200/2472, compile-tied to Y but not X); counts X=12 (2+3+4+3), Y=3, Z=10 hold; the only ties are partial test membership checks (session-shutdown.test.ts:359 one label; b0376 test:127-134 the quiesce path only), so the "no mechanism" claim is mildly overstated but the parallel stands for 9 literal sites + the interface union; anchor is mechanical (bug 0376 is a fixed prior instance of exactly this roster/emitter drift and the spec at session-shutdown-semantics.md:15 pins the closed set as wire contract); spec-vector carve-out does not apply (call-site literals cite no clause); no existing/resolved PTQ tracks this root cause (PTQ-0341/0423/0188 and the same-file D9 intake are different concerns) — the shared source of truth (roster-index parameter vs derived literal-union type) is a design decision for a human ruling (triage: claude-fable-5-1)
verdict: questionable — independently re-verified: roster (61-75) and interface (95-101) excerpts are byte-exact; grep lists exactly 10 `runIsolatedCall`/`teardownStepFailedDiagnostic` emitter sites (498/501/531/534/537/566/573/639/662/701 — 639/662 are one line off the cited 638/661, tolerated drift), both helpers accept `call: string` (176-180, 594-598) so the 8 free literals are compile-unchecked against the roster, 1 site (566) reads `TEARDOWN_STEP_CALL_LABELS[4][3]`, 1 site (573) is typed only by the `ForwardingSignalSource.label` union, itself a hand-mirrored copy of the step-5 row and re-spelled at production-theta-producer.ts:2188/2200/2472; counts X=12 (2+3+4+3), Y=3, Z=10 hold; the only ties are partial tests (session-shutdown.test.ts:359 checks 1 label; b0376 test:127-134 checks membership only on the driven quiesce path), so "no mechanism" is mildly overstated but the parallel stands for 9 free-literal sites plus the union type; anchor is mechanical — docs/bugs/0376 is a fixed prior instance of exactly this roster-vs-emitter drift, and session-shutdown-semantics.md:15 pins the closed set as wire contract with "MUST NOT introduce a `details.call` value outside this enumeration"; spec-vector carve-out does not apply (the emitter literals cite no clause); no open/resolved PTQ tracks this root cause (PTQ-0341 = quiesce race body, PTQ-0423 = drain-state arm mapping, PTQ-0188 = capability-probe labels; the same-file D9 intake is a breakdown filing) — per D4-parallel rules the shared source of truth (roster-index parameter vs union type derived from the constant) is a design decision for a human ruling (triage: claude-fable-5-1)
