...
verdict: questionable — accounting re-verified against current code: size-scan map now places `evaluatePureExpression` at 8191-8396 (uniform -36 drift from the cited 8227-8432), still 206 LOC / band strong (FN_BANDS strong ≥ 200, not exempt-band); all 14 arms sit at the drifted lines with the cited LOC (member 8226-8261 = 36, index 8262-8290 = 29, call 8291-8319 = 29, try 8333-8362 = 30) and share only the three parameters, so the arm inventory is real; the bug-0476 catch block is byte-identical (modulo indent) at 8249-8259 and 8279-8289 with the third copy at 8452-8462 inside evaluatePureFnCall (7958 and 8474 are different bodies — retargetInterpolationPanic / pushPanicFrame), and no D4 issue in quality/issues tracks attachPanicSite/attachPanicRange; no overlooked reason — the switch mirrors the closed 20-member `Expr` union (theta-document.ts:474-494) but that is a concrete reason only and design §1.2 requires concrete + strong in the strong band: no cross-arm ordered sequence or PIC/EXST invariant, no measured cost, `git log -S"function evaluatePureExpression"` shows only the rename / bug-0116 / bug-0080 commits (no split/revert), quality/exemptions.json holds only `D8:…#firstAdmittingArmProperties`, and REVIEW_LOG:543's keep-whole is a lens worker note not a human ruling; not a duplicate — d9-01 keys the file host and lists the evaluator family as one row, d9-09 is misplacement, PTQ-1099 is a D2 self-citation drift; the seam shape needs a human ruling (triage: claude-fable-5-1)
