...
verdict: questionable — accounting verified: I re-ran size-scan map --files and got `#scanStringLiteral — 234-380 — 147 LOC — band justify` (FN justify=100, strong=200), and exemptions.json has no lexer key. The excerpt at 280-287 matches exactly. The `\u` arm spans 280-336 (57 LOC, ~39 %), while each simple arm (266-278) is 2-3 lines, so closed-enumeration fails on arm length. The arm's locals hex/braced/braceClosed/wellFormed/cp/isScalar are used only inside it; across the seam it touches only cursor/sinks/file/escStart and appends to value, which is below the ≥ 6 shared-locals bar. On one-grammar-production: the design doc requires that the routine already "call out for every sub-production", and lexical.md:26 gives `\u{XXXX}` its own well-formedness rule and `invalid-unicode-escape` code, yet that code is decoded inline. The earlier TRIAGE_LOG:278 rejection (triage, not a human ruling) rested on the since-removed `raw` accumulator and on function-wide locals; this arm-scoped claim answers both. Minor drift: the dangling-backslash row starts at :257. Not a duplicate: no issues/ or resolved/ PTQ has d9_host #scanStringLiteral (1167/1278 are #scanTokens, 1292 is D8), and `git log -S` shows only eed3cf39/128db09c, with no reverted split. Whether an escape counts as a sub-production, and the seam shape, need a human ruling (triage: claude-opus-5-5)
