...
verdict: questionable — accounting verified on a fresh re-run: size-scan map gives 1219 LOC, justify band (FILE_BANDS justify=1000), and all 21 declaration rows at the cited ranges. Excerpts match at :25-33, :614-618, :658-661, :719-721, :769 and :927-931. The scanners classifyGenericArgumentSegments and findCutBracketGroupText take plain strings and never read LowerCtx, and hoistInlineObjectType gets lowerFieldType as a callback. That makes at least 3 genuinely distinct concerns: row 1's tables belong to the recursion, and row 5's lowerLiteralSublanguage is called from row 2 at :537/:574. No overlooked keep-whole reason: quality/exemptions.json has no params-lowering key, data/type code is about 8%, the file is hand-written, and git log shows only split commit bf57e617 with no revert. Not a duplicate: resolved PTQ-1149/1259 have d9_host src/parser/params.ts, and no open issue or other intake names this host. Whether to cut Seam A/B, or keep the lowering family PTQ-1259 moved as one unit, needs a human ruling (triage: claude-opus-5-5)
