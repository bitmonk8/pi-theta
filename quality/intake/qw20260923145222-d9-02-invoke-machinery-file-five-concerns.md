...
verdict: questionable — accounting verified; target shape needs a human ruling: I re-ran size-scan map and it reproduces 1005 LOC / band justify (FILE_BANDS justify=1000). InvokeMachinery is 145-942 (798 LOC) with only the #input/#deps fields, and every member range in the inventory matches the member table. The groups are distinct concerns that pass data by explicit parameters, with 4 shared values (under the 6 bar). The only switch is the 3-arm #resolveReturnSite. No keep-whole reason was overlooked: exemptions.json has no key for this file, git history (f1b77776/39670a25/48b6a1b7) shows no reverted split, and no other filing uses this d9_host. The filing has minor inaccuracies that do not overturn the inventory: the typed-return row is 124 LOC, not 163; validateInvokeReturn is public and also reached through production-theta-producer.ts:294-295 → subagent-spawn-regime.ts:1616; #resolveReturnSite is called from #driveCallee:358; and InvokeReturnSite's importer is subagent-spawn-regime.ts:110 (triage: claude-opus-5-5)
