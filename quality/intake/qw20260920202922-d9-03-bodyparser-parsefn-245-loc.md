...
verdict: questionable — accounting verified independently: size-scan map reproduces BodyParser.parseFn 3629-3873, 245 LOC, band strong (FN strong ≥ 200), no exemption entry; excerpt byte-exact at 3789-3793; the five param-list flags are block-scoped to the `if (this.isPunct("("))` block 3647-3819 with zero reads after it (other file hits are parseFor 3590 and parseSchemaObjectBody/parseEnumVariants 4261+), S4/S5 own returnType/returnTypeAbsorbed/withClause/body, so the inventory is two clusters sharing only `params` — the ≥6-shared-locals reason does not apply at the natural seam; grammar.md:133 §"`fn` declarations" is concrete-only and insufficient at strong; not closed-enumeration dispatch, not data/type LOC, hand-written, `git log -S parseFnParamList` shows no reverted split, no spec clause pins S2 to S4/S5 (bug 0279's provenance mark is a shared concept, not a shared local); no PTQ or sibling filing tracks this host (d9-16's parseFnTail is a different symbol; d9-01 is the file-level host) — the parseFnParamList seam is a design decision for a human ruling (triage: claude-fable-5-1)
verdict: questionable — accounting verified: size-scan map (FN strong ≥ 200) gives BodyParser.parseFn 3633-3877, 245 LOC, band strong, 4-line drift from the cited 3629-3873; excerpt byte-exact at 3792-3797; the five S2/S3 locals (openTok, unclosed, closeParenAbsorbed, refusedTok, atParamStart) are declared inside the `if (this.isPunct("("))` block 3651-3823 so they are lexically unreadable by S4/S5 (the only other in-file hits are comments at 1290/3594/4265-4282 and parseSchemaObjectBody/parseEnumVariants' own openTok at 4280/4548), and S4/S5 own returnType/returnTypeAbsorbed/withClause/body, so the inventory is two real clusters sharing only `params` — the ≥ 6-shared-locals reason does not apply; grammar.md:133 §"`fn` declarations" is a one-production concrete reason, insufficient at strong; no closed-enumeration dispatch, not data/type LOC, hand-written; quality/exemptions.json has no theta-document/parseFn entry; `git log -S parseFnParamList` is empty (no reverted split); no PTQ names BodyParser.parseFn and no other intake file cites it — the parseFnParamList seam is a design decision for a human ruling (triage: claude-fable-5-1)
