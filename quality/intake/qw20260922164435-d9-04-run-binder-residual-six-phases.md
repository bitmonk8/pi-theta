...
verdict: questionable — accounting re-verified independently: size-scan map gives `| 1022-1208 | 187 | public | runBinder |` band justify (2-line drift from the filed 1024-1210, body text unchanged; `#runBudgetedBinderCall` 1211-1284 / 74 LOC is PTQ-1190's landed extraction), excerpt byte-exact at 1165-1169, every row edge lands on a real `return`/construction boundary (1054/1113/1133/1164/1179) with 1055-1071 and 1134-1139 being comment preambles, so the six rows sum to 187 though rows 3+4 (envelope+context → dispatch build) are one concern and row 5 is 15 LOC of glue — honest inventory ~4 concerns, still ≥2; exemptions.json carries only `D8:…#firstAdmittingArmProperties`, no D9 key for this host; not a duplicate — PTQ-1190 is in quality/resolved/ as fixed (human batch-ratified 2026-09-21, no keep-whole clause on the residual) and same-wave d9-01 carries the file-level key listing runBinder only as one member of a cluster row; reason (b) is considered not overlooked — my cross-row count is exactly 6 (params→rows 3/4/6, model/envelopeSchema/sessionContext→row 4, dispatch→row 5, call→rows 5/6) with strictly linear hand-off, four early returns reading nothing forward, and no loop accumulator or try/finally spanning rows, the same rebuttal the human accepted on this host at 7 locals (unlike the #driveUserVisibleTurn rejection, where a prior keep-whole ruling existed); whether a second cut after the first extraction is worth it, and which of Seams A/B/C, is a design decision for a human ruling (triage: claude-fable-5-1)
