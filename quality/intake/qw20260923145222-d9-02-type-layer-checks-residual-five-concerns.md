...
verdict: questionable — accounting verified; target shape needs a human ruling: re-ran size-scan map, which gives 857 LOC / band zone (FILE_BANDS zone=600), and quality/exemptions.json has no key for this host; my own grep finds that row-1 members (91-255) and row-5 members (761-841) have zero in-file callers — the only hits are the declarations, self-recursion at 169/207/829, doc comments at 237/475, and the 843-856 export block — while rows 2-4 share no locals with them, so the ≥ 2-concern inventory stands; this is not a barrel (~780 LOC payload) and there is no reverted split since PTQ-1158 (bd8e73b7); one claim is wrong: row 1 is not "solely" consumed by type-layer-operand-checks.ts — classifyOperand is also imported by type-layer-provable.ts:10, ORDERING_OPS/ARITHMETIC_OPS by type-layer-interpolation.ts, PRIMITIVE_NAMES by annotation-compat.ts:4, NO_SUNK_ARRAYS by type-layer-walk.ts, and fnParamNamesAreIdentifiers by invoke-imported-checks.ts/type-layer-walk.ts — which undercuts Seam A's home but not the accounting; REVIEW_LOG:625 (qw20260922150013) kept this host whole at 802 LOC on a reviewer precedent, but that is not an exemptions.json key, so the human should weigh it (triage: claude-opus-5-5)
