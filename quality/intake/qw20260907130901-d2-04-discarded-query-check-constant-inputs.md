...
verdict: questionable — seventh independent re-verification, unchanged: sole production caller is theta-document.ts:9596 (import :109), passing literal `isQuery: true`/`"bare-expr-statement"`; fresh grep of `disposition:` string-literal constructions across src/extensions/tools finds only that one site — no producer of `propagate`/`discard-let-underscore`/`bind`; git log confirms d88e07ec (V13g-T) → cbd64750 (V13g) → 684485e8, the wiring commit whose own message states the disposition "is always `bare-expr-statement`" at this site, so the constant was designed-in, not decayed; the anchor is therefore mechanical, matching confirmed PTQ-0020/PTQ-0044 — but there, the non-trigger arm was unread by production AND every test, while here tests/query-discard.test.ts:118-138 drives all four dispositions plus `isQuery: false` directly and whole-program-parser.test.ts:565-636 independently pins, at the AST level, that every accepted form never reaches this call site as a `QueryStmt`; PTQ-0164 (same file, resolved) is confirmed distinct (the QRY-20 `form` field, not QRY-19's disposition model), so no dedupe; collapsing the four-arm/boolean model into "only the site" would strip genuinely tested, spec-anchored surface for a benefit a human should weigh, not a mechanical fix (triage: claude-opus-5)
