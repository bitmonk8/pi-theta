...
verdict: questionable — own fuzz of 500,000 brace-leading strings through the real exported `annotationToCompatType` plus an exhaustive case proof (no `{`-leading text can hit `prim`/`array`/an identifier-shaped `named`, and any top-level union maps to `undefined` regardless of its arms) confirms removing the guard is output-identical; but `git show 72272a20:src/parser/query-schema-resolve.ts` (the commit that added this file, +568 lines) already contains the `startsWith("{")` guard and the `named`-arm regex byte-for-byte, so "the general gate subsumed the special case, and the special case was left behind" is false — the two were born together, not sequential — and docs/bugs/0097 Residuals (4) ("[c]onfirmed out of this class ... and left alone") plus 0130 §Fix (f) (both comments named as a HELD pair) show the overlap was deliberately examined and kept, unlike PTQ-0005's guard which the same taxonomy confirmed only because no such retention history exists there; taste-level belt-and-braces call for a human (triage: claude-opus-5)
