...
verdict: questionable — accounting verified: size-scan map on a one-line manifest gives 4056 LOC / band strong (FILE_BANDS strong=2000; −20 LOC since filing via later commit 7dcc46b8, still >2× threshold) with every inventoried member present at ranges drifted ≤20 lines (parseSchema 1434, parseImportExport 2094, parseType 2406, parseMatch 3237, parseParFor 3699, parseQuery 3932) and row sums re-added (schema/enum 461, import 255, type 173); par-for-body-checks import verbatim at :27 and sole emitParForBodyDiagnostics call at :3760; rows are disjoint grammar-chapter member groups over the shared cursor, a reason the filing weighed; exemptions.json D9 keys are only binder-system-prompt#normaliseParamLineBreaks and package-discovery; git log shows 702a1f2e→cfd246b6→19516518→7dcc46b8, no reverted split; not a duplicate — PTQ-1280 is resolved (ratified, only Seam A landed) and the only other intake hit (qw20260923185337-d9-01) is keyed on the function host #BodyParser.parseQuery, not the file — the residual seam shape needs a human ruling (triage: claude-opus-5-5)
verdict: questionable — accounting verified: size-scan map (one-line manifest) gives 4056 LOC / band strong (FILE_BANDS strong=2000; 20 LOC fewer than the filed 4076 after commit 7dcc46b8, still more than twice the threshold), BodyParser 357-4048, and inventoried members within 20 lines of the cited ranges (parseSchema 1434, parseEnum 1897, parseImportExport 2094, parseType 2406, parseMatch 3237, parseParFor 3699, parseQuery 3932); the par-for-body-checks import is verbatim at :27 and the sole emitParForBodyDiagnostics call is at :3760; a `this.` census shows the schema, import/export and type groups read only cursor methods, diagnostics/file and their own helpers, so they are separate grammar-chapter member groups rather than one concern split by adjectives; the shared-cursor reason was weighed, and no overlooked concrete or strong reason was found: the only D9 keys in exemptions.json are binder-system-prompt#normaliseParamLineBreaks and package-discovery, and git log shows 702a1f2e→cfd246b6→19516518→7dcc46b8 with no split reverted; not a duplicate, because PTQ-1280 is resolved (fixed; only Seam A landed) and the only other intake hit (qw20260923185337-d9-01) is keyed on #BodyParser.parseQuery rather than the file; the residual seam shape needs a human ruling (triage: claude-opus-5-5)
