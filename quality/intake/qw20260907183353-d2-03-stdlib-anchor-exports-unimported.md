...
verdict: questionable — independently redid every check rather than trusting the three prior notes: all six sites/excerpts byte-exact, sole importer type-layer-checks.ts:100 (`checkStdlibMethodCall` only), and my own greps confirm zero real importer anywhere for any of the six — the lone external hit, STDLIB_ARITY_MISMATCH_CODE at b0315live:64, is a local `const` re-declaration, not an import (no `from ".*stdlib-arg-diagnostics"` names it); but a repo census I ran myself refutes the "unfulfilled anchor-import contract" framing rather than confirming it: 11/20 `_HINT`, 6/32 `_CODE` and 5/21 parametrized `*Message` exports in src/parser alone have zero external refs, invoke-diagnostics.ts's FN_ARITY_TOO_FEW/TOO_MANY CODE+Message()+HINT sextet is a structurally identical zero-import sibling, and the documented *Diagnostic message anchors* MUST rule deliberately routes witness tests to the registry instead of these constants — whereas PTQ-0150's confirmed pair (bare `_MESSAGE` + `*Diagnostic()` builder, now de-exported) is a mechanically different, genuinely rare shape, since I confirmed 9/9 other `*Diagnostic()` builders and 25/27 other `_MESSAGE` constants elsewhere in src/ do have external refs; this candidate hits the widespread convention-backed shape, not proven cruft; a human should rule (triage: claude-opus-5)
verdict: questionable — re-verified independently from scratch, not by trusting the four prior notes: all six excerpts/line numbers reproduce byte-exact, sole importer is type-layer-checks.ts:100 (`checkStdlibMethodCall` only, 5-line drift from the filed :95), and the only other repo-wide hit for any of the six names is the local, non-importing `const` re-declaration at b0315live:64 (plus local literal re-declarations, not imports, at b0315-stdlib-arg-surface.test.ts:59-60 and b0439-kind-belt-message-honesty.test.ts:343) — so the finding's mechanics are true; but I independently reproduced the counter-census too: 13/22 exported `_HINT` constants repo-wide have zero external refs (verified each by name), and invoke-diagnostics.ts's FN_ARITY_TOO_FEW/TOO_MANY_CODE + fnArityTooFewMessage/fnArityTooManyMessage + FN_ARITY_TOO_FEW/TOO_MANY_HINT sextet is a structurally identical zero-import sibling whose own witness test (arg-mismatch-diagnostic-count-by-surface.test.ts:109-110) likewise re-declares the literals under different local names rather than importing; the *Diagnostic message anchors* rule those exports' doc comments invoke is itself now retired prose (docs/plan_topics/conventions.md is a pruned stub) yet the behavior it prescribes — witness tests sourcing literals instead of importing these anchors — is exactly what every cited test still does today, confirming a live, still-followed cross-module convention rather than one file's neglected cruft; a human should rule (triage: claude-opus-5)
verdict: questionable — re-verified from scratch: all six sites reproduce byte-exact (:41/:44/:52-59/:65-73/:76-77/:80-81), sole importer is type-layer-checks.ts:100 (`checkStdlibMethodCall` only, matching the filed :95 with drift), and my own repo-wide grep for the six names finds only the b0315live:64 local `const` re-declaration, not an import, plus `export *` count = 0 in src/ (no barrel re-export); but I independently confirmed invoke-diagnostics.ts's FN_ARITY_TOO_FEW/TOO_MANY_CODE + fnArityTooFewMessage/fnArityTooManyMessage + FN_ARITY_TOO_FEW/TOO_MANY_HINT sextet (:85/:88/:178/:191/:219/:223) is consumed only at :513-529 with zero external refs of its own — a structurally identical sibling in the same directory — and the *Diagnostic message anchors* rule this module's own doc comments invoke (and which is still cited live in src/binder/binder-model.ts, src/extension/watcher-recovery.ts, src/parser/query-schema-inference.ts, src/runtime/query-discard.ts) directs witness tests to source the literal from the registry doc rather than import the module constant, which is exactly what the b0315live test does — so the never-imported shape is prescribed convention, not this file's own neglected cruft, and unlike PTQ-0150 (confirmed only because every sibling export of its shape elsewhere DID have external refs) this shape has a live zero-ref sibling; a human should rule (triage: claude-opus-5)
