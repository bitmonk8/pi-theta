...
verdict: questionable — accounting verified (re-check): theta-document.ts:1406 (toolCallableName) and :1448 (piToolCallableName) test `/^[A-Za-z_][A-Za-z0-9_]*$/` inline on the whitespace-split parts[0] spec, the same check callable-set.ts:573-575 `isBareIdentifier` makes (its doc: Pi-tool name vs `.theta` path), and resolveEntry routes on that function at :429; theta-document.ts:75 already imports thetaDefaultName from ./callable-set, so the import adds no new edge; callable-lowering.ts:261, tools-entry-gate.ts:49 and runtime-tools.ts:100 delegate while theta-document has 0 hits; the other src/ matches of the regex check field/param/key shape; the bug-0106 "DELIBERATELY wider" rationale covers the parts.length>=3 split, not the spec predicate, so behaviour is unchanged and no spec clause is touched; resolved PTQ-1435 cited only callable-lowering/tools-entry-gate, and PTQ-1236 only the thetaDefaultName arm, so this is not a duplicate; exemptions.json D8 rows cover only discovery-walk/production-theta-producer; D8 reimplemented is never confirmed, so the simpler shape needs a human ruling (triage: claude-opus-5-5)
