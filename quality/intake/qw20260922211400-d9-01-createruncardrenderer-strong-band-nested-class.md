...
verdict: questionable — accounting re-verified independently: size-scan map on the host file lists `#createRunCardRenderer — 101-335 — 235 LOC — band strong` and `RunCardComponent.#renderLive — 142-309 — 168 justify` under "over threshold" (the file's own 335-LOC `exempt` band governs only file-level breakdown filings, not this function host); every inventory row's range (103-121 fields+ctor, 123-131 render/invalidate, 133-140 #renderStatic, 142-309 #renderLive, 312-335 entry closure) matches the current file and the 101-107 excerpt is verbatim; grep of `deps\.` inside the class body (lines 134/143/146/155/232/287 — staticFallback/clock/bus/cardStateFor/readSourceBytes/ensureLut) confirms `deps` is the ONLY enclosing-scope binding the 211-LOC class reads, so class-vs-entry-closure is a real 2-concern inventory sharing one binding; no overlooked reason (the 8-shared-locals algorithm lives inside #renderLive and is untouched by a class hoist; no spec-table switch; imperative hand-written; not data-dominated), quality/exemptions.json has no run-card key, `git log --follow` shows only c5a3d05a (no reverted split); importers reproduce at run-card-renderer.ts:60/236 (1 src / 0 tests); dedupe: PTQ-1260 (resolved) d9_host is run-card-renderer.ts#createRunCardController and its Seam A text ("renderer factory takes a small deps record") never ruled on the nested-class shape, PTQ-1256 is the double-snapshot root cause, no other PTQ or intake names this host — whether to hoist `RunCardComponent` to module scope is a design decision for a human ruling, never confirmed (triage: claude-fable-5-1)
verdict: questionable — accounting verified from scratch: `node tools/quality/size-scan.mjs map` on the host file lists `#createRunCardRenderer — 101-335 — 235 LOC — band strong` and `RunCardComponent.#renderLive — 142-309 — 168 LOC — band justify` under "over threshold" (FN_BANDS strong ≥ 200; the file's 335-LOC `exempt` band bars only file-level breakdown filings, and the d9_host here is the function key); the 101-107 excerpt is verbatim and every inventory row (fields+ctor 103-121, render/invalidate 123-131, #renderStatic 133-140, #renderLive 142-309, entry closure 312-335) matches the member table; `grep deps\.` over 102-310 hits exactly six sites (file lines 134/143/146/155/232/287 → staticFallback/clock/bus/cardStateFor/readSourceBytes/ensureLut) and the factory's only other locals (`seed`/`cardTheme`/`tracked`) live inside the entry closure, so the 211-LOC class and the 24-LOC entry closure are two concerns sharing the single `deps` binding — no ≥ 6-shared-locals reason at the host (the 8-local algorithm is internal to #renderLive and unaffected by a hoist), no spec-table switch, hand-written, imperative; `grep run-card quality/exemptions.json` is empty; `git log --follow` shows only c5a3d05a (no reverted split); importers reproduce at run-card-renderer.ts:60/236 (1 src / 0 tests); dedupe: PTQ-1260 (resolved) is keyed to run-card-renderer.ts#createRunCardController and its Seam A text only says "renderer factory takes a small deps record" without ruling on a nested class, PTQ-1256 is the double-snapshot root cause, no other PTQ/intake names run-card-component.ts — hoisting the class is a shape decision for a human ruling (triage: claude-fable-5-1)
