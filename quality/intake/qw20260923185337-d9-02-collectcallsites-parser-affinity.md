...
verdict: questionable — accounting verified; target shape needs a human ruling: re-verified at HEAD — the pair (invoke-static-checks.ts:163-179, :206-233) touches 6 parser/theta-document members (type imports :94-100, `walkCallSiteNodes` :101) and nothing from its host except each other; grep reproduces every importer (parser/with-clause-static-checks.ts:23/:353, import-static-checks.ts:98/:584, invoke-imported-checks.ts:57 +5 parameter sites, host :192/:301/:742, no test callers); the reverse value import at :146 closes a real parser↔extension cycle; the sibling pattern holds (`collectClauseBearingCalls` theta-document.ts:746, `CallSiteNode` :770, `CallSiteWalkOptions` :778, `walkCallSiteNodes` :808); PTQ-1266:60 names `collectCallSites` as a move blocker and 65fe42f7 is that fix; minor slip: of the 5 `../extension` hits in src/parser, 2 (not 3) are system-note-channel and 2 are collectProvableArgTypes, which does not affect the affinity count; no open or intake item keys this placement (triage: claude-opus-5-5)
