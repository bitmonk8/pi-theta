...
verdict: questionable — accounting verified: all 5 cited locations match verbatim (types.ts:1-10 header+import, types.ts:192-194 childEvent member, child-tap.ts:43-55 declaration, bus.ts:42-43 import, bus.ts:283-296 switch), affinity re-counted both ways reproduces exactly (child-tap.ts touches its own type 8x: 2 signature params + 6 publish({type:…}) construction sites, one per variant; types.ts touches it 1x; bus.ts touches it via 1 signature + 3-of-6 switch tags matched, other 3 fold to default), and the sibling-pattern count reproduces (8 other producer-payload/snapshot types live in types.ts at the cited lines; checkpoint-decorator.ts hosts none; quality/exemptions.json's 4 entries name none of these files; no duplicate in quality/issues or quality/resolved — PTQ-0369 and PTQ-0316 are unrelated concerns on the same/nearby files); minor flaw noted but not disqualifying: the false-positive check's own "5 hits" tally undercounts its own itemized list (which sums to 7, matching my independent grep) — but per D9 misplacement policy accurate accounting caps at questionable, never confirmed; the move is a human design ruling (triage: claude-opus-5)
