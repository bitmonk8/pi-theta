...
verdict: questionable — independently re-verified at HEAD: all four citations content-match (consumer excerpt drifted to :806-811, content identical; frontmatter/declaration range likewise off by ~1-8 lines) and my own `turnKind` grep across src/, tests/, extensions/, tools/ reproduces the same 12 hits with zero `"free_phase"` producers for `FollowUpSurfacingTurn`; confirmed the candidate mislabels tests/query-tool-loop.test.ts:335 as such a producer — it is a bare `MaskedPredicateInput` literal handed straight to `computeMasked`, no `FollowUpSurfacingTurn` in sight; the "why"'s load-bearing claim is also false — `followUpSurfacing?.turnKind ?? "forced_respond"` only substitutes on absence, so a minted `"free_phase"` would flip `computeMasked` from its otherwise-reachable `["ceiling#2"]` to `undefined`, a real behavioural effect, not a repeat of the default; and both of `computeMasked`'s only two production call sites (query-tool-loop.ts:712, :807) likewise always resolve `turnKind` to `"forced_respond"`, so the sibling `MaskedPredicateInput.turnKind` arm this type mirrors is equally unproduced in production and only unit-exercised — this is a repo-wide, spec-driven typing choice (PIC-1(d)'s named two-valued scalar), not an isolated vestige, and the introducing fix's review already logged the identical trait as non-blocking residual R2 (docs/bugs/0355-repair-terminal-masked-parent-slot-count.md:270) and kept it deliberately — a settled shape-vs-precision design call, not a proven defect, for a human to rule on (triage: claude-opus-5)
