...
verdict: questionable — independently re-verified at HEAD: all four citations content-match (declaration/mints off by 1 line, consumer drifted ~8 lines to :806-813, content identical); my own repo-wide `turnKind` grep reproduces the same 12 hits with zero `"free_phase"` constructions of `FollowUpSurfacingTurn` anywhere, confirming tests/query-tool-loop.test.ts:335 is mislabeled (a bare `MaskedPredicateInput` literal passed straight to `computeMasked`, not a `FollowUpSurfacingTurn`), so the arm is genuinely unproduced; but tracing `computeMasked`'s own conjunction (runtime-event-channel.ts:122-141) shows the "why"'s load-bearing claim is false, not taste — `?? "forced_respond"` is the predicate's positive-enabling default, so a minted `"free_phase"` would suppress an otherwise-reachable `["ceiling#2"]`, a real effect, not a repeat of the default; and PIC-1(d) (runtime-event-channel.md:128/132) names "the surfacing turn kind (forced-respond vs. free-phase)" as a normative two-valued scalar this type mirrors alongside the equally production-unproduced `MaskedPredicateInput.turnKind` (both live `computeMasked` call sites, query-tool-loop.ts:712/:807, always resolve to `"forced_respond"`) — and the introducing commit's own review (0dc66dcf) already logged this exact trait as non-blocking residual R2 (docs/bugs/0355-repair-terminal-masked-parent-slot-count.md:270) and kept it, which the candidate's filed checks never surfaced (no git-intent check run) — a settled design call, not a proven defect, for a human to rule on (triage: claude-opus-5)
verdict: questionable — independently re-verified at HEAD, distrusting all four prior notes and re-deriving from source: locations content-match (typed-query-validation.ts:332 byte-exact; the others drift ≤8 lines, e.g. consumer now at query-tool-loop.ts:806-813) and my own `turnKind` grep over src/, tests/, extensions/, tools/ reproduces the same 12 hits with zero `"free_phase"` constructions of `FollowUpSurfacingTurn` — the arm is genuinely unproduced, and structurally so, since the sole upstream seam `FollowUpRespondOutcome` (typed-query-validation.ts:106-118) carries no turn-kind field for either mint to read; but the report's own "no distinguishable consumer effect" claim does not hold up — `computeMasked` (runtime-event-channel.ts:122-140) requires `turnKind === "forced_respond"` to reach its only positive `["ceiling#2"]` result, so a hypothetical `"free_phase"` value would suppress that outcome rather than repeat a default; the shape instead deliberately mirrors PIC-1(d)'s spec-named two-valued "surfacing turn kind (forced-respond vs. free-phase)" scalar (docs/spec_topics/pi-integration-contract/runtime-event-channel.md:128, confirmed verbatim) shared with sibling `MaskedPredicateInput.turnKind`, and the introducing commit's own review (0dc66dcf, confirmed in git log) already logged this exact trait as non-blocking residual R2 (docs/bugs/0355-repair-terminal-masked-parent-slot-count.md:270, text matches verbatim) and the team kept it — a settled shape-vs-precision design call, not a proven defect, for a human to rule on (triage: claude-opus-5)
