...
verdict: questionable — accounting verified: I re-checked the excerpts and they match (frontmatter-params.ts:52-57 uses a plain `split("|")`, :15 imports `skipQuotedRegion` from ./type-text-split, :232 stores `nullable` on every BypassParamsField; type-text-split.ts:426-434 `splitTopLevel` sits on `splitTopLevelSegments`, which has a typed opener stack and quote tracking at :359-414). Both cited needs, angle nesting and quoted literals, are covered by the facility, so `array<a | null | b>` and `"a|null|b"` come out false with it where the naive split gives true. The only src reader of `.nullable` is binder-envelope.ts:211, inside the `type === "string"` guard. The two tests that assert it (params-brace-union-rhs-lowering:1333, params-inline-object-lowering:676) use null-free inputs and pass either way. The host has no row in exemptions.json. PTQ-1146 names the function only in a D9 inventory, and PTQ-1140/1141 do not cover it, so this is not a duplicate. The spec's top-level semantics drop no behaviour. Fixer caveat: inline-object `params:` values need `"angle-and-brace"` nesting, because `{a: string | null | b}` still gives true under the default. The simpler shape is a design decision for a human ruling (triage: claude-opus-5-5)
