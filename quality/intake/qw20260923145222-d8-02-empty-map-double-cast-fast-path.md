...
verdict: questionable — accounting verified: lexical-call-sites.ts:195-214 matches the excerpt exactly, with one `size === 0` special case and one uncommented `as unknown as` cast at :199-201; grep of src/ extensions/ tools/ tests/ finds one live caller (theta-document.ts:390, imported at :110) and one comment mention (annotation-compat.ts:114); runtimeToolPresentedNames (runtime-tools.ts:91-108) returns a fresh Map on every call, so the branch only avoids one empty-Map allocation, and the general loop returns an equal empty map; quality/exemptions.json has no row for this host and no spec clause pins the behaviour; the harm is a latent type-mislabel hazard, not a live defect, so the simpler shape needs a human ruling (triage: claude-opus-5-5)
