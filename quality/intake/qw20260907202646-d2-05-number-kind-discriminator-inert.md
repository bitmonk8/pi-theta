...
verdict: questionable — independently re-verified: all six excerpts match (2–3-line drift at three sites), `canonicalDecimal` takes only `value` so `kind` is provably unobservable at all 6 call expressions (in 4 caller functions, not the candidate's "five"), and tests :38/:67 and :45/:86 pin identical bytes for both kinds; but the "vestigial" anchor fails — `git log` shows the arms born byte-identical in the sole implementing commit e78482b9 with the "sole selector" rationale already present, and the identity is deliberately documented in three places (canonical-number.ts:69-72, schema-lowering.ts:837-843, tests/echo-array-per-element-descriptor.test.ts:582-585) as traceability to the two separately normative rules BNDR-4/BNDR-5 (spec :36-37, two spec-anchored describes), so unlike the accidental identity branches confirmed in PTQ-0102/0149/0076 this is a documented design choice the candidate contests on shape grounds it itself concedes; collapsing the switch would leave `kind` unread (a PTQ-0001/0020-class smell) or force a signature change across 4 production files and ~13 test calls in 3 files — cost plausibly exceeds benefit, a human should rule (triage: claude-opus-5)
