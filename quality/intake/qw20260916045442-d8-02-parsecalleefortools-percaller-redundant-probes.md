...
verdict: questionable — independently re-verified all 6 excerpts and both call sites of resolveThetaToolsAtLoad (1329 in runComposePass's loop, 4184 in parseCalleeTheta) myself; read pass-parse-cache.ts, pass-verdict-memo.ts, and the closure-cache code directly and confirmed none of the four pass-scoped caches (passParseCache: parsed document only; passVerdictMemo: {fails, ownEscapes} only, and its own doc-comment concedes a hit needs nestedContainment "not at all" yet parseCalleeForTools computes it unconditionally beforehand; closureSourcesCache/closureHashCache: PTQ-0348's distinct closure-hash computation, confirmed wired at 4230/4271) memoises onDiskCalleeName's fs.readdir or probeNestedToolsContainment's fs.realpath probe; confirmed PiFileSystem.readdir/realpath (src/seams/pi-file-system.ts) are bare fs.promises/realpath.native passthroughs with no cache; confirmed distinct from PTQ-0349, whose own doc-comment (3016-3022) scopes its landed fix to "ONCE here" within one parseCalleeForTools call, not across separate referencing callers, which is exactly this finding's residual; no exemptions.json entry for this host; per D8 protocol an accurate heavier-than-scale accounting caps at questionable, never confirmed, matching the identical pre-ratification treatment of this same reporter's sibling PTQ-0348/PTQ-0349 on this exact call chain (triage: claude-opus-5)
verdict: questionable — re-verified against current HEAD myself rather than trusting the two entries above: 5 of 6 citations (1,3,4,5,6) hold, content matching at ≤16-line drift, but citation 2 (2656-2669) is stale — its exact inline parseToolsEntry/toolsEntrySpec/isBareToolName excerpt is the pre-fix code PTQ-0382 (D4, now resolved) quoted at this identical location before extracting it into the shared admissibleToolsSpec helper; PTQ-0382's own resolved file explicitly names this pending filing and disclaims covering it ("this finding is that the ADMISSION GATE itself... has no shared implementation", not the probe cost), confirming no duplicate. That refactor is orthogonal to this claim: calleeCache is still confirmed function-local (only declaration is inside resolveThetaToolsAtLoad, now at 2671), and neither onDiskCalleeName's fs.readdir (3071) nor probeNestedToolsContainment's fs.realpath probe (via checkInvokePathAtLoad -> checkInvokePathContainment -> canonicalizePath, invocation.ts:131-136, a bare realpath call) is covered by passParseCache/passVerdictMemo/closureSourcesCache/closureHashCache (all four traced directly in source) or by PiFileSystem's bare fs.promises passthrough (pi-file-system.ts) — the cross-caller redundancy holds. Distinct from resolved PTQ-0348 (closure hash) and PTQ-0349 (own doc-comment scopes its fix to "ONCE here," within one call, not across callers); no exemptions.json entry for this host (the one D8 heavier-than-scale entry present is a different function, discovery-walk.ts#enumerateDirectory, where a human rejected an analogous "extra readdir isn't worth a mechanism" argument — supporting, not requiring, the same cap here). D8 heavier-than-scale accounting verified accurate; the cache is a human cost/benefit call, never confirmed by triage (triage: claude-opus-5)
