...
verdict: questionable — independently re-verified against current HEAD (unchanged since the prior pass): declaration (:186-193) and all 5 statement-executor.ts call sites (:1287, :1609, :2006-2007, :2076-2077, :2303) reproduce verbatim; `grep -n "deps\.mutator\|deps\.mode" src/runtime/statement-executor.ts` = the same 7 lines/5 statements with no other read of either field anywhere in src/ (theta-composition-producer.ts's `.mode` hits are the unrelated `theta.frontmatter.mode`); terminal-outcomes.ts's two-`void` body is unchanged, and diffing 3465acf2→4ac46f54 confirms the only calls `mutator`'s methods ever received were the deliberately non-compliant V4c-T stub's, so the empty body is the landed ERR-8…ERR-12 contract (docs/plan_topics/coverage-matrix.md:25, "ERR-8 … ERR-12 | `V4c`", verbatim); both fields entered together with their witness in 886feda4, and tests/statement-executor.test.ts:586-638 (verbatim) threads a RecordingMutator through these exact fields, including an explicit `mode: "subagent"` case, then asserts `mutator.calls` is `[]` — a deliberate absence-of-mutation witness, not leftover cruft. Confirms the prior pass's finding that PTQ-0171's fix (35df0ce3) deleted the candidate's second cited production constructor outright — exactly one live production `mutator:`/`mode:` literal remains (:2198-2199) — so 1 of 11 cited sites is gone outright rather than merely drifted, while the root claim (both fields threaded through live, reached production code yet functionally unread by their sole consumer) otherwise stands untouched. A genuine design trade-off (executor-level witness seam vs. terminal-outcomes.test.ts's own unit-level witness) for a human to rule, not a false-positive; not a duplicate of PTQ-0171/PTQ-0032/PTQ-0108/PTQ-0078 (triage: claude-opus-5)
verdict: questionable — fresh independent re-verification concurs: every excerpt reproduces at current HEAD (declaration :186-193; call sites :1287/:1609/:2006-2007/:2076-2077/:2303; `deps.mutator|deps.mode` grep = same 7 lines/5 statements, no other read in the file; terminal-outcomes.ts :86-88/:103-105 unchanged; `NoopConversationMutator` at :731-737) and confirms commits 3465acf2/4ac46f54/886feda4 and coverage-matrix.md:25 as quoted; the "vestigial" anchor still fails because tests/statement-executor.test.ts:586-638 (verified verbatim) threads a `RecordingMutator` through these exact fields via `deps({..., mutator, mode: "subagent"})` and asserts `mutator.calls` is `[]` for both prompt- and subagent-mode cancellation — a deliberate absence-of-mutation witness the interface exists to carry, not dead cruft; independently reconfirmed that PTQ-0171's fix (35df0ce3) removed the second production `ExecuteBodyDeps` construction outright (theta-composition-producer.ts now types the subagent arm as `SelfDrivenConversationBinding` with `executeDeps?: undefined`), leaving exactly one live production `mutator:`/`mode:` literal (:2198-2199) — 1 of 11 cited sites gone outright, not mere drift — while the root claim (fields threaded through live, reached production code yet unread by their sole consumer) otherwise stands; a genuine design trade-off, not a false-positive, and not a duplicate of PTQ-0171/PTQ-0032/PTQ-0108/PTQ-0078 (triage: claude-opus-5)
verdict: questionable — independently re-verified once more at current HEAD, no drift since the last pass: declaration :186-193 and all 5 call sites (:1287, :1609, :2006-2007, :2076-2077, :2303) match verbatim; `grep -n "deps\.mutator\|deps\.mode" src/runtime/statement-executor.ts` = the same 7 lines/5 statements, no other read anywhere in src/; diffing 3465acf2→4ac46f54 myself confirms the only `mutator.truncate/rewrite/injectCompensatingTurn` calls ever made were the deliberately NON-COMPLIANT V4c-T red-stub's, so terminal-outcomes.ts's unchanged `void outcome; void mutator;` body is the landed ERR-8…ERR-12 contract (coverage-matrix.md:25 verbatim: "ERR-8 … ERR-12 | `V4c`"); tests/statement-executor.test.ts:586-638 (verbatim) threads a `RecordingMutator` through these exact fields, including `mode: "subagent"`, and asserts `mutator.calls` toEqual `[]` — a deliberate absence-of-mutation witness, not leftover cruft, so "vestigial"/"discarded" is the wrong anchor even though the mechanical claim (fields reach only a callee that voids them) holds; independently confirmed PTQ-0171's fix removed the candidate's second production constructor outright (`grep -rn "mutator:" src` → exactly one live site, :2198), and PTQ-0032/PTQ-0108/PTQ-0078 are unrelated fields/files, not duplicates — a genuine design trade-off for a human to rule, not a false-positive (triage: claude-opus-5)
