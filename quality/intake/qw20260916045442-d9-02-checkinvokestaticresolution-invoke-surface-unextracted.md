...
verdict: questionable — independently re-verified against current HEAD (not the pre-existing note above, which repeats the candidate's own stale line numbers uncritically): size-scan confirms checkInvokeStaticResolution now at 1515-1759/245 LOC/strong, 1/6 importers, live caller production-composition.ts:1397; every cited line in this candidate is a uniform +14 offset from current, fully explained by the same-wave PTQ-0370 fix (commit c3f8a0ef) relocating checkImportedFnCallArgs out of this file's header/imports shortly before this filing — content, LOC and bands of the 150-LOC invoke loop and all four sibling delegations match verbatim once corrected; INV-1/3/6/8/4 anchors verified at invocation.md:14/44/59/63/95; checkWithClauseDefaultReject's real 4-param signature (no typeEnv/typePass) confirms the 5-shared-locals-under-6-bar claim; PTQ-0321 and PTQ-0351 read in full — both inventoried this exact loop as its own phase but neither's Suggested-direction ever proposed extracting it, so this is fresh ground, not a re-file; no exemptions.json entry, generated-code marker, or reverted-split commit — but D9 breakdown target shape remains a human ruling, never confirmed (triage: claude-opus-5)
verdict: questionable — re-verified again against current HEAD, now a further wave stale from both notes above: `node tools/quality/size-scan.mjs map` shows checkInvokeStaticResolution has moved AGAIN, to 1568-1815/248 LOC/strong, 1/6 importers (neither prior note's 1529-1773 nor 1515-1759 holds any more; the shift is commit fbbd9c9e, wave qw20260916144930, extracting an unrelated shared `withClausePromptModeRefusal` helper — PTQ-0364's mode-gate dedup — not an extraction of the invoke loop itself). The invoke(...) loop is confirmed still fully inline at 1610-1762 (153 LOC, ~62% of the function, six ordered sub-checks in the claimed order, same `clauseRefused`/`arity`-gated shape); setup 1568-1609 (42 LOC), the four sibling delegations 1763-1804ish (matching content verbatim, incl. the exact `checkThetaCallableCallSurface` call-site quote), tail 1805-1815 (11 LOC) — every quoted excerpt in the candidate reproduces verbatim at its shifted location, so this is drift, not refutation. Independently confirmed: the five shared locals (`diagnostics`/`callerPath`/`callSites`/`typeEnv`/`typePass`) stay under the 6-bar and `checkWithClauseDefaultReject`'s real signature still omits `typeEnv`/`typePass`; INV-1/3/6/8/4 anchors verified at invocation.md:14/44/59/63/95; `checkRuntimeToolCallSurface`'s doc comment confirmed to state "Mirrors `checkThetaCallableCallSurface`'s structure" verbatim; no `quality/exemptions.json` entry, no generated-code marker, no reverted-split commit; PTQ-0321 and PTQ-0351 (both resolved) re-read in full — their Suggested-direction sections propose Seams A/B/C only (`.theta`-callable loop, Pi-tool disjointness, with-clause default-reject), never the `invoke(...)` loop, so this is unaddressed ground, not a re-file; not a duplicate of PTQ-0364 (D4, the mode-gate clone, a narrower/different root cause already fixed by the very commit that shifted these lines) or of the two newer same-shard candidates touching this host (`qw20260916144930-d8-01`, `collectCallSites` re-derivation; `qw20260916144930-d9-03`, `runComposePass` in a different file) — but D9 breakdown target shape remains a human ruling, never confirmed (triage: claude-opus-5)
