...
verdict: questionable — accounting verified with drift: since filing, Seam C landed (quality/resolved/PTQ-1122, fixed in 1332eb76 → `wrapInvokeCalleeFailure` 558-612), so size-scan map now reports `runToolCallEffect 343-508 — 166 LOC — band justify` (same band; FN justify=100), no exemptions.json entry; the four routes reproduce at 359-404 (theta-callable, 46 code / 55 with comment, selector 360), 405-421 (with-clause gate, selector 412), 422-460 (runtime-tool, selector 429), 461-507 (Pi-tool, `resolveToolCall` 461) and are genuinely route-private — every non-comment occurrence of child/invokeOutcome/result/rtCall/settlement/call/outcome sits inside its own route (410/445 are comments, 488 is `outcome.result`), so the ≥6-shared-locals reason is not overlooked; closed-enumeration reason is real (tool-calls.md:36-40 Pi-tool/registered-theta/runtime-tool table) and with the longest arm now 55 LOC the keep-whole vs Seams A/B call is a human ruling; no reverted prior split in git log; not a dup of PTQ-0594 (test scaffolding) or PTQ-0940 (D7 fixture), and PTQ-1122 covered only Seam C (triage: claude-fable-5-1)
