...
verdict: questionable — clone is real and re-verified: both excerpts reproduce verbatim at their current spans (copy 1 relocated by PTQ-0370 to src/extension/invoke-imported-checks.ts:226-241; copy 2 unchanged at src/parser/type-layer-checks.ts:2672-2690), `clone-scan.mjs map --files` still lists G055 (65 tokens, renamed-only(6)), both copies live (checkImportedFnCallArgs pushed from import-static-checks.ts:1679; checkFnCallArgs called at type-layer-checks.ts:3207), type-system.md:54 "Absent operands" is a real single spec clause (not a normative vector table), and the one-directional "mirroring … identical guard" comment rules out incidental similarity — but the fixer's twice-stated blocker also verifies, so this is not the mechanical dedupe a D4 clone verdict presumes: both predicates the guard calls are defined only in type-layer-checks.ts (4006 LOC, band strong, Home-band-disqualified per quality/README.md:245), a parser/ sibling importing them while type-layer-checks.ts imports the guard back is the import cycle the rule forbids, and the only compliant route relocates annotationToCompatType/annotationSourceIsNotTypeExpression out of that host across 7 importer files — a D9-scale decision for a two-branch guard, so a human must either accept --note the relocation direction or reject as not worth its cost (triage: claude-fable-5-1)
verdict: questionable — re-verified independently: both excerpts reproduce verbatim at the current spans (copy 1 at src/extension/invoke-imported-checks.ts:226-241 after PTQ-0370's move, copy 2 unchanged at src/parser/type-layer-checks.ts:2672-2690), `clone-scan.mjs map --files` with a one-line manifest of invoke-imported-checks.ts still lists G055 (65 tokens, renamed-only(6), exactly those two spans), both copies live (checkImportedFnCallArgs spread into checkThetaImports at import-static-checks.ts:1679; checkFnCallArgs called from the "call" walk at type-layer-checks.ts:3207), type-system.md:54 "Absent operands" is one prose clause (not a normative vector table), and the one-directional "mirroring checkFnCallArgs's identical guard" comment rules out incidental similarity — but the fixer's twice-stated blocker verifies too: annotationToCompatType (:951) and annotationSourceIsNotTypeExpression (:1311) are defined ONLY in type-layer-checks.ts, which size-scan reports at 4006 LOC / band strong, so quality/README.md:249's Home-band rule bars the helper from landing there, a parser/ sibling that imports the predicates and is imported back by type-layer-checks.ts is a cycle, and the only compliant route relocates the two predicates out of a strong-band host across 7 importer files (invoke-imported-checks, invoke-static-checks, params, query-schema-resolve, theta-document, type-layer-checks, runtime/query-schema-lowering) — a D9-scale move for a two-branch guard, so this is not the mechanical dedupe a confirmed D4 clone presumes; a human must accept --note the relocation direction or reject as not worth its cost (triage: claude-fable-5-1)
