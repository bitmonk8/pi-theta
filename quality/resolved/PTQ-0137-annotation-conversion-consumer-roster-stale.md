...
## Triage
verdict: confirmed — my own census reproduces exactly the 12 cited production call sites; the four bullets cover 8 (:891; :819/:1146; :651/:652/:699; :439/:2051) while :1359 `paramsFieldBindings` (live via :362), :2147 `checkSubagentReturnAnnotation`, :2674 `checkFnCallArgs` and invoke-static-checks.ts:1451 `checkImportedFnCallArgs` (resolves under `libraryEnv`, not the bullet's EMPTY `TypeEnv`) fall under none, so "four consumers besides" / "any of the five" / :1561's "those four" are provably wrong though a widening would move each omitted site — excerpts verbatim at cited lines, the untouched `letAnnotationToCompatType` "ONLY caller" claim is accurate (sole call :1564), and no intake duplicate (triage: claude-opus-5)
