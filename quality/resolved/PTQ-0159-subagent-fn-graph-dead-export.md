...
## Triage
verdict: confirmed — re-verified: decl at :187 (doc :179-186), sole call at :250; repo-wide grep of `buildSubagentFnGraph` over src/tests/tools/extensions/docs/.pi gives only those two hits, no re-export, no `import * as` of this module, no string-keyed access, and the only public entry (extensions/index.ts) exposes just src/extension/factory — the `export` modifier reaches no consumer. (triage: claude-opus-5)
