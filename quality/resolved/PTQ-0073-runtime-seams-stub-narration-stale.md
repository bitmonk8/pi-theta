...
## Triage
verdict: confirmed — all 15 excerpts byte-match at the cited lines and my own `grep -n stub` over the nine files returns exactly 15 lines, one per cited site with none missed; every named function is implemented in the same file (runEffect dispatches :596-614, evaluateSource tokenizes/parses :92-97, checkBooleanPosition emits the non-boolean-condition diagnostic :579-607, discardForVoid returns null :72-77, renderLeafKindNote/renderTopLevelErrNote render the SNK rows + SLSH-5 suffix, checkInvokePathContainment/AtLoad/Recheck/runStaticResolutionPass all real, runInvokeChild awaits the checkpoint :131, enforceInvokeParams/ReturnDepth run wireFormDepthWalk, selectCalleeContext carries the non-inverted mapping :132, jsonDepth/depthWalk/routeDepthBoundary :141/:195/:218) with zero `not implemented|UNIMPLEMENTED|TODO|stub`-throw hits anywhere in the nine, and blame proves the scaffolding anchor: each block was authored in its `*-T` tests-task commit (6d5efdcf V3a-T, 2f720178 V3d-T, 9cc4b7f8 V15m-T, 1fedfb71 V12b-T, fb387e9c V19d-T) beside genuine stub bodies (`return null`, `return tailValue`, `return { kind: "cancelled", committed: [] }`) that the paired leaves (81667f13 V3a, 3e952b45 V15m) replaced while `git log -S "V15m-T stubs this inert"` shows the comment untouched since, and siblings terminal-outcomes.ts:17 / query-error.ts:20 carry the past-tense landed-pair form so retention is leftover rather than house style (D2 brief line 39 names historical narration comments); locations are disjoint from the six other intake files that cite these sources for unrelated causes and from the runtime-modules / runtime-query-modules / render-runtime same-class siblings, so not a duplicate; the Observation's "six ... the other nine" miscounts the actual 7 function-docs + 8 module headers, but `sites: 15` and the location list are correct and the two chronology-style headers (invoke-cross-mode.ts:27-36, depth-walk.ts:32-37, each followed by an implementation paragraph) are the weakest of an otherwise flatly contradictory set (triage: claude-opus-5)
