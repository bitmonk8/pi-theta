...
## Triage
verdict: confirmed — re-verified independently: repo-wide `callCount` grep reproduces 24 hits and none reads the field on a `BinderCallResult` (producer reads only `.kind` at :1096 and `.outcome` at :1106 through a transparent generic pass-through, the witness test has zero `callCount` hits, and no destructuring/string-keyed access/export-star barrel exists), so the field is written at :143 and read nowhere — dead in production and not test-reachable, so the tests-are-callers carve-out does not apply (triage: claude-opus-5)
