...
verdict: questionable — accounting verified: size-scan map reproduces 815 LOC / zone (FILE_BANDS.zone=600) with no exemptions.json row; the declaration table matches all six inventory rows; sed-extracted halves 77-414 / 415-815 grepped for every counterpart symbol show zero code references (only the :84 doc-comment prose), the parse half importing only ../parser/* + Diagnostic types and the runtime half only value/depth-walk/query-error; importer roster holds (parse: exactly the 3 cited, theta-document at :126 not :115; runtime: the 2 cited plus statement-executor.ts:64 and effectful-statement-host.ts:72, no importer spans both halves); reasons-considered hold (type/const 84/372 ≈ 23 %, no shared locals, not an enumeration or grammar production, hand-written, git log shows tool-call-*.ts siblings added never deleted so no reverted split, the V14a "one seam" header is a task description not a spec-pinned invariant); not a duplicate (no issue carries d9_host tool-call.ts; intake d4-15 is the splitTopLevelUnion clone); caution: buildCodeToolArgSchemaViolation:649-660 documents deliberate co-location with enforceCodeToolArgDepth, bearing on seam B only — the split shape is a design decision for a human ruling (triage: claude-fable-5-1)
