...
verdict: questionable — fifth independent pass, same conclusion: deadness re-confirmed from scratch (0 real `.invalidate(` calls in src/, tools/, extensions/ bar one unrelated comment at stale-ctx.ts:5; tests/ hits are only the wrapper's own uncalled delegation body at invoke-return-enum-carrier-projection.test.ts:245 plus four `StaleHarness.invalidate()` calls in hot-reload-stale-ctx-replacement.test.ts modelling Pi's own zero-arg runtime-invalidate at :138, confirmed unrelated; witness suite tests/schema-validator-seam.test.ts has 0 hits; barrel src/seams/index.ts is type-only; `git log -S 'invalidate(schemaSlug' -- src/` = only 8595ed6c/07bb79b6, no caller ever added); new corroborating detail: `git show 860e989d` — the very commit that rewrote host-interfaces-services.md's PIC-11 intro from "the interface shape is normative" to today's GOV-18-arm-(a) disclaimer ("the normative contract is the behavioural list above") — left the :48 "Architectural constraints ... the file watcher calls into the service's invalidate path" paragraph as an untouched context line in that same diff hunk, and that paragraph's own origin (d826fae5) added it to implementation-notes.md to fix cache-ownership (no module-level singleton), pairing it in the same breath with "the watcher MAY evict ... but invalidation correctness does not depend on it" — so whether :50's disclaimer folds :48 in (member safely deletable) or leaves it standing as a live architectural MUST (deletion needs a spec edit or a real caller) stays unresolved by the text itself, not a mechanical question — a human should rule. (triage: claude-opus-5)
verdict: confirmed — human ruling (thomasa, 2026-09-11): accept - direction: DELETE the member (interface, implementation, the two test doubles) and CORRECT the spec sentence at host-interfaces-services.md:48 (the compiled-validator cache is keyed by the schema slug = canonical-form content hash with byte-equality verified on every hit, so a changed schema can never hit a stale entry and no invalidation path has a job; GOV-18(a) makes the member set non-binding). Do NOT wire the watcher - that would be a behaviour change for a mechanism with no purpose
