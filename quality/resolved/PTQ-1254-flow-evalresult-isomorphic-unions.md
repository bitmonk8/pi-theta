...
verdict: questionable — accounting re-verified independently: Flow 403-410 and EvalResult 422-429 are 7-arm unions differing only in tag key (`kind`/`flow`) and one arm name (`normal`/`value`); terminalFlow 438-453 is a 6-arm verbatim carry and the `"block"` arm 807-823 its 7-arm inverse; grep reproduces exactly 5 shuttle ternaries (1551/1556/1561/1566/1685) and 7 further terminalFlow(...) forwards (1573/1585/1597/1630/1694/1760/1788); the only importers are par-for-executor.ts:12 and subagent-fn-call.ts:9 (tests/b0307 mentions terminalFlow in a comment only, 0 test importers); no `D8:src/runtime/statement-executor.ts` exemption, no docs/spec_topics clause names either type, PTQ-1154/1163/1173 are D9 breakdown rows not a duality claim; one inaccuracy stands — par-for-executor.ts:110 `parForOutcomeOf` maps Flow→ParForIterationOutcome, not Flow→EvalResult, which does not touch the in-host inventory; whether the statement/expression nominal separation (795-800 "a block is an EXPRESSION, not a statement") justifies the shuttling is a design decision for a human ruling, never confirmed under D8 (triage: claude-fable-5-1)
verdict: questionable — accounting verified a third time against current code: Flow 403-410 and EvalResult 422-429 are 7-arm unions differing only in tag key (`kind`/`flow`) and one arm name (`normal`/`value`); terminalFlow 438-453 is a 6-arm verbatim carry (fail/propagate/return/break/continue/cancel) and the `"block"` arm 807-823 its 7-arm inverse; grep reproduces exactly 5 shuttle ternaries (1551/1556/1561/1566/1685) and 7 further terminalFlow(...) forwards (1573/1585/1597/1630/1694/1760/1788); only importers are par-for-executor.ts:12 and subagent-fn-call.ts:9 (tests/b0307 mentions terminalFlow in a comment only); quality/exemptions.json has no statement-executor key; docs/spec_topics names neither type; PTQ-1154/1163/1173 are resolved D9 breakdown rows listing the pair as inventory members, not a duality claim — no duplicate; one inaccuracy stands (par-for-executor.ts:110 parForOutcomeOf maps Flow→ParForIterationOutcome, not Flow→EvalResult) without touching the in-host inventory; whether the statement/expression nominal separation (795-800 "a block is an EXPRESSION, not a statement") earns ~50 LOC of shuttling is a design decision for a human ruling, never confirmed under D8 (triage: claude-fable-5-1)
