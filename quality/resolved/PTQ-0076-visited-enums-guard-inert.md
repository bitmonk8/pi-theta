...
## Triage
verdict: confirmed — verified at :284/:328-345: `visitEnum` has no recursion and no statement after `visitedEnums.add`, so the guard's return skips only its own feeding write while storage above it is already first-wins idempotent; my own repo-wide grep finds the identifier only at those 3 lines (declaration, read, write) with no test/tool/doc reader, and `git blame`/`log -S` confirm it was born inert in d03f7398 (triage: claude-opus-5)
