...
## Triage
verdict: confirmed — all three excerpts verbatim at cited lines and dispatchers undocumented at 77/125/167; `git show d23c22be~1` proves each block (incl. the dangling ` *` scar) was attached to its dispatcher before that commit inserted the const between them; constants/dispatchers live in type-layer-checks.ts:248-252, statement-executor.ts:1474-1483, producer:8312-8321; no jsdoc tooling; disjoint from d2-06 (producer-only) and the runtime-panics/theta-document/discovery-walk detached-doc filings (triage: claude-opus-5)
