...
## Triage
verdict: confirmed — both copies re-verified at cited lines with byte-identical bodies (only name/doc differ), clone-scan reproduces G054 renamed-only across exactly these two spans, both copies live (callers at query-schema-resolve.ts:131 and type-layer-checks.ts:356), in-scope D4 clone in src/, no existing PTQ tracks this root cause (triage: claude-fable-5-1)
