...
## Triage
verdict: confirmed — both excerpts are verbatim at :395-397/:461-463 and both target facts reproduce (spec_topics/type-system.md:48 is the `NamedType`-head parse-time-refusal paragraph, *Unresolvable operands* at :50 carrying the opposite disposition; lexical.md:15 is the PascalCase bullet, the lowercase-first "schema field names" half at :16), `git show 24093511` proves site 1 is real decay (type-system.md:48 WAS *Unresolvable operands* at the authoring commit, v0.212.0), the stated searches reproduce exactly (2 hits; the file holds no `.ts:NN` or bare `:NN` pin either) and both methods are live (`#matchArmScope` :409, `#memberType` :498 — not the FP-check's :488), and the alternate-copy check holds (docs/reference/type-system.md:65, no docs/reference/lexical.md); correcting the filing: site 2 never drifted (c1e663a6 had the same :15/:16 numbering) and :15 does carry the cited `enum`-variant-PascalCase clause verbatim, so only the lowercase-first half misdereferences — imports.ts:358-360 pins :15/:16 for exactly those two halves — and that comment names no section title, so the direction paragraph's "titles in both comments" is wrong for it; not a duplicate: the identical :48→:50 target fact is confirmed against type-compat.ts's pins, a different citing file, and no intake covers this file's comment lines (triage: claude-opus-5)
