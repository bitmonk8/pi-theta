...
## Triage
verdict: confirmed — all 3 excerpts verbatim at 88-96/129-140/152-162; blame shows each trailing paragraph is original V9r text (5590aeba) left in place under a later fuller rewrite (d3fe75c0), so the residue is mechanical not taste; count accurate (only 4 multi-para comments exist, the 4th correctly excluded as additive) and no overlap with the two named siblings (theta-document.ts, drain-state.ts) (triage: claude-opus-5)
