...
verdict: questionable — re-confirmed independently: sites 1/3/4 (subagent-isolation.ts:16-22, subagent-json-driver.ts:1, :19-21) reproduce byte-for-byte at the cited lines; site 2 (subagent-isolation.ts:49-54) is gone, its whole rationale block deleted along with the already-fixed PTQ-0051; both absence claims hold on a fresh grep (`preSpawnModelGuard` has zero declarations/imports/exports, only comment hits in subagent-isolation.ts:18, production-theta-producer.ts:2292, tests/subagent-isolation.test.ts:9,64; `subagent-rpc-driver.ts` is not a file anywhere in `git ls-files`); but `git blame` shows every surviving line was authored in 4866d4d2 itself — the RFC-0006 commit whose own message records retiring the RPC driver — as a contemporaneous provenance note, not decayed narration, and every "moved elsewhere" pointer resolves to real live code (`guardResolvedModel` subagent-model-guard.ts:101, `confirmChildModel` :146, the PIC-66 `killChild` in subagent-json-driver.ts:360-363), and "not kept as a fallback" is true (only one driver file in src/runtime); this refutes the "carries no information about the code as it stands" premise, so the anchor is a taste call over accurate migration comments, not a mechanically demonstrated defect (triage: claude-opus-5)
verdict: questionable — independently reproduced without relying on prior notes: sites 1/3/4 (subagent-isolation.ts:16-22, subagent-json-driver.ts:1, :19-21) match verbatim at the cited lines; site 2 (subagent-isolation.ts:49-54) is entirely absent from the file, its whole PIC-62 block removed by ae6733f3 while fixing PTQ-0051; both absence claims hold on a fresh check (`preSpawnModelGuard` has no declaration/import/export anywhere, only comment hits at isolation.ts:18, production-theta-producer.ts:2292, tests/subagent-isolation.test.ts:9,64; `git ls-files | grep -i rpc` returns only `tests/helpers/fake-rpc-child.ts`, no `subagent-rpc-driver.ts`); but `git blame` puts every surviving line in 4866d4d2, the RFC-0006 commit whose own message records deleting the 360-line RPC driver and `preSpawnModelGuard` in the same diff, making these contemporaneous migration notes rather than decayed leftovers, and every "moved elsewhere" pointer is live and checked directly (`guardResolvedModel` subagent-model-guard.ts:101 called at production-theta-producer.ts:2294 directly beneath its own copy of this note, `confirmChildModel` :146, PIC-66 `killChild` at subagent-json-driver.ts:360-362) — this refutes the "carries no information about the code as it stands" premise, leaving only a taste call over the 3 surviving sites for a human to weigh (triage: claude-opus-5)
verdict: confirmed — human ruling (thomasa, 2026-09-11): accept - the pointers resolve, but naming deleted files/functions is historical narration (D2 smell list; house comment rule); restate the current-state facts (pre-spawn guard lives in subagent-model-guard.ts; the parent-side driver is the JSON envelope consumer) without naming the removed artefacts
