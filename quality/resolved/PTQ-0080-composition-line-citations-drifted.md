...
## Triage
verdict: confirmed — re-verified all three in the current tree (`:808` is the Bug-0178 hoist comment, `sink.emitGroup(parsed.dropped)` is at :972; callable-set.ts:442 is the `unresolvable-theta-path` arm, the `prompt-mode-callable` raise is the sole code site at :494; schema-validator.ts:126-136 is `declaresFilteredProperty`, the collision arm is :397-411), and git blame/show proves each anchor was accurate when written and has since drifted (triage: claude-opus-5)
