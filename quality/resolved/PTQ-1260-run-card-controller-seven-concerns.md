...
verdict: questionable — accounting verified from scratch: size-scan map on the host file reproduces createRunCardController 247-628 / 382 LOC band strong and RunCardComponent.#renderLive 389-527 / 139 justify (file itself 628 zone); the 250-257 state excerpt and every inventory row's member range match the current file; grep of the six closure bindings confirms the partition (cards only at 260-272 in cardStateFor; tui at 278-284/578/595-598 in requestRender/sink/attachTui; terminalBg/lut/lutKey at 299-314 in ensureLut plus the 614-615 OSC-11 resolve arm; staticFallback at 381/538) and animationOwed 556-573 reads only its parameters + HEAT_FADE_MS, so no ≥ 6-shared-locals single-algorithm reason spans the seven rows; no closed-enumeration switch, hand-written, imperative not data-dominated; quality/exemptions.json has no run-card key; git log shows only e4c4f5d5 (D5 landing) and bde73af5 (D6 supersession) — no reverted split; importers reproduce at factory.ts:41/500 and tests/execution-status-run-card-renderer.test.ts:15/84; dedupe: no PTQ names this host (PTQ-1146..1218 D9 set has none; PTQ-1241 is endpoint-ladder D2; the sibling qw20260922150013-d8-01 on the same d8_host is a heavier-than-scale double-snapshot claim, a different root cause), REVIEW_LOG:616 records only this wave's filing — the seam shape (A/B/C) is a design decision for a human ruling, never confirmed (triage: claude-fable-5-1)
