...
verdict: questionable — accounting verified at HEAD with one path drift: PTQ-1266's fix (commit 65fe42f7) moved the module to src/parser/with-clause-static-checks.ts, where both excerpts match byte-for-byte at the SAME lines 144-203 / 241-271; both live (checkWithClauseDefaultReject imported at invoke-static-checks.ts:145 and called at :818; checkImportedWithClauseCallees re-exported at invoke-static-checks.ts:146 and called at production-composition.ts:1477; test callers tests/call-with-clause-erratum-b.test.ts, tests/call-with-clause-static-checks.test.ts); clone-scan map on a one-line manifest of the new path reports "(no clone groups)" so the pair is structural not token-level; partition recounted as filed (pass 1 admits callable-set `theta` + same-file `subagent fn`, defers imported names, convicts `pi-tool` + default; pass 2 admits only materialised imported `subagent fn`, skips set-bound names, convicts the rest with the identical WITH_CLAUSE_IN_PROCESS_CALLEE code/message/hint trio); dedupe: PTQ-0364 (prompt-mode gate, resolved) and PTQ-1266 (D9 move, resolved) are different root causes and no open or resolved filing names these two functions as a D4 pair; d4_class parallel with accurate accounting → questionable — docs/spec_topics/invocation.md INV-8 itself prescribes the two judgement points (same-file at load pass, imported once materialised), so whether a shared predicate is worth extracting is a human design ruling; any acceptance should re-point locations to src/parser/ (triage: claude-fable-5-1)
triage worker failed (verdict not applied; re-triaged next wave) (loop, 2026-09-23)
verdict: confirmed — RATIFIED (human, 2026-09-23): confirmed - batch ruling; triage verification trusted. TARGET SHAPE: ONE classification function; the pre- and post-materialisation passes both call it (or the pre-pass result is carried to the post-pass) - no re-derivation.
