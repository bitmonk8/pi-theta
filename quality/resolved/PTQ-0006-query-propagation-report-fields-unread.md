...
## Triage
verdict: confirmed — re-verified: `queryRange` has 3 repo-wide occurrences (decl + 2 writes, zero reads) and `annotationSource`'s only read copies it into that never-read report field; sole consumer `indexQueryPropagations` reads `.capture` alone, with no re-export, barrel, dynamic/string-keyed access, serialization, or test reader, and 76489c61 shows both fields landed readerless. (triage: claude-opus-5)
