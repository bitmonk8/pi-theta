...
## Triage
verdict: confirmed — verified at HEAD: `piNames` (:1512) and `piOwnedByName` (:1513-1521) are both built by an unfiltered pass over the same `readonly piOwned` keyed on `command.name: string`, `grep -w piNames` across src/extensions/tools/tests yields only the declaration, the sole read `piNames.has(name)` (:1541), and two test prose mentions, and that guard's own block already calls `piOwnedByName.get(name)` (:1545), so the Set is a redundant duplicate index; `git log -S` confirms the Map post-dates it (070a1ef3 2026-07-01 vs 69e8f632 2026-09-05) (triage: claude-opus-5)
