...
## Triage
verdict: confirmed — all three excerpts are verbatim at the cited lines; `git blame` shows the RFC-0006 commit (4866d4d2) amended header lines 6-8 yet left the drive-model sentences from the RFC-0005 commit (fda23a4b), which the same file contradicts at :389 and :412 ("the child owns the interpreter under RFC 0006"), `grep -i subagent src/runtime/conversation-drive.ts` returns 0 hits (launch is driven from production-theta-producer.ts:2585), no subagent-rpc-driver.ts exists, and "one invocation per process: envelope → self-exit" is confirmed in four production files; distinct from the parent-PID (:63-70) and root-marker (:53-61) candidates on non-overlapping lines (triage: claude-opus-5)
