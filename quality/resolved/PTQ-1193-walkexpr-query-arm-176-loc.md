...
verdict: questionable — accounting re-verified at HEAD with post-filing drift: `size-scan map --files` (mktemp manifest) now gives `walkExpr — 9612-9964 — 353 LOC — band strong` (filed 10063-10442/380; the −27 LOC and −451 line shift are resolved PTQ-1131's fix in bb21dacc, which replaced the invoke arm's inline returnSchema block with `validateTypeAnnotation` from ./annotation-validation — so the invoke arm is now 9675-9706 = 32 LOC, not 59, and the filing's optional Seam B is already landed and moot), band unchanged (FN_BANDS strong=200), no `theta-document` key in quality/exemptions.json; the root-cause row holds untouched — query arm 9765-9940 = 176 LOC (now 50 % of the function; 106 comment / 70 executable lines), excerpt byte-exact at 9783-9787, reads only `e`/`refs.typeNames`/`file`/`out`, never `scope`/`bareObjectAllowed`, never recurses into walkExpr, every `const` arm-block-private so no ≥6-shared-locals reason applies; 16 `case` labels + default cover the 20-member code-declared `Expr` union at 474-494 (filing's 21 at 463-483 is a miscount/drift, immaterial; closed-enumeration is concrete-only and the 176-LOC arm defeats the short-arms premise); `git log -S checkQueryAnnotation` empty and bb21dacc left the query arm inline (no prior split to have reverted); dedupe clean — PTQ-1131 (resolved) covered only the invoke arm, qw20260921001431-d9-01 targets `#walkStatement`, d9-04 targets `type-layer-checks.ts#TypeLayerWalk.walkExpr`, no PTQ in issues/ or resolved/ carries `d9_host: src/parser/theta-document.ts#walkExpr`; whether the query-annotation body becomes a helper beside queryResponseAnnotation/queryErrorModelAnnotation (7711/7737) or in annotation-validation.ts is a design ruling for a human, never confirmed (triage: claude-fable-5-1)
