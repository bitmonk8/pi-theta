...
verdict: confirmed — RATIFIED (human, 2026-09-17): Home A - move the six typed-query gate declarations (TYPED_QUERY_SUPPORTED_PROVIDER_APIS, TYPED_QUERY_UNSUPPORTED_PROVIDER_CODE, typedQueryUnsupportedProviderMessage, TypedQueryProviderCheckInput, checkTypedQueryProviderSupport, synthesizeUnsupportedProviderTransportError) to a new src/runtime/typed-query-provider-gate.ts with a header citing conversation-drive.md 'Provider compatibility for typed queries'. Mechanical move; update importers INCLUDING tests (typed-query-provider-gate.test.ts, the b0480 live cell) - re-point imports, never weaken assertions. provider-error-mapping.ts keeps the classifier half and drops the gate from its header.
