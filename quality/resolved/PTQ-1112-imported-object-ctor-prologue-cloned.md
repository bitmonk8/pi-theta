...
## Triage
verdict: confirmed — both excerpts verified verbatim at 325-341/515-531; clone-scan.mjs reproduces G032 (79 tokens, renamed-only(1)) at exactly those spans; both functions live via import-static-checks.ts:1974/2001 (imported at :126-127); doc comments at :458 ("mirroring … exactly") and bug rosters 0429/0448 at :9/:15 confirm the two are one documented rule; not a spec vector table; distinct from resolved PTQ-0373, whose root cause was only the shadowedNames.has guard (now isShadowedImportName) and whose evidence explicitly set the `typeName === null` skip aside — this is the residual object-site prologue that fix left duplicated, and a filter-sequence helper is a mechanical dedupe (triage: claude-fable-5-1)
