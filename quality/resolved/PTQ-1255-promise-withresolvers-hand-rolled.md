...
verdict: questionable — re-verified independently: all three excerpts reproduce (production-theta-producer.ts now 2122-2125, ~2-line drift; cap-race.ts:25-28; session-shutdown.ts:326-329) and each resolver is consumed only as a void resolve handle (settleDispose at :2140/:2146, resolveCap at :29, resolveRace at :335/:357 — no reject path, noop initializer unreachable since the executor runs synchronously); `withResolvers` appears in src/ only in the site-1 doc-comment (:2109); engines `>=22.19.0` at package.json:43-45 and `typeof Promise.withResolvers === "function"` on Node v24.16.0; tsc probe from $TEMP reproduces TS2550 under `--lib ES2022,ES2024.Collection` and is clean with `ES2024.Promise` added; spec active-invocation-registry.md:11 says "`Promise.withResolvers()` construction (or equivalent)", so the swap drops no required behaviour; only D8 exemption on the file is `#firstAdmittingArmProperties` (different host); PTQ-0341/PTQ-1016 cover cap-timer harness duplication, not this idiom; the fourth definite-assignment copy at src/runtime/subagent-isolation.ts:178-181 is uncounted but does not refute the accounting; per the D8 rule the tsconfig-lib add + site collapse is a design decision for a human ruling (triage: claude-fable-5-1)
verdict: questionable — accounting re-verified against current code: all three excerpts reproduce (production-theta-producer.ts:2120-2123 after ~4-line drift, doc-comment naming `Promise.withResolvers()` at :2107; cap-race.ts:25-28; session-shutdown.ts:326-329), each resolver consumed only as a void resolve handle (settleDispose :2138/:2144, resolveCap :29, resolveRace :335/:357) with no reject path and an unreachable noop initializer; the stated regex re-run gives exactly 11 src/ hits with those 3 as the only resolver-escape sites, `withResolvers` appears in src/ only in that comment; facility available on the pinned runtime (engines `>=22.19.0` package.json:43-45, `typeof Promise.withResolvers === "function"` on Node v24.16.0) and the tsconfig gap reproduced from $TEMP (TS2550 under `--lib ES2022,ES2024.Collection`, clean with `ES2024.Promise` added — same cherry-pick pattern PTQ-0320 used); spec check clear: active-invocation-registry.md:11-12 says "`Promise.withResolvers()` construction (or equivalent)", requiring only a closure-held resolver, not the hand-rolled spelling; only D8 exemption on the file is `#firstAdmittingArmProperties` (different host); no issue/resolved PTQ names this idiom (PTQ-0341/PTQ-1016 are cap-timer harness duplication); uncounted fourth `let resolveExit!` copy at src/runtime/subagent-isolation.ts:178-181 noted, does not refute the accounting; per the D8 rule the lib add + site collapse is a design decision for a human ruling, never confirmed (triage: claude-fable-5-1)
