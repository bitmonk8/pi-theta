...
## Triage
verdict: confirmed — re-verified independently: `grep -rnE "applyBinderBypass\("` gives exactly the declaration (binder-envelope.ts:272) plus 4 calls; production narrows at :919 (`decision.kind !== "binder"`) and reads only `bypass.args` (:930), the only `{ kind: "binder" }` construction is inside `classifyBinderBypass` (:224) and no test hands one to `applyBinderBypass`, and `bypassed` is read only at tests/binder-bypass-envelope.test.ts:251/:260 (both `.toBe(true)`) — no `export *` barrels, no string-keyed/dynamic access, spec §Binder bypass mandates no such flag, so the `binder` arm is unreached and the flag's two-valuedness is unobservable (triage: claude-opus-5)
