...
## Triage
verdict: confirmed — re-ran both greps (8 `V18c-T stub` at :130/157/183/207/248/293/309/363; 7 `V9g-T stub` plus :12 header and :162 section head) and read all 27 sites: every excerpt matches verbatim with a fully implemented body adjacent (gates read `SDK_SURFACE_INVENTORY` despite "no gate consults the pinned SDK yet", both constants populated at :135/:177, `runSessionShutdown` orchestrates sub-steps 1–5, composed `run` at :431 does binder+routing+executor, `drain`/`initDrainStateTag`/`classifyShutdownReason`/`armWatcherWithTerminalRecovery` all filled), git shows each `V*-T` commit followed by its paired implementation commit, and no other intake file cites any of these 27 locations or this root cause. (triage: claude-opus-5)
