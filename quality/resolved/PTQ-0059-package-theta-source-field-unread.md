...
## Triage
verdict: confirmed — TS language service over all 963 program files finds exactly two references to `PackageDiscoveredTheta.source`, the declaration (:57) and the single write (:755), zero reads; caveat for the fix: removal also requires widening the `DiscoveredTheta[]`-typed `named` helpers in tests/discovery-glob-universe-enumeration-failure.test.ts and tests/discovery-tree-walk-lstat-failure.test.ts (6 TS2345 sites depend on structural assignability, not on the value) (triage: claude-opus-5)
