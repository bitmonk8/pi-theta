...
verdict: confirmed — RATIFIED (human, 2026-09-17): migrate the ~24 guard-then-substitute ...Fragment helpers in tests/live/live-production-acceptance.test.ts to load-row-harness registryMessageOf. STRICT byte-preservation: every assembled '<CODE>: <message>' string must be identical before/after (mechanical substitution only; keep any helper whose shape genuinely differs). The offline gate cannot run this file - tsc is the lane's only witness; the next live-suite run validates it (run the file once after integration if in doubt).
