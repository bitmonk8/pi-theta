...
verdict: questionable — accounting verified independently: size-scan map re-run gives evalSubagentFnCall 688-813 / 126 LOC / band justify (FN_BANDS justify=100, 0/0 exports/importers) and excerpts match byte-for-byte at 718-741; quality/exemptions.json has no statement-executor key; the four inventory rows are real and locals-disjoint (grep of 713-741 finds 0 `scope`, 743-813 finds 0 `argValues`, the 790-813 tail references none of scope/argValues/entered/bodyDeps/expr/env/deps/moduleEnv — only flow/fn), so no seam threads ≥ 6 locals; closed-enum is only the 24-LOC Flow switch; invocation.md INV-4/INV-5/§Failures pin shared behavioural obligations (same FN-6 downgrade, same callee-Err wrap in both regimes), not a single-body shape; `git log -L688,813` is monotone growth 645bcb02→96f9a136→770cbb82→89faa7c5 with no reverted split; no quality/issues entry cites this function and same-wave d9-01 keys on the file host, not a duplicate; D9 breakdown caps at questionable — Seam A/B vs keep-whole needs a human ruling (triage: claude-fable-5-1)
