...
## Fix attempts
- qw20260921130057: skipped — [PTQ-1198-type-grammar-pipeline-stages-bundled.md] PTQ-1198: Moved walker, supporting constants, GENERIC_ARITY and comments into type-walk.ts, preserving barrel exports and existing importers. size-scan map: type-grammar.ts 1766→1243 LOC. Concrete diff remains uncommitted, addressing the rejected attempt. Required tsc/full gate passed: 698 files, 11616 tests; no tests removed. / PTQ-1199: Still reproduces; skipped under the last-attempt shed rule rather than introduce an unselected shared-state design. Entire TypeParser remains byte-identical; size-scan map: parseObject 293→293 LOC. No partial edits remain. / PTQ-1200: Extracted checkInlineFieldKeys in type-walk.ts, preserving gates, precedence, diagnostics and recursion order. size-scan map: walkType 273→165 LOC. Mechanical reinlining reproduces the original walker byte-for-byte. Required gate passed; no tests or assertions changed. | review unconfirmed: PTQ-1199-parseobject-293-loc-entry-state-machine.md — shed by the fixer: TypeParser.parseObject is untouched in the working tree (size-scan still reports src/parser/type-grammar.ts#TypeParser.parseObject at 755-1047, 293 LOC, band strong, only shifted by the walker extraction above it); all seven per-entry latches (entryStart, entryRefused, pending, pendingSlotOpen, emptySlotBodyPushed, entryTainted, namesStopped) and the five loop arms plus gated epilogue flush remain inline — neither Seam A (EntryRefusals helper) nor Seam B (non-ident field-name arm as a private method) was attempted. ||
