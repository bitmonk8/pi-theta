...
verdict: questionable — accounting re-verified independently: all five excerpts reproduce at the exact cited lines (discovery-walk.ts:392/398/409/414; package-discovery.ts:373/393/404/410/416); both copies parse the same four prefixes (""/!/+/-) and run the same four phases in the same order; both live (discoverThetas→resolveSettingsSource :471, resolvePackage→resolvePiThetas :598); clone-scan map reports no group for either file so `parallel` is the right class; spec binds both to one contract (package-and-settings.md:21 #disc-5, :99 for thetaPaths) and tests/settings-glob-disc5-matcher.test.ts records a prior drift between the two sites, so the anchor is mechanical; no issue/resolved row tracks the override orchestration (PTQ-0281/0305/0333/0367 are D9 breakdowns, 0287 listTree, 0286 path helpers); one overstatement that does not refute the counts — the settings copy has no empty-plain fallback (returns [] on empty entries, step 1 starts from an empty Map), so the "why"'s fallback hypothetical is package-only; the shared driver shape is a design decision for a human ruling (triage: claude-fable-5-1)
verdict: confirmed — RATIFIED (human, 2026-09-17) with direction OVERRIDE - do NOT build the parameterised four-phase driver (it would exceed the two folds it replaces): add ONE shared DISC-5 spec-vector test that drives BOTH resolveSettingsSource and resolvePiThetas over the same fixture table (all four prefixes, precedence order, empty-plain fallback) so grammar drift between the two implementations is loud. No production code change.
