...
## Triage
verdict: confirmed — re-verified: the docstring (actual :74-78, ~5-line drift, content verbatim) asserts the registry Message column is `subagent dispose failed: <dispose error first line>`, falsified by its own body :84, diagnostics.md:293, code-registry-runtime.md:26 and tests:219 which all pin the teardown spelling; bug 0189 Residual 1 misses this site (token is line-wrapped, escaping its grep) so it is untracked. (triage: claude-opus-5)
