...
verdict: questionable — accounting verified on re-triage: cluster at reload-wiring.ts:494-562 (AvailableModel 501-508, ModelRegistrySurface 511-513, createModelReferenceMatcher 523-551, outcomeOf 554-562; 3-line drift) touches 0 own-host members (grep of the range for ThetaRegistry/ParsedTheta/rebuildAndSwap/dropCollidingThetas/structuralChangeNote/ReloadFailureInjector/SlashCommandInfo/SystemNote/Diagnostic: none) and only frontmatter.ts:71,86's ModelReferenceMatcher/ModelMatchOutcome contract — the filed "4 foreign" is inflated by two types reached via LoadPassDeps 570-576 outside the range, but 2-vs-0 still holds; binder-model.ts:48-51 value-imports from extension/ and production-composition.ts:200-204/876 is the other consumer; size-scan map reproduces 0/2, 1/1, 2/4, 0/1 with the host 595 LOC band exempt (placement review permitted, no exemptions.json row); no duplicate (PTQ-1103 is the header's collision-pass claim; d9-01 only lists the matcher as a runComposePass phase); the header bullet 16-18 claims the wiring point, binder-model.ts:54 already type-imports from extension/, and matchAvailableModel at binder-model.ts:133 is a second matcher in binder/, so the home (frontmatter vs new parser module vs binder-model vs stay) is a design ruling for the human (triage: claude-fable-5-1)
