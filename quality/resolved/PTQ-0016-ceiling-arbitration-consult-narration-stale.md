...
## Triage
verdict: confirmed — reverified: both excerpts exact (ceiling-arbitration.ts:12-17, query-tool-loop.ts:38-41 with computeMasked imported at :61); my own hunt (imports, `arbitrate(`, seams barrel, `export *`, dynamic `import()`) finds zero production consumers — only tests/ceiling-arbitration.test.ts and tests/integration-acceptance.test.ts; `git log --all -S` confirms load-pre-eval.ts was the sole src importer ever, deleted by 94e81974 whose message calls the route unreachable, and bug-0066 doc:112 matches verbatim; computeMasked hardcodes ["ceiling#2"] without calling arbitrate, the spec never mandates the consult, no deadness is claimed (witness-test rule respected), and D2 lists historical narration comments in scope; distinct root cause from d2-01-check-sites-dead-export and the depth-walk roster candidate (triage: claude-opus-5)
