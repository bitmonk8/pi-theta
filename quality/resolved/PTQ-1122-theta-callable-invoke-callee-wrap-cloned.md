...
## Triage
verdict: confirmed — both excerpts reproduce in the tree (executable clone at effectful-statement-host.ts:391-406 / :586-601, the cited 377-406 span merely includes the leading comment block); `node tools/quality/clone-scan.mjs map` on a one-file manifest independently reproduces G012 (102 tokens, renamed-only(4)) at exactly the cited ranges, and a mktemp diff of the two spans after normalising the four renamed tokens (outcome var, message prefix, style literal, token key) is empty; both copies are live production paths (runToolCallEffect/runInvokeEffect dispatched at :673/:675, `classifyCall` returns "theta-callable" from production-theta-producer.ts:4316); the code's own comment names the two arms as one XMODE-1 rule, so this is stated-breakage clone not incidental similarity, not a spec-normative vector table, no exemption for the host; not a duplicate — PTQ-0940 is a D7 tests/ fixture-builder finding and PTQ-0289's "G012" is a different scan's group over pass-parse-cache/pass-verdict-memo; the fix is a mechanical private-helper dedupe (triage: claude-fable-5-1)
