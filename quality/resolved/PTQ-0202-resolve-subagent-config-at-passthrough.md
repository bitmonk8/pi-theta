...
verdict: confirmed — human ruling (thomasa, 2026-09-11): accept - the wrapper is a pure same-signature forward to the module-private resolveSubagentSessionConfig and adds no behaviour (D2's redundant pass-through smell); its doc block (re-resolve against a DIFFERENT enclosing frontmatter, FN-9; identical to parse-time for an in-file fn) is real information that belongs on the one function. Direction: export a single function (fix stage picks the name - keeping the FN-9 spelling is fine), merge the two doc blocks, have both the parse-time caller (:1485) and import-static-checks.ts:431 call it, retarget the FN-9 test binding (subagent-fn.test.ts:1289). Zero behaviour change, module-scoped
