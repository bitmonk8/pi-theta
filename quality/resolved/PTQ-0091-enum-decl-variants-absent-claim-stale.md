...
## Triage
verdict: confirmed — all three excerpts byte-match at 839-845/854-862/4138-4151 and I ran the shipped parser on every non-brace shape (`enum Color`, `enum Color\n`, `enum Color = 3`, plus `enum Color { }`): each records `keys:[kind,name,variants,variantDecls,range]` with `variants: []` / `variantDecls: []` — never absent — and `theta/parse/empty-enum-body` fires precisely BECAUSE the empty `variantDecls` is present, so the docs' named absence is unreachable and the code-side direction would silence a live diagnostic; parseEnum:4144 is the only `kind: "enum"` constructor in src/ (other hits are system-param/interpolation classifier shapes) and the only reconstructions (`{...decl, name: asName}` at import-static-checks.ts:338, query-schema-resolve's `...stmt`) preserve both keys, so the three consumer guards reproduce as inert (hoistEnumVariants:8224, walkStatement:9413, lowerEnumToSchema's `variants ?? []` at body-type-lowering.ts:132) while the sibling `SchemaDecl.fields` IS genuinely absent (parseSchema:3697/3705 returns `{kind,name,range}` for the headless head) — the enum docs narrate schema's pattern, which the enum producer never follows, and that false premise has already propagated into import-static-checks.ts:1432-1437's "stays absent from the map" withhold comment; history root 2bc69157 carries both the sentence (:447) and the unconditional writes (:1759-1761) with no docs/ text mandating absence, so this is src/ narration cruft (D2 line 39) not a spec-backed behaviour gap; no other intake file cites 839-862 (triage: claude-opus-5)
