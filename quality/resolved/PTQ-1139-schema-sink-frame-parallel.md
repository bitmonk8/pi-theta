...
RATIFIED (human, 2026-09-21): reopened — the qw20260921173728 cluster resolve swept this finding despite the reviewer's 'untouched — shed by the fixer' note; the parallel still exists on the tree. TARGET SHAPE (named per the D4 parallel-class rule): a COMPILE-TIME EXHAUSTIVENESS TETHER between the SchemaSinkFrame union and its producer/consumer sides (the PTQ-0292/PTQ-0415 ratified shape) — e.g. a kind-keyed `satisfies Record<SchemaSinkFrame["kind"], ...>` table or never-check so that adding a union member is a tsc error in BOTH query-schema-resolve.ts (construction) and query-schema-inference.ts (the resolveQuerySchemaSink switch). Do NOT move construction or consumption code between the files; the tether is the fix.
