...
verdict: questionable — accounting verified on independent re-run: size-scan map reports 1851 LOC / justify band (filing's 1853 is 2-line drift from this wave's own f50bb3c2, which removed 2 lines of params.ts) with all 30 declaration rows at the cited ranges; the three rows are distinct concerns by code-level reference count — parse→lowering only via the LowerCtx literal (:218) and one lowerParamsFieldType call (:242), projection→lowering via classifyInlineObjectEntry (:1762) and lowerLiteralSublanguage (:1824) (two calls where the filing says one; row stands), lowering→parse/projection zero code references (comments only); no module-level shared state crosses rows; interface+const LOC is 130/1851 (~7%), no closed-enumeration mirror beyond lowerTypeExpr's 119 LOC, header is hand-authored, no src/parser/params.ts key in quality/exemptions.json, only revert-matching commit a43855de is bug 0099's slug fix (no reverted split), bug 0097 pins import direction only; PTQ-1149 is in quality/resolved as status fixed with Seam A landed in ac4e7697 (params.ts −698 / type-text-split.ts +444) — the filing's 71af3b5b/09722760 attribution and 'file tail' re-export location (actually :70-76) are clerical errors that do not touch the inventory; no open issue carries d9_host src/parser/params.ts and no other intake names the host, PTQ-1165/PTQ-1176 are function-level and fixed, so not a duplicate (PTQ-0351 seams-remain precedent) — whether Seam B and Seam C cut together or separately (projection documents itself as mirroring lowerParamsFieldType's dispatch order) is a design decision for a human ruling (triage: claude-fable-5-1)
