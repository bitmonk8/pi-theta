...
verdict: confirmed — RATIFIED (human, 2026-09-14): already fixed on main — administrative catch-up only. Wave qw20260914091051's tests lane landed tests/helpers/scripted-live-session-harness.ts and re-pointed b0288/b0319/b0414 to it (commit feefe7ca) but its reviewer did not list this issue in fixed_confirmed, so the issue stayed open while its fix shipped; wave qw20260914130212's reviewer re-verified the problem is gone (per-file ScriptedLiveSession classes remain only as each file's divergent behaviour, the issue's own boundary). Resolved by the store directly after this accept; no fixer lane.
