...
## Triage
verdict: confirmed — independently reproduced: all three excerpts byte-match at params.ts:1870/:277/:452, and a whole-repo grep (every file type) yields only the declaration, those two same-file production calls, two same-file comments, and test/doc prose — no import statement, no `export *` anywhere in src/extensions/tools, no barrel (body-type-lowering.ts:35 forwards only `isSingleEnclosingBraceGroup`), no namespace/dynamic import of params, no string-keyed access — while `git log --all -S` shows only c4dd92c2 and 162ec5c0, both wiring same-file callers; the `export` keyword is load-bearing for nothing (citation-symbol-form-gate's `anchorIndex` regex makes `export` optional at :271 and no test anchors the line, tsconfig sets no `noUnusedLocals`, docs mandate no shared predicate — its own doc comment calls it "the `params:` position's OWN detection", unlike `isUnspellableTextRefusable`'s four cross-file readers), and the function itself stays alive on two production callers, so the unreached export surface is the only cruft claimed (distinct root cause from d2-07-classify-lowered-union-arm-stale-export; quality/issues and quality/resolved are empty, TRIAGE_LOG has no rows) (triage: claude-opus-5)
