...
verdict: questionable — accounting re-verified independently: excerpts match at theta-document.ts:213-232 and :280-292 (early call's sole consumer is earlyFm.paramFields[].name; authoritative call at :281 supplies bodyTypes); parseFrontmatter (frontmatter.ts:1006-1183) has no early return between parseDocument (:1015-1018), checkRecognisedFields with modelMatcher (:1065-1067), extractParsedParams against the empty bodyTypeDecls (:1083-1094) and buildSystemTemplate (:1138), so the early pass does run the whole pipeline and discards every diagnostic; extractParsedParams pushes every scalar-keyed params: item to fieldInputs including shapeRefused ones (frontmatter-params.ts:105-106 is the only continue, :219-225 push), so a keys-only read yields the identical name set; grep confirms theta-document.ts:224/:281 are parseFrontmatter's only two src callers and parseThetaDocument is the per-file entry, so the 0-10-keys-vs-full-second-parse data-size claim holds; quality/exemptions.json has no theta-document/parseThetaDocument row; the comment's rationale is ordering-only and is preserved by the direction; not a duplicate — PTQ-1166 (D9, resolved) merely inventories the pass as P2, PTQ-1231 (D8, resolved) fixed the fence rewrap at these sites and explicitly deferred the full-parse cost, PTQ-0146 concerns BodyParser's default only; the simpler shape (keys-only reader vs stop-after-collection option) is a design decision for a human ruling (triage: claude-fable-5-1)
verdict: confirmed — RATIFIED (human, 2026-09-21): confirmed - replace the early full parseFrontmatter run with a keys-only phase-1 read; triage verified the recovered params: name set is identical (refused fields retained, only non-scalar keys skipped) and every early-pass diagnostic is currently discarded, so no observable change.
