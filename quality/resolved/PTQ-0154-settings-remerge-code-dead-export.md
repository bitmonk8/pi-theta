...
## Triage
verdict: confirmed — reproduced independently: repo-wide grep of tracked files yields exactly the decl (src/extension/reload-wiring.ts:313) and one module-internal read (:418), no importer/re-export/namespace-or-string-keyed access anywhere, and the sole historical importer tests/pre-evaluation-reload-failure.test.ts was deleted in c63941b3's dead-code audit (only remaining hits are stale gitignored dist/), so the export modifier is a proven leftover; sibling REGISTRY_SWAP_FAILED_CODE verified imported at the three cited test lines, tests-only-caller rule inapplicable (zero test references), and no overlap with the two other reload-wiring intake files. (triage: claude-opus-5)
