...
## Triage
verdict: confirmed — re-verified independently: all four excerpts match at the cited lines; grep across src/, extensions/, tools/, tests/ (no `export *`/barrel/dynamic access) shows `forwardToolExposedCancel` and the `routeToolCallLateSettlement`/`ToolCallSettlement`/`ToolCallCancellationGuard`/`ToolCallSideChannels`/`ToolCallLateDisposition` family referenced only from tests/cancellation-core.test.ts and tests/no-rollback.test.ts, while production imports only createThetaAbort/deriveChildThetaAbort/forwardSlashCommandCancel/abortForAgentEnd/makeCancelledError/runCancellableSequence/OperationResult/CancellableStatement/abortInvocationsOnResultChannelDeath from this module and routes the CNCL-1/2/3 discard through tool-call-off-surface.ts (`discardPostCancelSettlement`) / tool-call-execute.ts (`awaitToolSettlementOrAbort`), so the header's unqualified "owns"/"fills in" narration for those two items is a mechanically false structural claim of the same class the store already confirmed for this file's swallowing-handler section (PTQ-0035, whose fix is the :258-266 disclosure) and for conversation-drive's header (PTQ-0100); not a deadness filing, not a duplicate (PTQ-0035 covered different symbols and is fixed) (triage: claude-fable-5-1)
