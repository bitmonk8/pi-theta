...
## Triage
verdict: confirmed — all 5 excerpts verbatim at the cited lines; my own repo-wide hunt (no re-export, no dynamic/bracket access) finds 5 call expressions / 4 consumer positions, and at the comment's authoring commit db98c7b66 only the `?` belt and rule 5 consumed the descriptor — stdlib-array.ts:155 (7a513015b) and runtime-panics.ts:509 (31bb73cfc) landed after, so "two call sites" no longer describes the code (triage: claude-opus-5)
