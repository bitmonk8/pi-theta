...
verdict: questionable — third independent re-verification at HEAD 6883c525: all 5 excerpts reproduce content-exact but with drift — params.ts copies now at 1231-1243/1817-1834/2098-2125 (−44..−49 lines), frontmatter.ts 1450-1471 stable, and the fifth copy has MOVED out of type-layer-checks.ts to src/parser/annotation-validation.ts:393-423 (locations entry stale; body byte-exact, mirror comment at :388-391 intact); all 5 live from src (params.ts:1323, :270/:446, topLevelColon ×5 incl. new caller type-grammar.ts:1336, frontmatter.ts:1543, annotation-validation.ts:529); clone-scan reproduces the clusters renumbered again (params manifest G004/G006/G008/G010/G011, frontmatter manifest G031, all renamed-only); no PTQ tracks it and sibling d4-15 is a distinct root cause; but the roster still undercounts — grep `if (c === "\\" && i + 1 <` in src/ → 9 hits, all src/parser (omits classifyGenericArgumentSegments params:1105, isSingleEnclosingBraceGroup :1541 — the mirror anchor the "why" itself quotes — isBraceBalanced :1606, splitTopLevelSegments :2193) — and the "why" overclaims bracket-nesting agreement that bug 0238 §Fix deliberately splits (typed opener stack in topLevelColon/braceGroupCarriesUnmatchedCloseToken vs bare depth in splitParamValue/isBraceBalanced vs angle-only floor in findCutBracketGroupText, "reproduces byte for byte" comment at params:1246-1248), so only the ~10-line quote/escape prefix is shareable and 2 of 9 sites also accumulate escaped bytes into `current`; a skip-quoted-region helper is a design to rule on, not a mechanical lift — human should rule on the 9-site quote-core-only scope and refresh locations to annotation-validation.ts (triage: claude-fable-5-1)
