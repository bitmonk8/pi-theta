...
## Triage
verdict: confirmed — all nine count sites reproduce verbatim (:90, :95, :191-199, :1360, :1407, :1433, :1471, :1477, :1547) and a runtime probe of `parseTypeExpression(..., "inline-object-shape")` emits seven distinct object-arm codes, not five (empty-schema-body, reserved-keyword-as-identifier, binding-case-mismatch, duplicate/quoted/renamed/not-identifier), with the arm's only `rules` gates at :1494/:1511 outside it, so "eight checks" (rules-selectable set is now ten), "five checks"/"fifth check" and "six rules" are each falsified by bug 0228's (2026-08-22) and bug 0249's (2026-08-23) later-landed emissions; a tenth stale site at :1536 ("the same six object-arm rules") escaped the filing's search pattern and the "eleven distinct codes" comparator overcounts by including parser-side `malformed-schema-field` :1176, neither of which weakens the root cause; no duplicate — the other type-grammar filings (202646-d2-03 open-report deferral, 202646-d2-02 wire-bound restatement, 183353-d2-05 registry citations) overlap lines but not this claim (triage: claude-opus-5)
