...
verdict: questionable — accounting verified: size-scan map re-run on a one-line manifest gives 4346 LOC / band strong (FILE_BANDS strong=2000) with BodyParser 354-4338 (3985 LOC) and every inventoried member at its cited range (parseSchema 1450-1487, parseImportExport 2112-2200, parseType 2424-2548, parseMatch 3255-3325, parseParFor 3717-3791, scanParForStmt/Expr 3835-4041, parseInvoke 4043-4123, parseQuery 4222-4332); the scanParForStmt excerpt is verbatim at 3835-3841 and a grep of `this.` over 3803-4041 finds only this.diagnostics/this.file/self-recursion (no pos/peek/advance), so the CTRL-4 row is a genuinely cursor-independent concern and the other rows are disjoint grammar-chapter member groups, not one concern split by adjectives; exemptions.json D9 keys are only binder-system-prompt#normaliseParamLineBreaks and package-discovery (no growth-bar exemption to weigh); git log --follow shows the file's first commit is 702a1f2e (PTQ-1156 fix), so no reverted-split reason exists; no closed-enum/≥80%-data/generated/one-production reason covers the whole file; dedupe: PTQ-1156 is in resolved/ keyed on theta-document.ts, PTQ-1172/1182 were function-level, and the only other intake mention (qw20260922150013-d9-01) is keyed on theta-document.ts — the class-split shape (seam A/B/C) is a design decision for a human ruling (triage: claude-fable-5-1)
