...
verdict: questionable — clone re-verified again (excerpts match verbatim at current 1054-1085/1167-1191; clone-scan.mjs map with a one-line manifest → G008, 110 tokens, renamed-only(6); both surfaces live at :1727/:1740 ← production-composition.ts:1412; not a duplicate: PTQ-0364 fixed + disjoint, PTQ-0375 is G055, intake qw20260917121953-d9-03 is a D9 breakdown of the inline invoke(...) loop), but the store has parked it after 2 fixer skips and the skip rationale holds mechanically: host is 1777 LOC ≥ the 1000 home-band bar, and a sibling helper must import collectProvableArgTypes (exported, value-imported only by invoke-imported-checks.ts; parser/test mentions are comment-only) and module-private renderCollectedTypes (:379/:765/:1439) from the host while the host imports the helper back — an import cycle whose only escape is relocating that ~196-LOC block (:640-835) with the helper, a relocation decision a human must rule on via accept --note (or reject) (triage: claude-fable-5-1)
verdict: confirmed — RATIFIED (human, 2026-09-17): dedupe G008 (the per-argument type-mismatch loop in checkThetaCallableCallSurface/checkRuntimeToolCallSurface) into the surface module created by the qw20260917121953-d9-03 ruling, which relocates collectProvableArgTypes + renderCollectedTypes there - the cycle-free home the two prior skips lacked. Sequencing: the store defers this behind that D9 lane on the same host; attempt only after the relocation lands.
