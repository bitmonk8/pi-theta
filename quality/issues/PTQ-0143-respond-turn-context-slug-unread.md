...
## Triage
verdict: confirmed — reproduced with a TypeScript compiler-API reference scan over the whole program (src+tests+extensions): the only references resolving to `RespondTurnContext.slug` are the declaration (5002) and the shorthand write (3466), zero reads, while the same scan finds 8 reads for `toolName`, 5 for `template` and 2 for `captureHost`; interface is non-exported (confirmed at 5001), no `.slug` access on any respond value, no string-keyed/dynamic `"slug"` in src, `respond` only ever flows into `RespondTurnContext`-typed params (5142/6242/5591/7029/7064) so no structural assignability needs it, and blame confirms 304929486 (2026-07-27) minted declaration and write together — a never-read member; the FP-check's "10 hits" undercount (12; omits 3419/7029/7064) does not touch the conclusion (triage: claude-opus-5)
