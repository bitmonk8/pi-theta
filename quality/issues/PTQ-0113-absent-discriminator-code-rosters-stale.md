...
## Triage
verdict: confirmed — reproduced: header roster (:333-349, eight bullets) and `checkDiscriminatedUnion`'s "every diagnostic" doc roster (:394-397) both omit `theta/parse/absent-discriminator-field`, which :656 returns live via the `decl.by !== undefined` arm (vitest cell "an ABSENT `by` field now draws its own code" passes; literal exists only at :711 in src/), and blame shows f5862ab0 updated both rosters when adding `non-literal-discriminator` while 0544c61a added this code and skipped them (triage: claude-opus-5)
