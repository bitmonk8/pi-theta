...
verdict: questionable — accounting verified; target shape needs a human ruling: re-verified from scratch — both classes sit at runtime-panics.ts:720-728 / 752-759 as cited; own-host affinity is 0 for the arity class (plain `Error`, no module-local member) and 1 for the kind class (exported `summariseNonResultOperand`, 755); repo-wide grep of `StdlibMethodArgument(Kind)?DefectError` yields exactly one importing module (stdlib-signature.ts:3) with four throws (60/63/66/94), zero importers under tests/ extensions/ tools/ (test hits are comments only), no re-export and no string-keyed access; the sibling pattern is real and self-justified at stdlib-array.ts:132-153 ("defined here (not in runtime-panics.ts) because the fix is confined to this file's owned surface"); `surfaceUnexpectedThrow` branches only on `isThetaPanic` (834), `HostFatal` (840) and `NonObjectReceiverError` (847), so routing is location-independent; not a duplicate (no quality/issues row names these classes — PTQ-1216 moved the signature substrate, PTQ-1119 deduped the dispatchers, the same-wave d9-01 breakdown that lists them as its Seam B is untracked intake); counter-signals for the ruling stand — docs/bugs/0315 l.205 and 0394 l.193 ratify `src/runtime/runtime-panics.ts` as the home and stdlib-signature.ts:12-13 / parser/stdlib-arg-diagnostics.ts:23 cite it, so a move also edits two doc comments and two bug-doc §Fix lists (triage: claude-fable-5-1)
