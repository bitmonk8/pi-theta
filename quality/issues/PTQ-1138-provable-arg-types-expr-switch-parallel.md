...
verdict: questionable — accounting verified independently: `Expr` (theta-document.ts:474-494, drifted from the earlier notes' :463-483) has exactly 20 members; `collectProvableArgTypes` (invoke-expr-call-surface.ts:58-216) and `#typeExpr` (static-type-inference.ts:271-416) each enumerate all 20 with no `default`; the binary arm reproduces `#typeBinary`'s five operator groups (:588-644) in order; all copies live (callers invoke-static-checks.ts:376/665/1108, invoke-imported-checks.ts:262, same-file :294); clone-scan map lists no group (expected for a parallel); no PTQ or intake file tracks this root cause (d9-03 is a D9 breakdown of invoke-static-checks.ts). Own tsc fixture under `strict` without `noImplicitReturns` (the repo tsconfig) reproduces the asymmetry — a missing arm in the `CompatType[] | undefined` switch compiles to a silent withhold while the `CompatType`-returning switch hits TS2366 — so the header's "compile error" guarantee holds on one side only and the co-edit risk is real; but `BOOLEAN_BINARY_OPS`/`isStaticZeroIntegerDivisor`/`pass.typeOf` are already shared and docs/bugs/0142 (:825-833, :972) records the MIRROR as the chosen disposition, so a shared source of truth is a design decision for a human ruling (triage: claude-fable-5-1)
verdict: confirmed — RATIFIED (human, 2026-09-21): accepted as confirmed - batch ruling, all D4/D9 intake items accepted.

## Fix attempts
- qw20260921152514: skipped — [PTQ-1136-teardown-call-labels-parallel-call-sites.md] PTQ-1136: Replaced all eight free-literal emitter labels with TEARDOWN_STEP_CALL_LABELS references and derived ForwardingSignalSource.label from its step-5 row. Actual working-tree changes retained; no tests deleted. Required gate passed: tsc and 698 test files / 11,617 tests. / PTQ-1138: Reconfirmed, but shed under the last-attempt instruction: I could not safely consolidate the full contract while preserving match-arm scopes, tool-success unwrapping, and deliberate collector withholding. Both files remain unchanged; no partial fix or test deletions. | review unconfirmed: PTQ-1138-provable-arg-types-expr-switch-parallel.md — shed by the fixer: no working-tree change touches either cited file; `collectProvableArgTypes` in src/extension/invoke-expr-call-surface.ts (the 20-arm Expr switch plus the binary sub-switch) and `#typeExpr`/`#typeBinary` in src/parser/static-type-inference.ts remain the two unchanged parallel passes with no shared source of truth. ||
