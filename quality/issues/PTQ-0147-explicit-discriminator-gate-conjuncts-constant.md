...
## Triage
verdict: confirmed — all five excerpts reproduce verbatim at the cited lines and my own exhaustive replay of `evaluateOccurrences` + the gate chain over 4369 occurrence shapes finds `presentInAll`@:681, `allLiteral`@:687/:693 and `allString`@:694 true at every one of the 1464/1111/15 reaching executions with zero outcome change when stripped (`kind: EnumValueKind` at :160 is non-optional, so `allString === false` always mints `firstNonStringKind` and returns at :687); git 0544c61a (bug 0046) added the `!presentInAll` return at :655 that inerted conjuncts which were live before it, so this is a proven vestige, not taste — only defect is the narration's `const evaluation` line, which is :627 not :654. (triage: claude-opus-5)
