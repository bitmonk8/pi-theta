...
## Triage
verdict: confirmed — independently reproduced: `tempFilePath` has exactly 5 repo-wide hits, all inside src/runtime/subagent-params.ts (decl :139, local :192, env value :196, return prop :198, cleanup body :200), zero in tests/extensions/tools/other src; the sole production caller reads only `.cleanup` (:2540) and `...marshalled.env` (:2564) and never spreads or structurally compares the record, no string-keyed/dynamic access exists, `MarshalledParams` is imported nowhere (package.json has no `exports`), and git blame shows the field landed with RFC-0006 (4866d4d2) and never gained a reader — dead schema field proven dead. (triage: claude-opus-5)
