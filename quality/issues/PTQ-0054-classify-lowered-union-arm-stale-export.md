...
## Triage
verdict: confirmed — independently reproduced: whole-repo grep finds only the definition (params.ts:999) and one same-file call (params.ts:1754), no import/re-export/`export *`/dynamic access anywhere and only two test COMMENT mentions; `git show ad27694f` deletes `classifyLoweredUnionArm,` from body-type-lowering.ts's import list while `git blame` shows the justifying paragraph unchanged since 52e257bc, and the export is load-bearing for nothing (citation gate's `anchorIndex` makes `export` optional; `LoweredUnionArm` is exported independently at schema-lowering.ts:156). (triage: claude-opus-5)
