...
## Triage
verdict: confirmed — independently verified: 3/3 construction sites pass an explicit body source, class is unexported with no barrel/subclass/dynamic construction, so the `= ""` default is provably unreachable; parseQuery's fallback is live via the no-closing-backtick path, which softens the rationale but not the root cause (triage: claude-opus-5)
