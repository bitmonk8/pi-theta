...
## Triage
verdict: confirmed — both excerpts match at the cited lines; clone-scan reproduces G009 renamed-only (106 tokens) at exactly those ranges; both copies live (theta-document.ts:5861/7513/8542; match-result via static-type-inference.ts:455 and type-layer-checks.ts:834/2138); load-bearing shared PatternNode binder walk, not a spec vector table; no existing PTQ or intake row covers the pair (note for fixer: theta-document.ts value-imports type-layer-checks.ts which imports match-result.ts, so the suggested export-from-theta-document direction would cycle at runtime — the reverse direction does not) (triage: claude-fable-5-1)
