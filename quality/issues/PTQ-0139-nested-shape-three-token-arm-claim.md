...
## Triage
verdict: confirmed — all three excerpts match verbatim at the cited lines; inside `emitNestedShapeDiagnostic` (:388-431) `entry` is only destructured (:392) and handed to `emitConstructionSiteFallback` (:407), so the serialiser-throw arm's sole emit is the two-token literal (:425) and no branch can produce the three-token form the doc (:377-380) promises, while the inline comment (:411-415) and PIC-25 (two-token for both nested-shape codes on serialiser throw; three-token only under *Construction-site wrap*) both agree with the code — the doc sentence is the sole wrong artefact, not a behaviour bug; blames 643fe3b47/2f4ae337e reproduce, `emitNestedShapeDiagnostic`'s only production caller (:480) adds no emit, and no other intake file cites this sentence (qw20260907183353-d2-07 covers only the :385-386 `V9g-T stub` line and its fix explicitly preserves the behavioural prose). (triage: claude-opus-5)
