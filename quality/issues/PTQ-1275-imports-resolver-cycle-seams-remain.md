...
verdict: questionable — accounting verified a third time from scratch: size-scan map reproduces 754 LOC / zone with no quality/exemptions.json key; header (1-13), Resolver (168-171), opaque-id DFS doc (683-690) and ThetaLibImportGraph (691-693) byte-match HEAD; `posix` occurs only at 226-228 inside RelativeThetaLibResolver.resolve; declaration-table sums give resolver row 114 (142-294) and cycle row 56 (671-752), each with its own error class / graph type and sharing only the ImportSite type (params at 268, 707) — no locals, fields or module state — with the parse-phase check family, so ≥ 2 distinct concerns holds; all seven quoted importer counts match the map; no closed-enumeration dispatch across rows, data/type LOC well under 80 %, hand-written; `git log` shows accretion then de539319 moving only the V15i section to thetalib-exports.ts (Seam A) with no reverted split; PTQ-1184 (quality/resolved) is fixed via a batch ratification carrying no note declining B/C and no other open filing cites this host, so this is a PTQ-0351-precedent residual re-file, not a duplicate; row 3's "~380 LOC" is actually 278 declaration LOC and Seam B does reference the host via `site: ImportSite`, both immaterial — which seam(s) to cut is a human design ruling (triage: claude-fable-5-1)
