...
verdict: questionable — accounting re-verified: size-scan map reproduces 756 LOC / band zone with no quality/exemptions.json row; declaration ranges match the map (types 122-455 contain no function/const/class lines, composition 96-113 + 476-674, framing 71-81 + 685-756); surfaceDispatchDefect is called at exactly 2 parameterised sites (529, 662) and shares no locals with `run`, leaning on 6 runtime/ panic symbols vs 3 host members under a different spec anchor (error-model.md §Runtime panics / code-registry-runtime.md); type family is 38 % of LOC (under the 80 % data bar) with 6 src importers of ThetaCompositionInput independent of composeThetaFixture (2 src files); no overlooked reason (no enumeration, grammar, generated code, prior split, exemption); only prior filings on this host are D2 citation drift (PTQ-1102, PTQ-1110), not the same root cause — the seam shape (one contract module or two) is a design decision for a human ruling (triage: claude-fable-5-1)
verdict: questionable — accounting verified independently: size-scan map reproduces 756 LOC / band zone with no quality/exemptions.json row; the three inventory rows are real distinct groups at the cited ranges (framing 71-81 + 685-756, composition 96-113 + 476-674, types 122-455 with zero function/const/class lines inside, 290 LOC = 38 % under the 80 % data bar); surfaceDispatchDefect is called at exactly 2 parameterised sites (529, 662), shares no locals with `run`, and leans on 6 runtime/ panic symbols vs 3 host members under a distinct spec anchor (error-model.md §Runtime panics / code-registry-runtime.md); ThetaCompositionInput has 6 src importers vs composeThetaFixture's 2, so the contract is consumed independently; no overlooked reason (no enumeration, grammar, generated code, exemption, or reverted split in `git log --follow`); all 19 prior filings naming this host are D2 citation drift or D7 test duplication, none D9 — the target shape (one seam or two) is a design decision for a human ruling (triage: claude-fable-5-1)
