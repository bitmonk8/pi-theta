...
verdict: confirmed — all 3 sites reproduce verbatim (722/1387/4600, minor drift) and git proves insertion-detachment: the docs sat directly on BinderForcedToolDispatch, #emitBinderFailureNote and callableSetPiToolNames before 94e81974 / 57a62e6b / 33ecc35b inserted the interposing declarations, and TS quickinfo now returns EMPTY documentation for BinderForcedToolDispatch and callableSetPiToolNames; the "doc tooling misattributes the first block" leg is refuted (quickinfo binds only the nearest block, and the repo has no jsdoc/typedoc) and site 2 is an orphan carrying unique BND-3 text rather than a strict duplicate, but the root cause stands (triage: claude-opus-5)
