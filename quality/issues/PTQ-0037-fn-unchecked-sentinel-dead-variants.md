...
## Triage
verdict: confirmed — re-verified: `"unchecked"` occurs only at src/parser/functions.ts:33,130,132,134,143,241,249,265 (declarations + stub-era docs) with zero construction or comparison sites in src/, extensions/, tools/, tests/; the sole producers return only "resolved"/"unresolved" (:151) and "checked"/"inferred"/"inference-no-common-type" (:285,:300,:317,:331), the only consumers narrow at type-layer-checks.ts:2105,:2157, and `git log -S '"unchecked"'` confirms f1f7b935 (V3d) deleted `return "unchecked"`/`return { kind: "unchecked" }` while leaving both union members declared (triage: claude-opus-5)
