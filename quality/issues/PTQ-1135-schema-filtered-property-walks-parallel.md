...
verdict: questionable — accounting re-verified independently: excerpts match at :121-162/:237-297 (one-line tail drift), both walkers live (#build :430/:432), both dispatch over the same 15 keywords (4 map + 8 valued + 3 list, 15/15 each) and duplicate the tuple-`items` arm plus isSchemaNode/Array.isArray gating, clone-scan map → no groups, identifier grep across src/extensions/tools/tests hits only this file, no tracked PTQ shares the root cause (same-wave d9-01 is placement of the whole V8c block, PTQ-0730 is the tests-side hasOwn); Observation overstates that detection special-cases patternProperties (it folds it into the map arm — the Why has it right) but the coverage count stands, and since the keyword lists are already the three shared constants the residual drift surface is the branch structure only — a shared visitor walker is a design decision for a human ruling (triage: claude-fable-5-1)
verdict: questionable — accounting re-verified from scratch: excerpts byte-match at :121-162/:237-297, both walkers live (compile :430 → #hardenedAjv.compile(translateFilteredProperties) :432), both dispatch over the same three constants (4 map + 8 valued + 3 list = 15/15) and each re-implements the tuple-`items` arm and isSchemaNode/Array.isArray gating; clone-scan map on the file → (no clone groups); identifier grep across src/extensions/tools/tests hits only this file; dedupe clear (PTQ-1118 is the params/query-schema-lowering `$defs` hoist, same-wave d9-01 is whole-block placement, PTQ-0730 is tests-side hasOwn); Observation overstates that detection special-cases `patternProperties` (it is folded into the map arm — the Why has it right) but the coverage count holds; since the keyword lists are already one source of truth, the drift surface is the duplicated branch structure, and a shared visitor walker is a design decision for a human ruling (triage: claude-fable-5-1)
