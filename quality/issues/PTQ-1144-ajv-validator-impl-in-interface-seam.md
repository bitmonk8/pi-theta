...
verdict: questionable — accounting verified a third time from the current tree: wc -l 452; interface decls at 14/16/29/35 with zero imports and no reference below 37; V8c divider at 39-41, first import at 43; the 15 production decls sit at 63/71/85/97/99/104/121/170/202/237/306/318/321/329/348 and lean on exactly 4 interface names (SchemaValidator via implements, CompiledValidator, ValidationError, LoweredSchema); the six sibling H3a files declare no const/function/class (token-estimator.ts carries one type import) and each pairs with a separate V8* adapter (host-tool-snapshot.ts is a standalone non-H3a seam, not a counterexample); index.ts re-exports only the 4 interface types; sole src importer of AjvSchemaValidator is production-composition.ts:132, SchemaSlug's is :141, 56 test files import it; no schema-validator row in quality/exemptions.json; git history shows the V8c commit 07bb79b6 added the adapter into the H3a skeleton 8595ed6c directly — no reverted prior split; no open PTQ tracks this root cause and the other two intake filings on this module (d4-01 parallel walks, qw20260921001431-d9-01 query-schema-lowering) are different root causes — misplacement accounting holds, but the sibling-module move is a design decision for the human's accept --note (triage: claude-fable-5-1)
