...
## Triage
verdict: confirmed — all 5 excerpts verbatim at cited lines and every search reproduces (entry ctor at 2009/3134 are the only two in src/; #trackForwardingSources call sites 2195/2512/3142); #openInvocationTicket's callers are only 1986/2190/2511 and the comment at 3074-3076 states the fn path "does NOT go through" spawnSubagentConversation, so the third registering site is real and detaches in `dispose` with zero finishInvocation refs, falsifying both field-doc rosters; blame dates confirmed; not a dupe of d2-01-registry-exposes-only-two (different file/header) or d2-08-filesystem-activeroots (adjacent fields). (triage: claude-opus-5)
