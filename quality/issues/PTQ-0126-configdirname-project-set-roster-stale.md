...
## Triage
verdict: confirmed — re-verified: the "exactly" roster (file-system.ts:31-34) names 4 items while 5 seam call sites exist and settings.ts:394 matches none; sibling roster :59-60 and the same-commit witness roster (tests/host-config-dir.test.ts:24-27) both name the `thetaPathsBaseDir` arm, so the omission is real though git blame 7f360d20 shows same-commit authoring rather than "decay" (grep is 22 lines, not the claimed 16) (triage: claude-opus-5)
