...
## Triage
verdict: confirmed — both excerpts match at HEAD (395-408 / 608-621, 3-line drift); clone-scan map reproduces G058 renamed-only at the cited ranges; both copies live in production (containsNamedType at :440, containsWithheldBinderType at 12 gate sites); same-file recursive fold over the 6-arm CompatType differing only in the `named` leaf test — mechanical dedupe; no exemption row, no tracked PTQ or sibling intake (d4-01 is G069, d4-19 is the binder walk) covers it; note the stated drift would surface as a strict-mode TS missing-return error rather than silently, but the clone stands (triage: claude-fable-5-1)
