...
verdict: questionable — accounting re-verified independently: theta-document.ts:969 Stmt = 18 members, :463 Expr = 20; binder walk 739-798 covers 18/18 (11 explicit + default 7) and 800-871 covers 20/20 (14 explicit + default 6, `query` correctly defaulted since QueryExpr.template is a raw string with no nested Expr); type-layer walkStmt 1625-1957 covers 18/18 (same 11 + default 7) and walkExpr 3186-3434 covers 20/20 (15 explicit + default 5); excerpts byte-exact, both walks live (collectLocalBinderNames ← import-static-checks.ts:1937, type-layer-checks.ts:358; 42 internal walk calls); the "both copy names when entering blocks" claim is refuted for the binder walk (flat Set, walkBlockForLocalBinders 730-737, vs seven `new Map(bindings)` copies in the type-layer walk) but does not touch the counted claim; no tracked duplicate (PTQ-0288 resolved = extension-module call-site walker clone, PTQ-0330 = D8 repeated call, PTQ-0876 = D7, same-wave d9-01 lists the walk only as a D9 inventory row); D4 parallel is never confirmed — the shared traversal source of truth is a design decision for a human ruling (triage: claude-fable-5-1)
verdict: questionable — accounting verified at HEAD after post-filing drift (fix commits 71af3b5b/bb21dacc moved the spans to 651-710 / 712-787 / 1368-1691 / 2920-3161; all four excerpts byte-exact at the new offsets): theta-document.ts:980 Stmt = 18, :474 Expr = 20; both Stmt walks handle the same 11 explicit arms + default 7 = 18/18; binder walkExpr 14 explicit + default 6 (`query` correctly defaulted — QueryExpr.template is a raw string) and type-layer walkExpr 15 explicit + default 5 = 20/20; both walks live (collectLocalBinderNames ← import-static-checks.ts:1937, type-layer-checks.ts:351); the "both copy names when entering blocks" claim is false for the binder walk (sole `new Set` is the seed; type walk has 6 `new Map(bindings)` copies) but is prose outside the counted claim; no duplicate (PTQ-0330 D8 repeated-call fixed, PTQ-1133 for/par-for clone inside the type walk fixed, PTQ-1128/1129 other collectors); parallel class is never confirmed — the shared traversal source of truth is a design decision for a human ruling (triage: claude-fable-5-1)
