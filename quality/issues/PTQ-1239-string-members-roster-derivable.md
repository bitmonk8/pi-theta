...
verdict: questionable — accounting verified; the simpler shape is a design decision for a human ruling: STRING_MEMBERS (stdlib-string.ts:36-46) and STRING_MEMBER_SIGNATURES (:61-71) re-read byte-exact with the same nine names in the same order and the :57-60 "hand-written and independent … silent drift a reviewer must catch by inspection" comment verbatim; grep across src/ tests/ extensions/ tools/ finds exactly one external consumer (type-layer-checks.ts:70/219 builtinMembers, consumed only via .has), so new Set(STRING_MEMBER_SIGNATURES.keys()) covers every cited need; quality/exemptions.json has no stdlib-string row; expressions.md pins the member names not two carriers so no challenges_spec needed; ARRAY/OBJECT siblings are noted not filed (one root cause); PTQ-0024/1119/1216/1217 target detached docs, dispatcher clone, and substrate/concatElementType placement — none claims the roster pair (triage: claude-fable-5-1)
verdict: questionable — accounting verified; the simpler shape is a design decision for a human ruling: STRING_MEMBERS (stdlib-string.ts:36-46) and STRING_MEMBER_SIGNATURES (:61-71) re-read byte-exact with the same nine names in the same insertion order, and the :57-60 "hand-written and independent (the allow-list predates this table) … silent drift a reviewer must catch by inspection" comment is verbatim and traces to 52712fb3 (bug-0315), stating history not a consumer need; grep of STRING_MEMBERS across src/ extensions/ tools/ tests/ finds exactly one external consumer (type-layer-checks.ts:70 import, :219 builtinMembers return, read only via .has at :236 for theta/parse/unknown-method) plus two sibling doc-comment mentions (stdlib-array.ts:58, stdlib-object.ts:108), no string-keyed/dynamic access, and no test or gate pins the literal form, so new Set(STRING_MEMBER_SIGNATURES.keys()) covers every cited need feature-for-feature; quality/exemptions.json has no stdlib-string row; expressions.md pins member names not two carriers so no challenges_spec needed; ARRAY/OBJECT siblings noted not filed (one root cause); PTQ-0024/1119/1216/1217 target detached docs, dispatcher clone, and substrate/concatElementType placement — none claims the roster pair (triage: claude-fable-5-1)
