...
## Triage
verdict: confirmed — both cites exact; independent repo-wide hunt (`.mutable`, bracket/destructured/spread access, tests) finds zero `Resolution.mutable` readers, written only at lex-env:601, while every sibling field has a reader and enforcement lives in `writeBinding`'s `slot.mutable` check (triage: claude-opus-5)
