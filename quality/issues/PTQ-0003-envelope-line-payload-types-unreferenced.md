...
verdict: confirmed — independently reproduced: `\bEnvelopeLine\b` has one repo-wide hit (its own declaration) and `ThetaResultPayload` two (declaration + the field inside dead `EnvelopeLine`); no importer of the module names either, no `export *`/barrel carries them, and `git log -S` shows one introducing commit (4866d4d2) never wired to a consumer (triage: claude-opus-5)
