...
verdict: questionable — re-verified independently: `driveSubagentRootRegime` at 3003 dispatches `#driveSubagentFnEntry` (3270) at 3048 on `entry.kind === "fn"` so both drives are live; both cited excerpts reproduce (theta arm tail 3155-3172 catch stamps `cause: "internal_error"` for every non-HostFatal throw, fn-entry catch 3403-3413 splits `isThetaPanic(thrown) ? "panic" : "internal_error"`); `clone-scan map --files src/extension/production-theta-producer.ts` → `(no clone groups)` so the parallel is correctly outside the token map; 9-phase coverage table checks phase-for-phase (the body prose "8 of 9" is a slip, table shows 9/9 — non-blocking); the phase-9 divergence is real and the spec genuinely pulls both ways (pi-integration-contract/subagent.md:159 "panics routed as internal-error" vs invocation.md:95/99 `cause: "panic"` through the child envelope), so which copy is right and whether a shared entry-kind-parameterised drive helper is the home are human design rulings; no D4 duplicate in quality/issues — PTQ-1192/PTQ-1204 are per-host D9 breakdowns and PTQ-1285 the file-level D9 (triage: claude-fable-5-1)
verdict: questionable — accounting verified against current HEAD (927b8b63): both excerpts reproduce byte-for-byte but have drifted ~130 lines since filing (theta-root arm now 3184-3304 inside `driveSubagentRootRegime` at 3130, which dispatches `#driveSubagentFnEntry` (now 3397-3542) at 3175 on `entry.kind === "fn"`, so both copies are live); `clone-scan map --files src/extension/production-theta-producer.ts` → `(no clone groups)`, so a hand-diffed parallel filing is the correct route; the 9-phase coverage table checks phase-for-phase (intake→`emitErr` mint, bind, `bindPromptConversation`, `executeBody`, `surfaceCalleeFinalValue`+`mapTooDeepReturnValue`/`mapNonRepresentableReturnValue`, `serializeOkEnvelope`+`emitOutcome("ok")`+`#requestVisibleChildShutdown`, propagated Err, catch+`teardown`/`finishInvocation`) — the prose "8 of 9" is a slip against a 9/9 table, non-blocking; the phase-9 divergence is real (theta-root catch 3290-3299 stamps every non-HostFatal throw `cause: "internal_error"`; fn-entry catch 3537 splits `isThetaPanic(thrown) ? "panic" : "internal_error"`) and the spec pulls both ways (subagent.md:159 "panics routed as internal-error" vs error-model.md:33/:103 `cause: "panic"` to an `invoke` parent), so which copy is right and whether an entry-kind-parameterised drive helper is the shared home are human design rulings — D4 parallel is never confirmed; no D4 duplicate: PTQ-1285 lists both drives in one D9 concern row and PTQ-1192/PTQ-1204 are per-host D9 breakdowns, none files the cross-drive parallel (triage: claude-fable-5-1)
triage worker failed (verdict not applied; re-triaged next wave) (loop, 2026-09-23)
verdict: confirmed — RATIFIED (human, 2026-09-23): confirmed - batch ruling; triage verification trusted. TARGET SHAPE: ONE shared envelope-settlement/validation helper used by both the theta-root and fn-entry drives; the clone-twin finding (envelope-ok-arm-validation-cloned) is fixed by the same extraction.
