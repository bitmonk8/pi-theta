...
verdict: confirmed — RATIFIED (human, 2026-09-23): confirmed - batch ruling; triage verification trusted. TARGET SHAPE: compile-time exhaustiveness tether over the shared outcome union - both boundary projections (in-process and child) must fail tsc when the outcome set grows; keep the two projections separate (different boundaries), tether them.
