...
## Triage
verdict: confirmed — both excerpts match verbatim at the cited lines; clone-scan map reproduces G020 (renamed-only, type-layer-checks.ts:1882-1901 / 3351-3368); both `for` and `par-for` arms are live parser-constructed kinds (theta-document.ts:3621, :6401); not a spec vector table; no existing PTQ covers this dedupe (the D9 walkExpr-breakdown intake is a different root cause) (triage: claude-fable-5-1)
