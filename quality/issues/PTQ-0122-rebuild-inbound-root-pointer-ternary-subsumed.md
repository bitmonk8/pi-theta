...
## Triage
verdict: confirmed — re-verified: `pointer` has zero assignments file-wide and no shadow in the function, the :365 guard returns for every `pointer !== ""` plain object, and lines 366-407 are straight-line, so :408's `: wireKey` arm is unreachable; `rebuildInbound` is module-private with exactly 3 call sites (:293 `""`, :346 `/items`, :420 `fieldPointer`), and `orderedEntries`' doc at :522-526 states the same guarantee makes such a check redundant (triage: claude-opus-5)
