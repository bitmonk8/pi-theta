...
## Triage
verdict: confirmed — both vestige prongs reproduce (only body use is the `void` discard at :92; all 3 call sites pass the identical `{ green: true }`), `SurfaceInventoryOutcome` has no reference outside that parameter's annotation, and nothing constrains the arity — pruned V18d leaf, no spec-mandated signature, no `noUnusedParameters`/lint rule (triage: claude-opus-5)
