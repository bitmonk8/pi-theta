...
## Triage
verdict: confirmed — re-verified independently: `surface` is required at :550, `checkInvokeExtension` destructures only `{ literalPath, site }` (:568), `InvokeExtensionInput` has no other consumer, both production sites pass the constant "tools" (callable-set.ts:424, production-composition.ts:2152 — 8-line drift from the cited :2144, content exact), 0 readers and 0 dynamic access across src/extensions/tools/tests, and invocation.md §Resolution mandates one code for both surfaces, so the field satisfies both prongs of the lens's vestigial-field test (triage: claude-opus-5)
