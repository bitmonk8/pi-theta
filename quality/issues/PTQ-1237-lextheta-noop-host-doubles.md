...
verdict: questionable — third independent pass, accounting verified: lexer.ts:92-94 and :123-128 excerpts byte-exact; grep `sendMessage: () => {}` src/ → exactly 3 hits (theta-document.ts:1235/1264/3444), the three deps literals byte-identical inside parseExpressionSource/parseInterpolationSource/firstForbiddenInterpolationToken, each reading only lex.tokens (or BodyParser.diagnostics) while lexTheta returns the full diagnostics array on LexResult, so the quoted producer-delivery grain is genuinely fought at these sites; lexTheta has 4 production callers with :211 the only real-channel one; the :1227-1229 rationale defends inline-vs-shared-state, not the mandatory parameter; quality/exemptions.json has no row for lexer.ts or theta-document.ts; no spec_topics clause requires a note for the `<interpolation>` re-lex; only resolved PTQ rows mention lexTheta → not a duplicate; but the simpler shape sits on a pinned design ruling — docs/bugs/0255 §Fix 'V7d producer-facing seam contract … deliberately unchanged; the three no-op-channel callers untouched (constraint 2)' and 0264 §Non-goals 'Making lexTheta a pure returner is 0255 §Fix candidate 1, rejected on the record' — the filing's optional-deps / pure-sibling-entry route differs from the rejected candidate (the :211 whole-file caller keeps emitting) and 0255 constraint 2 forbids requiring a real channel from these callers, so the direction is consistent with the record yet unratified and would edit the docstring contract tests/lexer-core.test.ts:13-16 pins; D8 is never confirmed — human ruling required (triage: claude-fable-5-1)
verdict: confirmed — RATIFIED (human, 2026-09-21): confirmed with NAMED SHAPE - bug 0255 pinned the V7d design (lexTheta's system-note delivery channel is MANDATORY; a pure-returner was explicitly rejected). The fix must NOT alter lexTheta's signature or make the channel optional. Target shape: consolidate the three byte-identical inline no-op SystemNoteChannelDeps literals into ONE shared, named inert-channel helper (or a thin snippet-lex wrapper owning it) that the three snippet-lex sites import; the doc on that helper must state WHY discarding is sound there (diagnostics already returned on LexResult) and cite bug 0255.
