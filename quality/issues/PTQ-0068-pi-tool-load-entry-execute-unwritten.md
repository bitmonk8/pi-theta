...
## Triage
verdict: confirmed — verified: PiToolLoadEntry has exactly 2 refs (production-composition.ts:3364 decl, :3391 return annotation), unexported and absent from dist/; its sole constructor omits execute, both call sites (:2225, :3027) only wrap into toolDefinition: unknown, and `.execute =` / ["execute"] searches are 0 hits — git blame proves the vestige (fda23a4b had execute REQUIRED with a rejecting stub; b8d4fd2c0 deleted the stub and relaxed it to optional), while the real reader-side optionality lives in PiToolDispatch.execute? (production-theta-producer.ts:392) (triage: claude-opus-5)
