...
`max_cycles` (default 3), `shard_loc` (target lines per review shard, default
6000), `budget` (max candidates per shard, default 10), `parallel` (fan-out
width, default 4), `push` (default true), `gate_cmd` (offline verification gate,
default `npx tsc --noEmit && npm test`).
...
4. **Review** — one lens worker per shard in parallel (D2 cruft:
   ...
   scopes). Candidates land in `intake/`, shaped by `TEMPLATE.md`. Reviewed
   files are marked in `state.json` at the reviewed sha — fix commits re-dirty
   them, so the next cycle re-reviews exactly what changed.
...
8. **Commit/push** — one store commit per wave, one commit per fixed cluster.
...
| Path | What | Versioned |
|---|---|---|
| `surfaces.json` | lens → reviewable file set (include/exclude prefixes + extensions over git-tracked files) | yes |
...
Add a lens = one surface entry in `surfaces.json` + one worker theta in
`.pi/theta/workers/` (its own `model:` pin — that is why workers are separate files)
+ a roster entry in `quality-loop.theta` (needs-review/shard/review block per
lens). Experiment-backed picks: D4 duplication → kimi-k2.7-code or
gemini-3.7-flash; D7 test quality → kimi-k2.7-code; D8/D9 → gemini-3.7-flash
with fable arbiter; D1/D6 → fable only.

## Committing note

The repo's parse gate (`tests/committed-fixture-parse-gate.test.ts`) pins exact
counts of committed `.theta`/`.thetalib` files (currently 36/3, including the
