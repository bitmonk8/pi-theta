...
### Fixed
- **Bug 0477 — author-written `theta_progress(...)` was unreachable from theta code on Oh-My-Pi: pi-theta's OWN in-process tool was dispatched through the PIC-64 host-loop bridge, whose provider registration a host reserving built-in API names rejects and whose fabricated-turn settle that host does not drive.** The code-side dispatch treated `theta_progress` like any third-party extension tool (the `pi.getAllTools()` snapshot strips `execute` from all of them) and routed it through the host-loop bridge, which (a) called `pi.registerProvider` with `api: "openai-completions"` — a built-in name Oh-My-Pi's `api-registry` reserves, throwing `Cannot register custom API "openai-completions": built-in API names are reserved.` and surfacing to code as `Err(CodeToolError{cause:"execution"})` — and, once that was corrected, (b) fabricated a host turn the second host never drove to `agent_settled`, hanging the call in both the prompt-parent and subagent-child regimes. The author-facing RFC 0010 L3 surface was therefore completely non-functional from theta *code* on that host (the model-issued `tool_use`, which flows through the real host tool loop, was unaffected). **Fix:** the bridge provider now uses a bespoke, non-reserved API tag (`BRIDGE_API = "theta-host-loop-bridge"`; `provider-composer` runs the bridge's own `streamSimple` whenever `model.api === provider.api`, so no real adapter is consulted and Pi is unaffected); and — the headline — `registerThetaProgressTool` now returns a `ThetaProgressRegistration.codeSideExecute` that runs the SAME `executeThetaProgress` closure (sharing the one per-registration clamp/drop `state` with the model-facing `execute`), which the factory threads per process as `inProcessToolExecutors` (`composeExtensionInstance` → `runComposePass` → the producer input) so a code-side call to pi-theta's own tool dispatches its handler DIRECTLY — same CANCEL-3 swallowing-handler attachment as a built-in's `execute`, never a fabricated turn. Each process (parent and every subagent child) wires its OWN executor, so the child's `isChildRegime` handler still emits the EXST-15 wire line. Verified end-to-end on `omp/18.1.16`: a code-side `theta_progress(...)` returns `ok` in prompt mode and proceeds past the call in a subagent child, where it previously errored or hung. Filed as bug 0473 on the originating branch; renumbered to 0477 at merge time — main independently filed an unrelated bug 0473 while this fix was in flight. Details: [`docs/bugs/0477-code-side-theta-progress-routes-through-host-loop-bridge.md`](./docs/bugs/0477-code-side-theta-progress-routes-through-host-loop-bridge.md).
...
### Added
- **RFC 0010 — live execution visibility for running thetas: layers L0–L3 in one release** — a running theta is no longer visually silent between its binder echo and its final note. **Operator surface:** a footer heartbeat (`ctx.ui.setStatus("theta", …)` — theta, elapsed, current effect kind @ source site + age, child lane summary — plus `setWorkingMessage` while a driven prompt-mode turn streams; L0), a collapsible invocation-tree widget (`ctx.ui.setWidget`, `belowEditor`, 6-line budget with overflow elision; L2), the session-scoped `/theta-status off|min|tree` view command (live instance only, never persisted), and the `theta.progress: "off" | "counts" | "names"` settings key (default `names`, the first closed-string-enum scalar key) selecting the telemetry-class ceiling — view shape and class ceiling are distinct axes. **Plumbing (L1):** a per-instance **ExecutionStatusBus** (coalesced re-render on a 200 ms last-render-anchored throttle through the injected `Clock` seam — erratum B pins the deliberate contrast with the event-anchored reload debounce), fed by a checkpoint decorator publishing `(invocationId, kind, site)` per effect (checkpoint-site telemetry; the PIC-10 macrotask/microtask yield semantics survive the wrapper, pinned suites green unmodified), `par for` lane lifecycle hooks (`queued|running|done|err` slots), dispatch lifecycle, and a child-activity tap — one extra listener on the existing stdout line pump projecting each `--mode json` event to bounded class-1 fields at ingest (event kinds, counts, timing, tool names; content never retained), no per-line diagnostics. **Bug 0469 fixed — the principled fix:** the three operator-facing note classes (load/parse/type diagnostic batches, structural-change notes, binder-model recovery notes) migrate off `pi.sendMessage` onto the new LLM-context-free `theta-progress-entry` entry channel (PIC-71) — entries never enter provider replay, so a watcher-driven emission can no longer land between a `tool_use` and its `tool_result`; delivery is two-tier per PIC-72 (entry channel primary, the pre-existing message-channel realization as fallback with byte-identical rendered lines — erratum A: the migrated classes fall back to the message channel, never to silence), and the b0268-class entry-path normalization fix applies `withNormalisedFileSpelling` once above the entry/message branch so both realizations carry identical all-POSIX `details.diagnostics[].file` spelling. **Author surface (L3, hold discharged):** the `theta_progress` extension tool (`{ message, scope?, done?, total? }`) in both regimes — parent regime publishes to the bus and appends a durable milestone entry; child regime emits exactly one `theta_progress` reserved-key stdout line per accepted call and touches nothing else (EXST-15), under the PIC-74 two-member wire pin (`{theta_result, theta_progress}` closes the reserved-key set) with the ratified clamps — 200-char message / 64-char scope / 200 ms inter-acceptance interval / 4096-byte wire line, monotonic `seq`, no upward relay — the parent tap re-clamping defensively and rendering wire-ingested self-reports on transient sinks only, never as entries (erratum D). **Degrade posture:** every sink is best-effort per surface — compose-time presence probe, per-call try/degrade, first hard failure permanently degrades that sink (`RendererGate` pattern), `ctx.hasUI` advisory only — via the new optional-capability class (PIC-73, `OPTIONAL_UI_CAPABILITIES`, five members probed-never-gating; `CAPABILITY_OBLIGATIONS` untouched at 7, the GOV-31 witness intact). **Zero new diagnostic codes** (DIAG-2): wire and parse failures fold into the existing stray-line tolerance and first-offender discipline. Spec surfaces: the new `execution-status.md` topic page minting EXST-1…EXST-15, PIC-71…PIC-74 across the pi-integration contract, the SLSH-2 three-class telemetry carve-out (telemetry / author messages / content — content unobservability unchanged), and erratum log A–D on the RFC. (`docs/rfcs/0010-live-execution-visibility.md`; `src/extension/execution-status/` (bus, checkpoint decorator, child tap, entry channel, footer/widget sinks, progress tool, status command), `src/extension/sdk-inventory.ts`, `src/discovery/settings.ts`, `src/extension/production-composition.ts`; witnesses `tests/execution-status-*.test.ts` (12 files), `tests/rfc-0010-spec-surface-gate.test.ts`, live `tests/live/execution-status-parfor-ui-live-cell.test.ts`, `tests/live/rfc0010-l3-progress-parent-live-cell.test.ts`, `tests/live/rfc0010-l3-progress-wire-child-live-cell.test.ts`)

## [0.466.0]

### Added
- **RFC 0009 — call-site `with { cwd: <expr> }` clause (theta 1.3): per-call working directory for subagent-mode dispatch** — `callee(args…) with { cwd: tree }` on exactly the two surfaces that spawn a child `pi` process (frontmatter-`tools:` `.theta`-callable calls and `invoke(...)`/`invoke<T>(...)`), unlocking worktree fan-out. Errata A/A′ make callee admission **default-reject**, judged against the frozen callable set alone: a clause on a `subagent fn` (which runs in-process, no child), a plain or imported `fn` (re-export chains included), or any unclassified callee draws minted `theta/parse/with-clause-in-process-callee`; a Pi-tool callee draws `theta/parse/with-clause-pi-tool`; an out-of-set key draws `theta/parse/with-clause-unknown-key`; a statically-resolvable prompt-mode callee draws `theta/parse/with-clause-prompt-mode-callee` — four new parse codes; rules pinned as INV-6 (value semantics) / INV-7 (no pre-check) / INV-8 (mode gate) + TOOL-1 (Pi-tool arm). Value semantics: the `cwd` value is a full runtime expression evaluated after the args, left-to-right (panic/early-`?` aborts pre-spawn); `string`-typed via the existing arg-type checks; empty string and runtime prompt-mode callees surface the existing `Err(InvokeInfraError { cause: "validation" })`; relative paths `path.resolve` against the parent’s effective cwd (composes across nesting; Windows separator spelling normalized parent-side, bug-0467 class); no existence pre-check — a bad cwd maps to the existing `theta/runtime/subagent-spawn-failed` with an INV-7 ENOENT `(cwd: …)` message-slot enrichment in the launcher catch. **Zero new runtime codes** (DIAG-2). Launch threading is one bind — `cwd: resolvedCwd ?? ctx.cwd` into the existing `SubagentLaunchRequest.cwd` — so an absent clause is byte-identical to pre-0009. Identity/location principle: cwd relocates the callee’s **side effects**, never its **identity** — callable-closure hash (cwd is not a digest input), `--theta` roots, extension pin, and trust flags all unchanged; trust does not follow cwd. Tests-first: 22 red cells verified red at HEAD, then green; H8a live witness relocates a real spawned child’s marker file into a fresh temp dir (clause-free control unchanged), red-direction proven. (`docs/rfcs/0009-per-call-subagent-cwd.md`; `src/parser/theta-document.ts`, `src/extension/invoke-static-checks.ts`, `src/extension/production-theta-producer.ts`, `src/runtime/subagent-launcher.ts`; witnesses `tests/call-with-clause-{parse,static-checks,threading,failure-arms,hash-stability}.test.ts`, `tests/rfc-0009-spec-surface-gate.test.ts`, live `tests/live/call-with-clause-cwd-relocation-live-cell.test.ts`)
...
- **Bug 0399** — the SLSH-4 note's boundary-constructed `RuntimeEvent` omitted the three shape-pinned fields outside 0383's adjudicated freshness set: `attempts` (rendered in the SNK-a content line two statements earlier, absent from the payload — content and details disagreed about the same fact on one message), `tokens_used` (on the context-overflow leaf when the provider supplied it), and `masked` (PIC-1(d)'s only reachable V1 co-fire lands on exactly this event class; the 0355-corrected origin event carrying it was built by `buildValidationEvent`, returned on `TypedQueryOutcome.validation.event`, and read by NOBODY — the QRY-20 discard path preserved more from the same leaf than the user-facing boundary). PARENT ADJUDICATION both-halves-one-commit (recorded): (i) the origin `validation` event is threaded `OperationResult` → `BodyExecution.originEvent` → `emitTopLevelErrNote`'s 0383-installed `event?` forward hook, `masked` preserved VERBATIM per PIC-1(f) (re-deriving at the boundary is non-conformant even when the value comes out right — the originating site is the only authoritative source); (ii) the boundary-built arm (kinds with no origin event) preserves `attempts`/`tokens_used` from the leaf exactly `buildDiscardEvent`-shaped (`"attempts" in error` + number-typed; `tokens_used` number-only so null stays canonically absent). Witnessed by `tests/b0399-boundary-event-attempts-tokens-masked.test.ts` (6 cells both directions: attempts present, overflow counts present/null absent, repair-terminal cascade under the ceiling-#2 predicate carries `masked: ["ceiling#2"]`; red = all absent). 0383's freshness set not re-litigated. Live: shared with the co-landed 0397 run under the lock. Co-landed with 0397 (same lane, same producer/event surfaces).

## [0.392.0]

### Fixed

- **Bug 0397** — every binder-failure note shipped `details: { event: {} }` where the always-log contract makes binder failures group-A members: `runtime-event-channel.md` pins the sole emission per occurrence to carry a `RuntimeEvent` with `kind` = the failure cause and `invocation_id`/`theta` sourced from the ActiveInvocationRegistry entry (`:83`, `query_site` absent) — the shipped `#emitBinderFailureNote` hardcoded the empty object on all six routes (malformed envelope, cancellation ×2, the per-class retry-budget outcomes, post-default-merge AJV), so the exactly-once always-log guarantee was unwitnessable and the group-A dedup tuple read three absent fields (the 0383 mechanism on the sibling surface whose payload the spec pins independently). PARENT ADJUDICATION constraint-set-as-fix (recorded): (1) the binder-failure `RuntimeEvent` is constructed once per failure — `kind` from the retry taxonomy's cause, `message` = the template-interpolated string, `invocation_id`/`theta` from the registry entry per `:83` (NOT fresh mints), `occurred_at` via `Clock.wallNow()`, `query_site` omitted; (2) `buildRuntimeEventNote` reused (no forked builder, 0383 precedent) and the per-variant matrix's content column clarified for the binder rows same-commit; (3) seam premeasured both ways — READ was unreachable at the emit site, so the registry ticket is THREADED (the 0383-D2 sanctioned route; choice + evidence in the fix record); (4) witnessed across ALL SIX routes by `tests/b0397-binder-failure-note-runtime-event.test.ts` (7 cells; red = the `{}` signature). Live: `err-note-render-record-error-field-live-cell` under the lock (adjacent note-surface drive; recorded WHY). Lane intervention recorded: an implementer's unauthorized 57-file citation churn (including the line-pinned LPA) was caught and reverted byte-exact before review.
...
- **Bug 0374** — the published audit's exemption-marker subsystem diverged from the pinned rules on three axes: the multi-line originating-line rules were not honoured (a SPEC-PLACED marker drew a violation while the spec-malformed placement silently authorised — rule (iii)'s ctx-identifier-line and rule (ii)'s import-keyword-line placements both failed), the stale sub-kinds (s1)/(s2) never fired (pass 2 scanned only lines already carrying a ref, so orphan markers were unreachable and the resolved-ref short-circuit discarded (s2)'s evidence — stale markers could outlive their surface forever), and malformed clauses (a)–(g) collapsed onto one `malformed-grammar` token where the spec pins one stable token per clause with a fixture each. Per the settled §Fix, all three core-local: (1) a per-shape originating-line map (cat-1 property line as before; cat-3 the carrier `ctx` identifier line; cat-2 both the specifier line AND the declaration's import-keyword line — either authorises); (2) `classifyMarker` returns the violated clause and each of (a)–(h) mints its own `<symptom>` token (10 family-5 tokens, a fixture per token); (3) two stale passes over ALL lines — (s1) well-formed marker with zero recognised refs, (s2) marker whose line's every ref resolved upstream. The three real in-tree markers premeasured against the new attribution + stale passes: all still authorise, no flip. MERGE-TIME MICRO-RATIFICATION (recorded, 0322 class): two redundant `// allow-pi-surface:` markers on the inventoried carrier import in `tests/inventory-closure-audit.test.ts` removed — the ratified (s2) pass's own mechanical consequence (fixture-input only, assertions untouched). Gate test gains the 10 family-5 + 4 placement fixtures (with 0373's 13 family-4 fixtures in the same co-land). Live: covered by the co-landed `b0351live` run under the lock (build-time subsystem; recorded WHY).

## [0.381.0]

### Fixed

- **Bug 0373** — the published inventory-closure audit detected NONE of the family-(4) rebinding/laundering shapes the spec prohibits non-exemptibly: `const { ui } = ctx`, `const reg = pi.registerCommand`, computed `pi[...]` access, carrier-typed class fields, `interface MyCtx extends ExtensionContext`, `<C extends ExtensionContext>` laundering, `Readonly<ExtensionAPI>` parameters — all passed the gate with zero records, so any one-line rebinding silently exempted all downstream `pi.*`/`ctx.*` reach from closure coverage forever. PARENT ADJUDICATION Option 1, carrier-scoped (recorded — resolving the s30 STOP's doc-vs-tree contradiction): the doc's Non-goal-1 premise ("today's tree contains no prohibited shape") was FALSE — `LivePromptQueryModel` carried `readonly #pi: ExtensionAPI` / `#ctx: ExtensionCommandContext` class fields predating the doc, the exact non-parameter carrier-binding shape, so the tree-clean refactor was adjudicated IN SCOPE: the fields are narrowed to `Pick<>` capability seams (behaviour-preserving stored-type narrowing; the typed-query driver's observable behaviour byte-identical, its suites green unflipped), and the fix record documents the falsified Non-goal (the 0362 doc-was-wrong-note pattern). Detection is CARRIER-SCOPED, consistent with the module's existing `inPiCarrier`/`inCtxCarrier` resolution rules: visitors key on carrier-literal-typed roots/annotations and on `pi`/`ctx` within canonical carrier scope — the header-documented structural-carrier laundering-avoidance pattern in `production-host-loop-dispatch.ts` stays green (sanctioned), and the producer's two non-carrier `ctx.model` sites stay green. Eleven new family-4 visitor arms with their own `<symptom>` tokens + the pass-3 captured-rebinding context check (property access whose parent is a variable-declaration initialiser / assignment RHS / property assignment inside carrier scope), each with red fixtures per the spec's per-symptom floor (13 family-4 fixtures in the gate test). The gate's green-on-main is now TRUTHFUL. Live: `b0351live` (real `pi -p` through the refactored driver plumbing) green under the lock.
...
- **Bug 0370** — a reassignment's TARGET was never resolved against the scope model: inside a `fn` body a write to an undeclared name or an immutable parameter parsed clean and was SILENTLY DISCARDED (the WriteResult thrown away), a write to a caller-scope `let mut` parsed clean and LANDED across the no-closures activation boundary (lexical capture the spec excludes — silent shared mutable state through an isolation boundary), and through that channel `x += f()` computed 12 where its normative desugar `x = x + f()` computed 3. The settled three-layer §Fix shipped: (1) PARSE — the identifier walk now covers reassign TARGETS under the same closure-free scope sets reads use (out-of-scope/undeclared → `theta/parse/unknown-identifier`, the read-side row REUSED per the recorded in-lane adjudication), and the mutability map records parameters, `for` variables, `match` bindings, and the `params:` seed (writes there → `theta/parse/immutable-rebinding`, with an exact `immutableRebindingEmitted` defer flag keeping G6 byte-identical); (2) RUNTIME BELT — `writeBinding` honours `fnActivationBoundary` (mirroring `localShadowsCallable`'s stop) and the reassign arm stops discarding the WriteResult (a rejected write after the gates hold throws `RejectedWriteDefectError` → `theta/runtime/internal-error`, the 0314/0369 family); (3) ORDER — the compound arm reads the target BEFORE the RHS, so bindings.md:14's desugar holds. NO new registry rows (DIAG-2 Trigger prose widened only); permitted-codes byte-identical. FLIPS under the PARENT RATIFICATION's two classes (the 0291 class-model, per-cell confirmations in the report): CLASS 1 — 0115 f2/f3 (the cells that PINNED the silence) + g4 and 0126 e2 gain `immutable-rebinding` with their own type/element verdicts unchanged; CLASS 2 — 0225 A10 gains `unknown-identifier` with its route subject preserved; dated notes on all three closed docs. Witnessed by `tests/b0370-reassign-target-scope.test.ts` (33 cells: the G-rows — silent no-op writes, cross-boundary landing, the 12-vs-3 desugar divergence — all die; same-scope mutation, G6/G8, and 0303 module-scope controls byte-identical) and the NEW H8a live cell `tests/live/b0370-reassign-target-scope-live-cell.test.ts` (red-proven, green under the lock). Residual recorded (follow-up filing candidate): the flat `this.bindings` map is not block-scoped (pre-existing) — three edge classes reach the loud belt instead of a clean parse refusal, and a reverse dead-shadow yields a pre-existing false `immutable-rebinding`.
...
- **Bug 0380** — a non-identifier `params:` key registered with ZERO diagnostics, and a break-carrying key (an explicit-key block scalar cooking to `a\nTheta: /evil`) then FORGED both line-oriented renderings that interpolate the field name bare: a second `Theta: /<name>` identity line / phantom per-field line in the binder system prompt (whose line shapes binder-bypass-and-envelope.md pins as MUSTs precisely so the binder can parse them), and a byte-perfect second `Running /<name>: …` line in the user-facing echo note — the 0060/0087 defect class re-opened on the one interpolated token neither fix normalised, on a fully green path. PARENT ADJUDICATION Option A (recorded: every sibling field-name position refuses this spelling — 0154's inline-object rows, schema-body grammar, 0149's case axis on this very walk; the implementation's own comment calls the params key "a field-name position twice over"; a non-identifier key can never be the typed body variable frontmatter-fields-a.md:57 promises, so normalisation would ship the defect; one concept, one discipline — 0326 anti-fork): `extractParsedParams` now runs a three-arm key gate — reserved-keyword (byte-identical) → NEW `theta/parse/params-key-not-identifier` on any cooked key that is not identifier-shaped → the 0149 case gate (byte-identical) — with quoted-but-identifier-shaped YAML keys staying LEGAL (YAML quoting is surface spelling; the deliberate divergence from 0154's separate quoted row is recorded). The message is the fixed string `params key must be an identifier` (no key embedded — the 0105-chain break-discipline law). DIAG-2 same-commit: the registry row, the reference mirror, and one frontmatter sentence closing the spec silence. Both render seams and the `as "WireName"` rename mechanism are byte-untouched; permitted-codes byte-identical `a4a8da04` (the new code fires on crafted fixtures only). Witnessed by `tests/b0380-params-key-not-identifier.test.ts` (12 cells: the block-scalar forgery carriers refuse, `"a b"` refuses, identifier/quoted-identifier/Topic-case/yaml-malformed/reserved controls byte-identical, both render seams covered both directions, registration outcome) and the NEW H9a acceptance cell `tests/live/acceptance/b0380-params-key-not-identifier-load-refusal.test.ts` (load refusal names the new code, the identifier control registers and drives — green under the lock). One enumerated-class cell flip WITHIN the ratified envelope, disclosed: `schema-field-name-case.test.ts` q2 (`"my topic"` — the 0149 §Non-goals row this report's §Related quotes as its origin) pinned the old acceptance and now asserts the refusal. Interaction recorded for open sibling 0384: the feeder is closed, so its raw-key-embedding messages co-fire with the new refusal — re-scope 0384 to the co-firing case post-merge.
...
- **Bug 0369** — control-flow constructs had no runtime kind discipline for values the static layer deferred on: `for`/`par for` over a laundered non-array silently iterated ZERO times (the theta reporting success with accumulators untouched), `if`/`while`/ternary/`&&`/`||` treated any laundered non-boolean as `false`, and unary `!` applied raw JS truthiness — so for `c = "x"`, `if c` and `if !c` BOTH skipped (two contradictory truthiness models at once). PARENT ADJUDICATION Route 1 (recorded: the 0324 clamp precedent is distinguished — `max` is a tuning clause with an intent-preserving degradation, iterand/condition positions have none since zero-iteration/false-steering IS the corruption; route 2 would fork the control-flow family from the operator family for one laundering mechanism, the 0326 anti-fork law): two plain defect classes — `ForIterandKindDefectError` (non-array reaching `executeFor`/`evalParFor`, thrown BEFORE worker scheduling so CTRL-5 never observes a fabricated empty fan-out) and `BooleanPositionKindDefectError` (non-boolean reaching `if`/`while`/ternary conditions or `&&`/`||`/`!` operands on EITHER host, including the pure host's `evaluatePureIf` found unbelted in review) — both routed through `surfaceUnexpectedThrow` → `theta/runtime/internal-error`, NO new registry rows; unary `!` stops JS-coercing entirely. The stale "deferred to the runtime safety net" comments now name the real belt; `control-flow.md` and `expressions.md` §Truthiness each gain the runtime-disposition sentence, with `!` added to the boolean-position list. Genuine-array iterands, all-boolean conditions/operands, CTRL-1 snapshot semantics, the parse gates' deferrals, and 0324's `par for max` clamp stay byte-identical. Witnessed by `tests/b0369-control-flow-kind-belts.test.ts` (34 cells: 17 flips red-at-fork across E/F rows on both hosts incl. the F6 both-false contradiction, 17 controls). One committed-cell flip under PARENT RATIFICATION (the 0292 (D)-row-v2 model): bug 0145's PIN f8 (`if match "hi" { x => x }` — the match-as-condition laundering vector both the doc's and the lane's census missed, disclosed) re-anchored to assert the loud belt abort with the kind-evidence regex pinning "got string" — the scope-inference subject is witnessed by the belt message itself (a broken inference would surface as unknown-identifier, never as a string reaching boolean position); red-direction proven by belt neutralisation; dated note on the 0145 doc. Live: `par-for-body-return-live-cell` ran green under the lock (the `evalParFor` admit branch on the live host).

## [0.349.0]

### Fixed

- **Bug 0366** — `array.join` on a laundered receiver silently JS-coerced non-string elements against the join row's explicit "no implicit type conversion in theta 1.0" clause: `f([1, 2])` bound `"1,2"`, schema objects rendered `"[object Object]"`, `null` elements rendered `""`, and nested arrays flattened — while the identical direct spelling is parse-refused `theta/parse/non-string-array-join`. The parse gate (0089/0127 dispositions kept: judge-and-refuse for provably-unresolvable elements, defer for withheld receivers/elements) now has its runtime backstop: `evaluateArrayMember`'s `join` arm walks the receiver before calling host join and throws the new `StdlibJoinElementDefectError` (routed through `surfaceUnexpectedThrow` → `theta/runtime/internal-error`, no new registry row) on any element that is not a JS primitive string — enum values (boxed carriers whose static type is never `string`) refuse rather than admitting their wire text, per the recorded adjudication. Bug 0315's arity belt in the same dispatcher stays untouched beside the new element walk (its arity-only pin honoured), the false parse-guarantee comment above the arm now states the real contract, all-string joins and the empty-array join stay byte-identical, and one sentence in `expressions.md`'s `join` row names the laundered-path runtime disposition. Witnessed by `tests/b0366-join-element-laundered-belt.test.ts` (11 cells: B1–B4 coercion flips red-at-fork plus enum-carrier and both-hosts rows, with parse-refusal/all-string/empty/arity-coexistence controls). One live-cell re-anchor under PARENT RATIFICATION (the 0292 (D)-row-v2 vehicle-collateral model): bug 0143's `withheld-binder-provenance-live-cell` CLEAN half swapped its carrier from an integer to a string element (`match "hi" { x => [x] }.join(",")`) — the withhold-provenance subject (a match-arm binder's element type is withheld, so the join gate defers) is preserved exactly, only the incidental integer coercion the belt now forbids is removed; the old carrier's would-red is proven offline by witness row B1, a dated note on the 0143 doc records the re-anchor, and the re-anchored cell ran green under the lock as the primary live witness (a withheld-element join reaching the belt boundary through a real drive).

## [0.348.0]

### Fixed

- **Bug 0368** — `+` and the four ordering operators had no runtime operand belt, so spec-refused pairings that reached the runtime through statically-withheld operands silently JS-coerced on both evaluation hosts: `f("x", 1)` bound `"x1"`, `f(null, 5)` bound `5`, `f(true, true)` bound `2`, `f([1], [2])` bound `"12"`, and cross-type ordering fabricated boolean verdicts (`g(true, 2)` under `<` → `true`) — while the identical laundering under `-` threw the bug-0332 belt. Both sinks now extend the settled belt discipline: `applyBinaryScalar` (executor) and `evaluateBinaryExpression` (pure host) require two numbers for `+` after the two-string arm and two-numbers-or-two-strings for `<`/`<=`/`>`/`>=`, throwing the new `BinaryMixedOperandError` routed through `surfaceUnexpectedThrow` → `theta/runtime/internal-error` (no new registry code, matching the 0332/0338 precedent), with the compound `+=` mirror covered; two-string concatenation, two-string ordering, and all-numeric pairs including `NaN`/`±Infinity` operands stay byte-identical, and the existing `-`/`*`/`/`/`%` belts are untouched. Witnessed by `tests/b0368-plus-ordering-laundered-belt.test.ts` (19 rows: ten flip rows red-at-fork naming the coerced value, controls for both sinks, the `+=` mirror, and the NaN/Infinity admissions). `tests/b0345-interpolation-operand-checks-at-parse.test.ts` PARITY (5c) — which pinned bug 0345's scope boundary ("deferred `+`/ordering still render, no belt covers them") — was re-anchored under PARENT RATIFICATION (the 0292 (D)-row-v2 vehicle-collateral model: forced by the adjudicated belt, subject-preserving — the deferral-parity subject lives in 5a, untouched; red-direction proven by belt neutralisation) to assert the loud framed abort, and a dated note on the 0345 doc records that its §Residuals-1 runtime concession is superseded on this surface. Live: zero live cells flip (census by grep; the line-pinned LPA not implicated); the `b0345-interpolation-operand-refusal` acceptance cell ran green under the lock as the adjacent live witness through a real `pi -p` session.

## [0.347.0]

### Fixed

- **Bug 0347** — a subagent-mode callee that `?`-propagated a NESTED `invoke_infra` of one of the five child-side-mintable causes (`load_failure` / `validation` / `return_validation` / `internal_error` / `subagent_model_preflight_mismatch`) reached the parent BARE — wrong kind (`invoke_infra` where the spec shape is `invoke_callee`), `callee_path` naming the grandchild the parent never invoked, no SLSH-5 hop — because the `theta_result` envelope carried no provenance marker and the driver read `cause` as a proxy over the closed union: the three no-child-writer causes tagged `callee-returned` (wrap), every other cause defaulted `boundary-minted` (bare), while the in-process leg wraps the identical propagated leaf for all causes — the INV-5 parity gap bug 0294's §Residuals item 1 deferred as 0342-scale. The envelope now carries the provenance the proxy stood in for: an OPTIONAL `err_provenance` sidecar (`"mint"` | `"propagated"`) on the `err` arm per the 0342 additive-envelope precedent — `v` stays 1, absent = the closed-set-proxy behaviour verbatim (old-child/new-parent and new-child/old-parent both degrade to the current disposition, both skew directions cell-proven), no new diagnostic code, permitted-codes byte-identical. The child stamps where provenance is known: the five boundary writers stamp `mint`, the sole body-outcome envelope write stamps `propagated`; the driver reads the marker when present (superseding `PROPAGATED_INVOKE_INFRA_CAUSES`) and retains the proxy fallback verbatim when absent — the three no-writer causes wrap byte-identically under both, and a genuine child-side mint of any of the five stays bare (the fix narrows the bare default to the mint case). INV-5 parity is now the oracle: identical surfaced `Err.kind` and SLSH-5 suffix whether the callee resolves in-process or as a subagent. PIC-59 documents the sidecar in the 0342 convention. Witnessed by `tests/b0347-subagent-leg-propagated-mintable-wrapped-unit.test.ts` (32 cells: mint→bare / propagated→wrap / absent→proxy per cause, skew both directions, the INV-5 oracle, the SLSH-5 hop) and the doc-enumerated growth of the b0294 (F) suite (extant bare-cause cells stamp `mint`; each gained a propagation sibling; `return_validation` gained both); `tests/subagent-return-depth-refusal.test.ts` row H — a real-spawn 3-level chain — flipped `e.message`→`e.inner.message` under PARENT RATIFICATION (the 0292 (D)-row-v2 model: forced by the fixed contract, subject-preserved, red-direction proven) and doubles as the offline end-to-end witness. The 0342 multi-hop live cell ran green under the lock in-lane as the envelope-compat live witness (recorded: a real depth-2 chain through the same serialize/parse module; no existing live cell pins the 3-level propagation shape — census verified — and the real-spawn offline row covers the flip).

## [0.346.0]

### Fixed

- **Bug 0322** — `CodeToolError.cause: "unknown_tool"` had no producer and the spec was internally split about it: the reference page flatly claimed the cause "is reachable only via the file-watcher reload path" while the frontmatter mechanism expansion's own description forecloses that path (a watcher rebuild that loses a Pi tool records `theta/load/unknown-tool` and refuses to register — uninvocable; in-flight invocations complete on strong references), and the one adjacent runtime seam — `#resolveToolCall`'s regime-inactive `tool === undefined` arm, self-documented "unreachable from a REGISTERED theta" — rejected with the execution-bound `UnknownHostToolError`, so if the safety net ever fired the author saw `cause: "execution"` ("tool's `execute()` threw") for a call that never dispatched anything. Adjudicated the COMBINED route — mint-at-the-seam plus the reachability correction: the regime-inactive snapshot miss now sets a pre-dispatch carrier (`unknownHostTool` field on `CodeSideToolCall`) that `runCodeSideToolCall` short-circuits into the new `unknown-tool-error` outcome kind lowering to `Err(CodeToolError { cause: "unknown_tool" })` with the human-visible message text unchanged and the same no-dispatch refusal surface (the outcome-switch consumer case mirrors the adjacent `arg-schema-error` arm — ratified at merge as exhaustiveness-driven mechanics within the granted envelope); the PIC-58 subagent-root-regime ladder branch is untouched and still throws `UnknownHostToolError` (class retained). The spec now tells one story: the reload path is intercepted at load, `unknown_tool` is the dispatch-time safety net's truthful answer with no theta-1.0 load-admitted reachable case (`reference/frontmatter.md` corrected; `tool-calls.md:27` parenthetical reconciled; the fields-b expansion names the seam; the `queryerror-variants.md` enum comment reworded — the CodeToolError section only, disjoint from 0321's edits above it). Neither load-refusal nor the strong-reference dispatch model weakens — the arm stays harness-only-reachable, but its name, `codeToolErrorCauses()`'s contract, and tool-calls.md's safety-net sentence are now true. Witnessed by `tests/b0322-unknown-tool-dispatch-safety-net.test.ts` (cause flip `execution`→`unknown_tool` red at fork with message/kind/tool_name pinned; known-tool control; cause-contract cell; regime-active ladder control); three test-comment citation reconciliations from the src insertions are comment-only. No live owed — registration and drive outcomes for load-admitted thetas unchanged (census recorded); permitted-codes byte-identical (the carrier is an existing QueryError variant, no new diagnostic code).

## [0.345.0]

### Fixed

- **Bug 0321** — `ModelToolError` (`kind: "model_tool"`) was a nine-variant closed-union member with no producer: both firing conditions the spec assigned it route elsewhere by design — a tool absent from the resolved callable set is answered with an `isError` tool-result under the in-loop feedback rule (the loop continues), and a Pi-adapter/transport failure while feeding a tool-result back folds through the off-session classifier (or PIC-51's prompt-mode mapping) to `TransportError` — so authors following the spec wrote dead `match` arms and the ERR-15 closed-set conformance claim was unfalsifiable for the member, while a mid-loop adapter failure presented under `transport`'s fields. Adjudicated Direction 2 — retire the firing claims, not mint the variant: post-0291 the transport fold treats every loop-round provider failure uniformly with evidence-derived `retryable`, and splitting rounds by "carried tool-results" would present the same network blip as different variants depending on loop phase (the same anti-fork law as 0326's clamp adjudication — no decision rule keeps the two variants disjoint because the underlying event class is identical), and the absent-tool condition is covered by the feedback rule the bug's own non-goals mark correct — no theta-1.0 input class remains. The member is now documented-reserved on the `last_tool_name | null` precedent: `queryerror-variants.md`'s firing sentence names both actual routes and marks the variant reserved (no theta-1.0-reachable case, kept for forward compatibility), `query-tool-loop.md` and `query-failure-and-repair.md` stop asserting producibility (proximate-list caveat + mid-loop reword), and five residual cross-references (`reference/errors-and-results.md`, `tool-calls.md`, `schema-subset.md`, `conversation-drive.md`, intro reconcile) plus two `tool-call.ts` comments are reconciled — comment/prose only. The union member, schema block, SNK-d renderer row, runtime-event kind, wire-kind entry, and `modelToolErrorKind()` all stay (ERR-15 forbids tightening; the renderer is correct as written for hand-built values). No src operand changes, no test flips, no live owed (registration and drive outcomes untouched); the locks are the existing suite and the spec-prose review, the 0309/0350 posture.
...
- **Bug 0327** — `ToolLoopExhaustedError.raw_response` was structurally always `null` on the untyped exhaustion path: the loop's exhaustion branch hardcoded `raw_response: null`, and the `FreePhaseTurn` `tool_use` arm carried no text slot at all, so the text the model emitted alongside the blocked terminal tool call — held by both production drivers (in the transcript on the prompt leg, in the parsed `AssistantMessage.content` text parts on the off-session leg) — never reached the field ERR-19 defines as carrying exactly that text; the field was dead in precisely the cases it exists for. The seam's `tool_use` arm now carries an optional text slot, both production drivers thread the terminal turn's accompanying text through it (`#exhaustionTurn` via the transcript read surface the prompt driver already owns; `#driveFreePhaseRound` mapping the final reply's text parts beside the toolCall parts), and the exhaustion branch passes it — `null` now means what the spec's biconditional says: exhaustion fired on a pure tool-use turn (parent adjudication: implement the obligation, no spec amendment; the null-always-intended alternative rejected). Success path, `rounds`/`last_tool_name`, the typed-query loop (verified to have no analogous seam hole — recorded), and the invoke surfaces are untouched; scripted-driver propagation is type-only via the optional field. Witnessed by `tests/b0327-untyped-exhaustion-raw-response.test.ts` (terminal-text threading, pure-tool-use null half, cap-0 composition with 0308, rounds/last_tool_name stability, success control) and `tests/b0327-off-session-exhaustion-raw-response.test.ts` (production off-session end-to-end), red at the pre-fix tree on the threading cells; the doc's narration-dependent live shape is stochastic by AGENTS.md's own rule, so the live obligation was discharged via the deterministic cap-0 exhaustion live cell plus an existing untyped tool-loop cell under the lock (recorded).

## [0.335.0]

### Fixed

- **Bug 0308** — the SNK-h note renderer substituted the literal `respond` for a null `last_tool_name` (`e.last_tool_name ?? "respond"`) on the strength of an "unreachable null" comment that the cap-0 path defeats: any untyped query under `tool_loop.max_rounds: 0` exhausts at initialisation before any tool call, so the user-facing SLSH-3 note read "(last tool: respond)" — naming the typed-query respond tool's reserved vocabulary for an untyped query that registered no such tool and called nothing, misleading triage toward the typed-query wire. Both `?? "respond"` sites are swept (the SNK-h arm now renders null through `summariseErrorField` like every other field — "(last tool: null)"; the `#exhaustionTurn` sibling gets a fail-loud guard in place of the dead fabrication), and ERR-19's false reachability sentences are corrected in the same commit to name the cap-0 untyped exhaustion as the reachable null case (three spec docs carried the premise; the extension beyond queryerror-variants.md was a recorded bounded self-authorization). Cap ≥ 1 notes with a real tool name are byte-identical; the theta-side `Err` value was already correct and is untouched. Three committed cells that pinned the fabricated rendering (with "no reachable case" comments) were flipped to the field's actual rendering as parent-ratified scaffolding of the corrected premise. Witnessed by `tests/b0308-snk-h-null-last-tool.test.ts` (renderer null/real-name cells + the scripted cap-0 reachability witness + the swept-sibling control) and live by `tests/live/hardening/b0308-cap0-exhaustion-note.test.ts` (zero-provider-turn cap-0 drive; note carries "after 0 rounds" and no "respond"), red-proven both directions under the lock.

## [0.334.0]

### Fixed

- **Bug 0300** — `theta/load/frontmatter-value-out-of-range` interpolated a string-valued `<observed>` with no line-break transform (`renderObserved`'s string arm returned `"${value}"` verbatim), so a `tool_loop.max_rounds` / `respond_repair.attempts` block scalar or `\n`-escaped string rendered a `message` of two or more physical lines — violating the single-line-summary rule and forging the serialised content format's reserved shapes (the `  hint:` continuation line, the blank-line batch separator) — the 0105/0250 defect class left open on the `<observed>` carve-out, while the settings-side twin already escaped via `JSON.stringify`. The string arm now renders through `JSON.stringify` (the parent-adjudicated convergence: escapes rather than collapses, preserving the value's identity — the byte-exact-assertion premise of the carve-out; the collapse alternative was rejected as forcing a change to the already-conformant settings renderer), and the placeholder-rendering carve-out sentence now states the JSON-escape handling for both out-of-range rows, resolving the spec self-contradiction with the single-line rule. The `"25"` worked example and every escape-free string render byte-identical; the numeric arm and the refusal itself are untouched. Witnessed by `tests/b0300-out-of-range-observed-string-single-line.test.ts` (escaped-string, block-scalar, `  hint:`-forgery, and blank-line-forgery rows one-line post-fix + `"25"`/numeric/settings-twin controls + a no-break sweep), red at the pre-fix tree on the multi-line rows.

## [0.333.0]

### Fixed

- **Bug 0292** — `ValidationError.validation_errors` was emitted in AJV's native error order (schema-declaration/`required`-array order) instead of the ERR-14 canonical (path, schema_keyword, message) sort: `validateAgainst` mapped `result.errors` positionally and no site on the typed-query path called the shipped `orderValidationIssues`, so `validation_errors[0]` — spec-defined as "well-defined (the canonically-first issue)" — was whatever AJV emitted first, and within one failed typed query the follow-up turn's `<ajv-summary>` listed issues canonically while the terminal `Err` carried them in another order. `validateAgainst` now returns `orderValidationIssues(issues)` — the sort applied at exactly the mapping site ERR-14 names — making the follow-up renderer's own sort an idempotent no-op re-sort; the binder path, `<ajv-summary>` renderer, and ERR-17 synthesised-issue arm are byte-identical. The lane's premeasure falsified the doc's "no committed cell pins the raw order" claim: one cell (`(D) row v2` in the quoted-field-name refusal suite, from bug 0176's fix) pinned the raw order and was parent-ratified to flip to canonical as vehicle-collateral (the issue SET was the subject; the order was scaffolding). Witnessed by `tests/b0292-validation-errors-canonical-order.test.ts` (two-required and two-type-error canonical arrays byte-for-byte, single-issue control, `<ajv-summary>` stability, determinism repeat), red at fork on the order-divergent cells.

## [0.332.0]

### Fixed

- **Bug 0301** — three recognised-field value shapes silently took defaults with zero diagnostics: a non-boolean `bind_echo:` (`no`, `"false"`, `0`, bare) left echo ON against a written suppression; a non-mapping `tool_loop:` / `respond_repair:` value (`tool_loop: 5`, `respond_repair: none`) discarded the author's declared cap or methodology in favour of the default budgets (25 rounds; 3×validator_error); and a typo'd sub-key inside either block (`max_round:`, `methodolgy:`) was invisible to the unknown-key warning that top-level keys get. Spec-first, the three dispositions are now pinned (frontmatter field-contract rows + the defaulting-note's bind_echo row + reference mirrors), then enforced: (a) `bind_echo:` outside literal `true`/`false` → NEW `theta/load/unknown-bind-echo-value` load error mirroring `unknown-bind-context-value` (no truth-coercion — `"false"` refuses rather than coerces; a warning was rejected: every closed-set sibling refuses, and the harmful direction is echo-against-suppression); (b) a present non-mapping `tool_loop:` / `respond_repair:` → NEW `theta/load/malformed-tool-loop-field` / `theta/load/malformed-respond-repair-field` refusals per the 0104 malformed-tools-field shape, rendering the observed node's kind token (`{}`, bare/null, and absent stay silent per the equivalence sentence — the bare-block reading recorded as bounded self-adjudication, control-locked); (c) unrecognised sub-keys inside the two blocks → the EXISTING `theta/load/unknown-frontmatter-field` warning with the dotted key (`tool_loop.max_round`), theta still registered — the top-level forward-compat posture one level down. The five doc controls keep their exact codes byte-identical; three new DIAG-2 registry rows + mirrors; the kind-token except-clause extended once more. Witnessed by `tests/b0301-bind-echo-tool-loop-respond-repair-holes.test.ts` (20 cells: 9 red at the pre-fix tree, 8 controls, 3 DIAG-4 anchors); live-locked by `tests/live/acceptance/b0301live-bind-echo-nonboolean-load-refusal.test.ts` (offender refused, control drives).

## [0.331.0]

### Fixed

- **Bug 0299** — a value-less `description:` or `system:` (YAML null scalar: bare key, `null`, or `~`) was stringified by `String(value)` into the four characters `null` and threaded verbatim to three surfaces with zero diagnostics: the slash-command autocomplete description, the binder grounding line `Description: null`, and the subagent child's system prompt (`--system-prompt "\nnull"`) — while the sibling `argument-hint:` arm's guard mapped the same shape to absent, and the EMPTIER spelling (`description: ""`) was correctly dropped. Both arms now map the null scalar to absent via the minimal `isScalar(v) && v.value !== null` guard: `description:` bare/`null`/`~` records absent (the binder omits the `Description:` line per spec); `system:` null records the zero-part template byte-identical to `system: ""` (the bounded reconciliation keeping the zero-diagnostics constraint — `undefined` would have tripped the 0298 malformed-system-field refusal). Numeric/boolean coercion STAYS at both arms (`description: 42` → `"42"`; non-null non-string scalars are the doc's recorded non-goal). The `argument-hint:` control, the `""` controls, and the 0209 witness are byte-identical. Witnessed by `tests/b0299-null-scalar-description-system-absent.test.ts` (12 rows: 7 red at the pre-fix tree, 5 controls).

## [0.330.0]

### Fixed

- **Bug 0297** — a present non-scalar `bind_context:` (`[session]`, a block sequence) recorded the absent-field value, skipped the unknown-value arm, and the theta REGISTERED SILENTLY with the default `none` — the binder ran without the session context the author declared, against the registry Trigger's own words ("present but its value is neither `none` nor `session` … The theta is not registered"); the sibling `bind_model:` arm silently took the `theta.binderModel` settings fallback the spec reserves for the ABSENT field. The `bind_context` arm now records presence and routes a present non-scalar to `theta/load/unknown-bind-context-value` with the bounded kind-token rendering (the 0296 except-clause extended to name both codes); the `bind_model` arm records a present-but-unresolvable marker — no fabricated string — threaded through the existing `binder-model-unresolved` machinery exactly as an unresolvable declared string (`resolveChainReference` returns null before the settings fallback; the bypass-eligible path keeps its existing present-but-unresolvable disposition, recorded). Scalar controls (`banana`, bare-null) byte-identical; registration refused on the bind_context refusal. Witnessed by `tests/b0297-bind-context-bind-model-nonscalar.test.ts` (12 cells) + `tests/b0297-bind-model-nonscalar-production-load.test.ts` (3 cells through the real compose pass), red at the pre-fix tree; live-locked by `tests/live/acceptance/b0297live-bind-context-nonscalar-load-refusal.test.ts` (offender refused registration, control registers and drives).

## [0.329.0]

### Fixed

- **Bug 0296** — a present `mode:` whose value was a YAML sequence, mapping, or alias collapsed to the absent-key representation before any validation arm ran, so the file drew `theta/load/missing-mode` ("frontmatter is missing required field 'mode:'") — a diagnostic the author can refute by looking at their own source, whose registry Hint ("Add `mode: prompt` …") directs them to add a duplicate key that YAML then rejects. The `mode` arm now records a `modePresent` flag, and the required-mode arm keys on it: a present non-scalar routes to `theta/load/unknown-mode-value` (whose registry Trigger — "present but its value is neither `prompt` nor `subagent`" — already covers it; no new code), rendering `<value>` as the bounded kind token (`array` for a sequence, `object` for a mapping or alias, `null` otherwise) per a new except-clause in the placeholder-rendering spec mirroring the `invalid-derived-tool-name` model. `missing-mode` now fires only when the key is genuinely absent; the null-scalar (`unknown 'mode:' value 'null'`) and unrecognised-scalar controls are byte-identical; registration is still refused. Witnessed by `tests/b0296-mode-nonscalar-value-collapse.test.ts` (9 cells), red at fork on the three collapse rows.
...
- **Bug 0294** — a callee-returned `Err` whose `kind` was `invoke_infra` passed the parent's XMODE-1 wrap UNWRAPPED (`runInvokeEffect`'s kind-exemption presumed every such `Err` trampoline-minted), so a grandchild's infra failure `?`-propagated by the callee reached the grandparent as the callee's OWN failure: wrong kind (`invoke_infra` where the documented "my callee failed" shape is `InvokeCalleeError`), a `callee_path` naming a file the parent never invoked, and no SLSH-5 hop suffix (the provenance ledger records hops only against `invoke_callee` wrappers) — a two-hop cascade rendered byte-identical notes for both thetas. The boundary now discriminates by PROVENANCE, not kind: `InvokeChild.drive()` returns a source-discriminated `DrivenInvokeResult` (`callee-returned` | `boundary-minted`) — the in-process body outcomes mark callee-returned, `runInvokeChild`'s catch and the parent-side fail-closed envelope maps mark boundary-minted — and the wrap keys on `source`, so a propagated `invoke_infra` gains its `InvokeCalleeError` wrapper and its ledger hop ("each invoke hop adds exactly one wrapper") while this hop's trampoline-minted infra errors stay bare (per-cause table Panic row byte-identical, fence-pinned). On the subagent leg the doc's premise ("the envelope err arm is by construction the callee's top-level Err") proved factually imprecise — the child root drive mints five `invoke_infra` causes THROUGH that arm — so the envelope is tagged by a per-cause closed set: the three no-child-side-writer causes (`parse_failure`, `panic`, `subagent_model_unresolved`) are provably callee-returned and wrap; the five child-side-mintable causes stay bare (provenance-ambiguous on the wire; a future cause defaults boundary-minted — over-bare beats a phantom-hop spec violation); the remaining INV-5 parity gap needs a wire provenance sidecar and is filed as bug 0347. The `cancelled` disjunct stays kind-based verbatim (bug 0295's subject, fence-pinned); the `invoke_callee` recursive wrap and bug 0293's cause partition are untouched. Witnessed by `tests/b0294-callee-propagated-invoke-infra-wrapped.test.ts` (two/three-hop wrap + panic/one-hop bare controls through the shipped composition root), `tests/b0294-callee-propagated-invoke-infra-wrapped-unit.test.ts` (subagent-leg per-cause tagging + cancelled fence), and the live cell `tests/live/b0294-callee-propagated-invoke-infra-live-cell.test.ts` (real two-hop cascade, red-provable).
