# Bug 0340 — An unannotated binding initialised from a literal records the LITERAL type, so every later write of a primitive-typed value into it is refused with a message that renders both sides identically: `let mut a = ""` / `a = s` (`s: string`) reports `theta/parse/reassign-rhs-type-mismatch: reassignment of 'a' type mismatch: expected string, got string`, the theta does not register, and the same asymmetry swallows TYPE-2's narrowing route
...
annotate every `let mut` — correct, and the reason the defect is easy to route
around once seen and impossible to diagnose before.
...
- **What shipped:**
  ...
  - `tests/b0340-inferred-literal-binding-refuses-primitive-rhs.test.ts` — the
...
- **Gates:**
  - Witness: `npx vitest run tests/b0340-…test.ts` → 20/20 pass. Red direction
...
- **Live cell:** NOT shipped, and the reason is environmental, not a judgement
  that none is owed. This fix does move a registration surface (the offender
  theta stops being refused at load), which is the corpus's usual trigger for
  an H9a cell. The live suite cannot run here — `requireLiveHost()` fails loudly
  with "no live provider/model configured (ModelRegistry.getAvailable() is
  empty)", confirmed by running `tests/live/acceptance/b0314live-compound-assign.test.ts`
  — and an unrun acceptance cell is worth less than an honest gap, since
  AGENTS.md requires proving both directions of any new assertion. The
  registration delta is instead witnessed offline in group E through the
  clause-for-clause mirror of `hasLoadParseError`. A maintainer with a
  configured provider should add `tests/live/acceptance/b0340live-…` (offender
  registers and drives; C1's byte-neighbour still refuses).
- **Residuals:**
