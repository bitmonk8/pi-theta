...
- What shipped: mechanical re-pin (comment/message-string only; no verdict/assertion logic) of the enumerated stale `grammar.md:N` cites in three committed test files, per the 0389 +9/+5 shift. `tests/fn-param-not-identifier.test.ts` — `grammar.md:139` -> `:144` at the two `FnParams` sites (comment + assertion message). `tests/ctor-field-type-check.test.ts` — `:216` -> `:225`, `:220` -> `:229` (comment), `grammar.md:216-221` -> `:225-230` (comment), `:220` -> `:229` (comment), and the `grammar.md:220` assertion message -> `:229`. `tests/nested-array-element-sink-descent.test.ts` — the `:216` sink anchor -> `:225`, and eight `grammar.md:221` recursive-descent cites -> `:230`. The already-correct cites 0389 re-pinned (ctor `grammar.md:225-230`, nested `grammar.md:230`) and the reference-side (`docs/reference/grammar.md:300`) and `expressions.md` cites are untouched per Non-goals.
- Gates: witness `tests/b0405-grammar-cite-sweep-gate.test.ts` 5/5 (SPEC-TRUTH cells A1/A2 green both directions; RE-PIN cells B1/B2/B3 RED at fork -> green after); the three target files verdicts unchanged (80/80); corpus citation gate `tests/citation-symbol-form-gate.test.ts` 3/3; full default suite 573 files / 10447 tests green; `npm run typecheck` clean; `npm run lint` clean.
- Review: 2 rounds. Round 1 (bug-fix-reviewer) — 1 blocking correctness finding F1 (a same-class stale cite `grammar.md:140` at fn-param-not-identifier.test.ts:416, in an assertion message, that the doc grep-enumeration missed; `FnParam ::=` moved 140 -> 145 in the same 0389 block) + prose R1. Round 2 (bug-fix-reviewer-fast) — CLEAN, no findings.
- Verification: bug-fix-verifier verdict SOLID. Revert-witness — reverting a `:144` reds cell B1, reverting the F1 `:145` reds cell B1 on the FnParam lock, reverting a ctor `:229` reds cell B2, reverting a nested `:230` reds cell B3; all restore byte-exact (numstat 3/3, 4/4, 9/9, 0 EOL churn). Every re-pinned number confirmed to name its cited grammar.md content (144=FnParams, 145=FnParam, 225=sink-exhaustive, 229=constructor-field bullet, 230=recursive-descent bullet); no stale cite remains. Citation gate green (no bare `:N` in the witness). Full suite 573/10447 green. Typecheck + lint clean. Live: the orchestrator ran the adjacent parse-refusal-at-load registration cell `tests/live/b0367live-null-left-minus-refusal-live-cell.test.ts` green (WHY: test-comment-only sweep, zero code/spec/behaviour change; the cell witnesses the theta/parse/* refusal-at-load registration path is unaffected).
- Residuals: 1. b0366 (0394 Residuals 3) NOT folded in — on inspection its header quote is a SEMANTIC drift, not a citation re-pin: `tests/b0366-join-element-laundered-belt.test.ts:22` quotes a pre-0394 `stdlib-string.ts` design-brief sentence ("the belt does not consult `params`") that 0394 REVERSED (`src/stdlib/stdlib-string.ts:64-66` now states the dispatcher belt reads `params` too, arity and kind); re-quoting rewrites a claim's meaning, outside a mechanical citation sweep and outside this lane's STOP-on-meaning bound. Follow-on. 2. b0345 (0395 Residuals 1 live half) NOT folded in — the comment-only `type-layer-checks.ts:3092/3094/3103` cites are a different (0392/0395) shift whose §Related remedy is 0134 symbol-form conversion (not a +9 grammar re-pin) and are outside §Affected's enumeration; the dispatch scoped this lane to the 0389 +9 grammar sweep. Follow-on. 3. The optional "add the bare spelling to a citation-form gate" was delivered as the committed content-anchored witness `tests/b0405-grammar-cite-sweep-gate.test.ts` (locks the re-pin targets by content so a future grammar shift reds here first), rather than by extending an existing gate.
- Discharge notes appended: none.
- Pinned dispositions / non-goals: honoured — the LPA line-pin (`tests/live/live-production-acceptance.test.ts`, 14864) was not touched (no LPA cite in the enumerated set); era-pinned `docs/bugs/*` citations frozen; reference-side and `expressions.md` cites left alone; no assertion weakened (every edit is comment/message-string only). Deviation recorded (bounded self-authorization): F1 re-pinned one un-enumerated same-class cite `grammar.md:140` -> `:145` at fn-param-not-identifier.test.ts:416 — citation/message-string only, provably bounded (one number), verified against spec truth (grammar.md:145 = `FnParam ::=`), with the witness B1 strengthened to lock it; three independent evidence sources (spec-line read; fn-param:16 already cites `:145` for this block; 0405 §Why-it-matters targets stale assertion-message cites) recorded in the fix report.
