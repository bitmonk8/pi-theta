...
- What shipped:
  - `src/parser/frontmatter.ts` — `toSystemParamType`: inline object types are parsed via the inline-object machinery (`isSingleEnclosingBraceGroup` + `splitTopLevel`/`topLevelColon`, mirroring `hoistInlineObjectType`) into real `fields` maps (§Fix a, 0406 i); a body `schema` builds a lazily-cyclic object shell via a per-param `resolving: Map` so a self-referential field re-expands rather than degrading to `string` (§Fix a, 0406 iii); an imported `.thetalib` symbol classifies as the new `opaque-object` kind (0406 ii); the inline-object structural check runs BEFORE the generic/union split so `{a: string \| null}` stays one inline object (round-1 F1).
  - `src/parser/system-interpolation.ts` — new `opaque-object` `SystemParamType` arm (admits any `.Ident` chain at parse, refuses nothing); path parts carry a `valueDriven` marker; `renderSystemPrompt` derives the canonical row from the resolved value's runtime kind (`interpolationTypeOfValue`, mirroring the producer's `interpolationTypeOf`) for `opaque-object`/`discriminated-union` terminals; `resolvePath` guards null/undefined intermediates (round-1 F3).
  - `src/parser/theta-document.ts` — a comment corrected (a `FrontmatterBodyTypes.schemas` rename-loss claim made false by the `wireName` addition).
- Doc-was-wrong-in-part (0362 pattern): §Fix option (a)'s inner-ordering sub-decision assumed one of two import-resolution orderings was reachable. Premeasure E1–E5 (recorded in `.pi/tmp/fixes/0406-report.md`) FALSIFIED both routes — the registry `Phase = parse` pin (`code-registry-parse.md:128–131`) plus the synchronous, `FileSystem`-free parser (`theta-document.ts:973`, `ParseThetaDocumentDeps` carries only `{ systemNote, modelMatcher }`) block BOTH moving `checkSystemInterpolation` after import resolution AND a post-import two-pass template revalidation. Parent adjudication (Rec A) superseded the sub-decision: imported schemas classify as `opaque-object` (admit at parse, no field-carry), and the imported-field-typo case is a DOCUMENTED RESIDUAL (below), not fixed here.
- Gates: witnesses `tests/b0406-*.test.ts` 9/9 green (+ 0407 3/3, 0408 7/7 = 19/19), each red at fork with the doc's symptom code (`theta/parse/system-interp-bad-field` / `[object Object]`), revert-witnessed byte-exact; full suite `npm test` 573 files / 10450 tests green; `npm run typecheck` clean; `npm run lint` clean; LIVE `tests/live/acceptance/b0406live-object-param-system-interp-registration.test.ts` green (prober answers 877 — the interpolated inline-object field `${cfg.addend}`=277 reaches the spawned child's `--system-prompt`) and revert-red at fork (token-free at the offline attribution guard, `system-interp-bad-field`).
- Review: 2 rounds. R1 (`bug-fix-reviewer`, deep): 4 findings — F1 inline-object-with-interior-`|` misclassified as discriminated-union (blocker), F2 nested-sidecar wire-key collision silent-wrong (correctness), F3 `resolvePath` TypeError on admitted null/absent intermediates (blocker), F4 inline/container nested renames (residual). F1/F2/F3 fixed (`bug-fix-fixer`); F4 recorded as a residual. R2 (`bug-fix-reviewer-fast`): CLEAN, no findings.
- Verification: `bug-fix-verifier` SOLID — (1) revert-red / restore-green byte-exact (blob hashes verified); (2) full suite green; (3) live green + fork-red (orchestrator-run, folded cell); (4) typecheck + lint clean.
- Residuals:
  ...
  3. Nested body-schema field renames inside a container / inline-object param are not translated on a bare-container render (`buildOutboundSidecars` is root-flat only; round-1 F2 resolution) — renders theta-side, never a WRONG wire name. Filing candidate (0407-adjacent).
  4. An alias / head-only body schema param (`schema B = Cat \| Dog`; `p: B`) keeps the permissive `string` terminal, so a bare `${p}` still renders `[object Object]` — unchanged from the fork, outside Rec A scope (round-1 R3). Filing candidate.
- Discharge notes appended: none (0407 and 0408 carry their own records).
- Pinned dispositions / non-goals: no new diagnostic code; no spec-phase amendment; the 0041/0059 `params-type-not-expression` refused cascade untouched; genuinely-unresolved atoms keep the permissive `string` fallback; `discriminated-union` `.Ident` descent still refused at parse; zero hunks in `production-theta-producer.ts`.
