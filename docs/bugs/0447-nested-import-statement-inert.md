...
## Fix (0.452.0)
- What shipped:
  - `src/parser/theta-document.ts` â extended bug 0446's recursive statement-placement walk (renamed `checkExportPlacement`â`checkStatementPlacement`, `walk*ForExportPlacement`â`walk*ForStatementPlacement`; mechanical, export behaviour byte-identical) with one arm: a from-bearing `import` (`stmt.path !== ""`) at any non-top-level position â `if`/`while`/`for`/`fn` body, `par for` body, block-expression, or `subagent fn` `with`-clause value â draws `theta/parse/import-not-top-level` in EITHER host. No host split (a nested import is illegal in a `.theta` and a `.thetalib` alike); a TOP-LEVEL import stays legal in both. The `stmt.path !== ""` guard keeps the from-less nested import inside `theta/parse/import-missing-from-clause`'s shape lane (no co-fire). Â§Fix Option 1.
  - `src/parser/imports.ts` â minted `IMPORT_NOT_TOP_LEVEL_CODE` / `_MESSAGE` / `_HINT` (one mint, no host-split pair â the 0431/0446 shape; reuse-vs-mint ratified MINT: no existing placement Trigger honestly covers a nested import, 0326).
  - `docs/spec_topics/diagnostics/code-registry-parse.md` + `docs/reference/diagnostics.md` â the DIAG-2 registry row for `import-not-top-level` and its mirror (Message byte-identical to the impl constant, DIAG-4).
  - `docs/spec_topics/imports.md` â an **Import placement** paragraph dispositioning the nested import in both hosts.
- Gates: witness `tests/b0447-nested-import-inert.test.ts` 8/8 green (M1/M2/M3 + `while`/`par for` breadth cells; top-level Ã2 + M4 shape co-fire controls; revert-proven â neutralising the import arm reds the 5 import cells while bug 0446's 11 export cells stay green, proving arm isolation; byte-exact restore confirmed by `git hash-object`); full default suite 611/611 files / 10686 tests green (both fixes landed); `npm run typecheck` clean; `npm run lint` clean; DIAG-2 closed set green.
- Review: 1 round. R1 (`bug-fix-reviewer`, deep): CLEAN on correctness/fidelity/spec â the import arm rides bug 0446's already-validated complete descent, DIAG-2/DIAG-4 parity confirmed byte-exact, the rename proven behaviour-preserving (b0446 11/11 + b0431 5/5 green), M2's identifier-root seeding statically unaltered. Findings were comment/prose/test only: F1 (a stale `parseStatement:2899â2900` citation in the witness header â the symbol is `parseForm` and the line shifted), R2 (the walk default-arm comment said âbug 0446's seamâ after 0447 folded in), R3 (bug 0446's fix record named the pre-rename walk symbols). All three resolved as a bounded citation/comment/prose-only correction (no assertion or behaviour touched; gates re-run green; recorded); no confirmation review round required (polish verified by gate-diff, no executable line touched).
- Verification: VERIFIED â witness revert-proven both directions (byte-exact restore, `git hash-object` match); full suite 611/611 green; typecheck + lint clean; DIAG-2 closed set green; live `tests/live/acceptance/b0428live-unreadable-thetalib-load-refusal.test.ts` 1/1 green under the global lock. WHY the adjacent cell discharges the obligation: the refusal is a parse class, offline-equivalent (no model participates), so the adjacent import/export-intake load-refusal cell proves the intake path registers and drives end-to-end through the real `pi -p` â the identical discharge 0431/0446 took for the same code family.
- Residuals: (1) an `import â¦ from` hidden inside a `@`...`` query-template `${â¦}` interpolation's `par for` body remains inert â the interpolation is re-lexed into a separate throwaway AST (`parseInterpolationSource`) the document-body placement walk does not own; the import sibling of bug 0446's residual (1), the same distinct mechanism, deserving the same joint follow-up filing. (2) M2's `unknown-identifier` co-fire is unchanged: a nested import binds nothing, so a use of the imported name still draws `theta/parse/unknown-identifier` beside the new placement refusal â the fix adds the fault-naming code, it does not alter identifier-root seeding (Â§Fix Option 1 kept the binding behaviour).
- Discharge notes appended: none.
- Pinned dispositions / non-goals: Options 2 (spec-pin inertness) and 3 (bind nested imports for real) not taken. GOV-15 never-conformant standing â a nested import was never spec-legal, so the DIAG-2 registry carve-out covers the mint and NO permitted-codes move is owed (permitted-codes.json holds no parse codes). The top-level import pipeline, `import-static-checks.ts` (L4-owned), and the runtime no-op arm are untouched.
