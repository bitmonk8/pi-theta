...
This is the mental model. It explains *what* a loom is and *why* it is shaped the
way it is: how code interleaves with model-directed text, why evaluation appends
turns to a conversation, what the two execution modes mean, how the `.loom` /
`.warp` split works, what the final value is, and how an evaluation ends. It
...
looms; complete, runnable examples live under `docs/examples/` and are cited from
the Tutorial and How-to guides.

## Where a loom sits: the code/model boundary
...
- **`.loom` files** sit on the boundary. Deterministic code (variables, loops,
...
parse a response, or return a typed value to a programmatic caller. A loom is a
scripting language for exactly those cases. Its side effects are *conversational
injections* — the text a loom emits becomes turns in a conversation with the
model.

## A loom is a program, not a template

A `.loom` file does not expand to a single prompt. It is a small program that
...
1. sends its rendered text as the next user turn into the loom's target
   conversation,
...
until the model reports it is satisfied. The loom alternates between code (parse,
...
Evaluating a loom appends turns to a conversation — one turn per query, in the
order the code issues them. This is the loom's primary effect. When an evaluation
ends, the turns it has already appended stay in the driven conversation; the
runtime performs no implicit rollback of a partially-run loom. Which conversation
those turns land in is decided by the loom's mode.

## Two modes: prompt vs. subagent

Every loom declares its own execution mode in the required `mode:` frontmatter
...
all see the same mode for a given loom.
...
- In **prompt mode**, each query runs as a turn in the *caller's current*
  ...
  turn is user-visible and nothing is hidden. The loom's final return value is
...
- In **subagent mode**, a fresh, isolated conversation is spawned for the loom.
  Each query runs as a turn in it. When the loom finishes, only its return value
  propagates back to the caller; the intermediate transcript is private and is
  not retained after the loom returns.

The mode selects *which* conversation the turns happen against — not whether the
loom is allowed to round-trip with the model. Both modes drive a conversation
...
The two modes compose. A loom can invoke another loom with `invoke(...)`, and the
...
the loom's tail expression, or — if an explicit `return expr` executes first —
...
- **Fail** — the loom returned an unhandled `Err`, panicked, or breached a
...
- **Cancelled** — the loom's abort signal fired and a checkpoint observed it.
...
A loom is stored in one of two file extensions that share a single grammar and
type system:

- **`.loom`** files are invocable — discovered as slash commands, targeted by
  `invoke(...)`, and registrable in another loom's `tools:`. A `.loom` file
  declares a `mode:` and runs.
- **`.warp`** files are library modules. Their top level is restricted to
  declaration forms — `import`, `export`, `fn`, `schema`, `enum` — with no
  top-level statements or queries. A `.warp` file is never discovered as a slash
...
callable surface and reusable building blocks. Code inside a `.warp` `fn` body
uses the full language, including queries; when such a function is called, its
queries execute against the *calling* `.loom`'s conversation. Import paths are
relative and must end in `.warp`; the resolution and re-export rules are in the
[grammar reference](./reference/grammar.md#source-files).

## What a loom cannot do

The Loom language has no file-writing, network, or process-spawning primitive.
Every external effect a loom produces flows through one of three surfaces: a query
against the model, a tool call, or a child loom invocation. The set of tools the
model and loom code can reach is the loom's **callable set** — the entries listed
under the `tools:` frontmatter field. If `tools:` is omitted the loom runs with an
...
host-process sandbox, and any tool a loom admits exposes that tool's full
...
- Query-and-await ("a loom is a program, not a template", the query expression
...
- Prompt vs. subagent mode, mode as a per-file author choice, final value not
  ...
  `docs/spec_topics/overview.md` §"Scope of a loom file",
...
- Cancellation checkpoints, downward-only propagation, no retroactive rewrite of a
  ...
  CNCL-5).
- `.loom` vs. `.warp`, shared grammar/type system, five permitted `.warp`
  top-level forms, warp `fn` queries running against the caller's conversation:
...
- Terminology (`Loom`, `.loom`, `.warp`, *prompt mode* / *subagent mode*,
