...
`provider` derivation: the `provider` field is an `api`-shaped [`Model<Api>.api`](../pi-integration-contract/host-interfaces-core.md#model-registry-pin) value — the same `api`-shaped field the [Provider error mapping](../pi-integration-contract/provider-error-mapping.md#provider-error-mapping) table is keyed on (e.g. `anthropic-messages`, `openai-completions`), not a short provider-id form such as `"openai"`. Its value space is the externally-owned `Api` union declared in `@earendil-works/pi-ai`; theta does not redefine that union here. The `Api` value's source depends on which surface synthesised the error, because theta cannot always observe the model a failing turn routed through: failures whose model theta itself resolved (the binder pass and the typed-query forced respond turn, both off-session through `complete()`; the subagent-mode driven turn, whose spawned session runs the theta-resolved model; and the load/dispatch-time unsupported-provider synthesis) take that resolved model's `.api`, whereas prompt-mode driven-turn failures take the `.api` of the model the turn was driven under — the theta-resolved `model:` when the turn ran inside a [PIC-17 model window](../pi-integration-contract/tool-registration-lifetime.md#pic-17-model-window), else the user session's selected-model `.api` (`ctx.model`) — substituting the fixed sentinel `"unknown"` when neither is defined, per [prompt-mode error detection](../pi-integration-contract/conversation-drive.md#prompt-mode-error-detection).

Reserved: no theta 1.0-reachable input produces `ModelToolError`. The two adapter-layer conditions once assigned to it both route elsewhere — the named tool being absent from the resolved callable set is answered by the in-loop feedback rule below (fed back as an `isError` tool-result, the loop continuing), and a turn-level Pi-adapter / transport failure of the model's tool-call loop classifies as `TransportError` through the off-session provider fold (see [Query — Tool calls during a query](../query.md)). The variant, its schema, its SNK-d note row, and its wire `kind` are retained for forward compatibility and hand-built values, per ERR-15's bar on tightening the closed set. An in-loop tool failure the runtime can lower to a tool-result — `execute()` throwing or resolving `{ content, isError: true }` — does **not** fire this variant: it is fed back to the model as that `tool_use` block's result and the loop continues. A non-conforming `{ content, isError }` envelope is routed off this surface through `theta/runtime/internal-error`, symmetric to the code-side rule in [Tool Calls — Failures](../tool-calls.md).
...
schema ToolLoopExhaustedError {
  ...
  last_tool_name: string | null,             // most recent tool the model called on the loop's terminal free-phase round; the typed-query forced respond turn never reaches this exhaustion path (see Query — Tool-call loop bound), but an untyped query under `tool_loop.max_rounds: 0` exhausts at initialisation (slot_count == max_rounds, 0 == 0) before any tool call, so the `| null` branch is reachable there
...
`ToolLoopExhaustedError` is distinct from `CancelledError`: the former is the runtime giving up on the model; the latter is the user or parent giving up on the runtime. `last_tool_name` names the tool the model invoked on the loop's terminal free-phase round. The typed-query forced respond turn does not reach this exhaustion path — it is the exempt-routed terminator dispatched by CIO-4's `max_rounds`-final branch per [Hard Runtime Ceilings — CIO-4](../hard-ceilings/ceilings-3-and-4.md#cio-4), and forced-respond non-compliance routes through the `validation` / `schema_validation` path per [Query — Forced respond turn non-compliance](../query/query-failure-and-repair.md#forced-respond-turn-non-compliance); the `| null` branch is reachable for an untyped query under `tool_loop.max_rounds: 0`, which exhausts at initialisation before any tool call exists to name.
