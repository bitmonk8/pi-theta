...
The `from` clause is part of both productions. A specifier list with no `from`
clause — the bare keyword (`import`, `export`), an empty list (`import {}`,
...
The no-implicit-shadowing posture extends one level in, at load time: when a directly-imported entry's `as` alias equals the SOURCE name of a DIFFERENT declaration reached in that entry's own same-lib closure — the entry transitively references a same-lib sibling that happens to spell the entry's own alias — the load pass refuses with `theta/load/imported-type-name-collision` rather than letting the alias silently claim the flat `$defs` name and drop the sibling; the same refusal covers two separate `import` specifiers whose transitive closures reach one name through two structurally different declarations (a true diamond — the identical declaration reached twice — stays exempt, exactly as above).
