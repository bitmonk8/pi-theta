...
**Reassignment** is a statement, never an expression. The plain form and the compound forms `+=`, `-=`, `*=`, `/=`, `%=` are all legal on `let mut` bindings; the RHS must be compatible with the binding's declared or inferred type per [Type System — Type compatibility](./type-system.md#type-compatibility), reported as `theta/parse/reassign-rhs-type-mismatch` (or `theta/parse/integer-narrowing` for the one-way `number`-under-`integer` case; see [Diagnostics — code registry](./diagnostics/code-registry-parse.md#code-registry)). The same compatibility relation governs the initialiser of every `let` (typed or inferred) and is the canonical referent of every "same rules as `let`" cross-link elsewhere in the spec. <a id="reassignment-binding-type"></a>A reassignment does not change the binding's type: every later reference resolves the type the binding was declared or inferred with, for the whole of the binding's scope. <a id="inferred-binding-type"></a>An unannotated binding is inferred with the type its initialiser has **in expression position**, so a literal initialiser infers the primitive that literal types as under [TYPE-3](./type-system.md#type-3), never the literal type itself: `let mut count = 0` is an `integer` binding and `let mut name = ""` a `string` one, each accepting every later write its primitive admits.

<a id="compound-assignment-desugar"></a>Each compound form is defined by desugaring: `x <op>= e` computes the same as `x = x <op> e`, so a compound assignment carries the same operand-type rule as the spelled binary it stands for. `+=` therefore admits [the `+` operator](./expressions.md#object-construction-array-construction-and-operator-rules)'s two shapes — string-plus-string concatenation and number-plus-number addition — and refuses every other operand pairing (mixed types, two arrays, two booleans) at parse with `theta/parse/mixed-plus-operands`, exactly as `x = x + e` does. `-=`, `*=`, `/=`, `%=` compute [the numeric-only operators](./expressions.md#other-arithmetic) (§"Other arithmetic"); a non-`number` operand reaching one of them at runtime — a gap the parse layer does not close for these four forms — is a loud runtime defect, `theta/runtime/internal-error`, never a fabricated `0` or `NaN` (see [Diagnostics — code registry](./diagnostics/code-registry-parse.md#code-registry)).
