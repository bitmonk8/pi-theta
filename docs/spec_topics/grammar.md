...
**Inline object types.** `ObjectType` admits an anonymous object type `{ field: T, ... }` in any `Type` position. Its fields reuse the same `Field` form as an object-schema body and carry the same field semantics ([Schema Declarations — Object schema](./schemas.md#object-schema)): each field is required by default and optionality is written `T | null`. The inline `Field` form admits **no** wire-name rename: `as "WireName"` ([Wire-name renaming](./schemas.md#wire-name-renaming)) is a schema-declaration-only clause, and a field spelled `ident as "WireName": Type` inside an inline object is `theta/parse/renamed-inline-field-name`, judged over the same raw entry text the duplicate and quoted rules below read, and raised once per renamed field in source order before the body is lowered. `theta/parse/wire-name-collision` and `theta/parse/redundant-wire-name` apply only to the named-declaration spelling ([Object schema](./schemas.md#object-schema)); an inline object has no name for either diagnostic's subject to be, which the refusal above avoids by construction. An empty inline object `{}` is `theta/parse/empty-schema-body`, the same diagnostic an empty named schema body raises. An entry that spells no top-level `:` — contributing no key to any of the four raw-key rules below — is `theta/parse/malformed-schema-field` wherever the inline field loop's own entry walk reaches it, the same disposition the declaration position gives the same shape ([Schema Declarations — Object schema](./schemas.md#object-schema)), unless it carries a stray depth-0 close token, which keeps its own tolerant registration instead. One entry class stands outside that reach: such a close-token entry. A leading, doubled or post-trailing top-level comma opens an entry SLOT spelling no token at all (`{,a: integer}`, `{a: integer,,b: string}`, `{a: integer,,}`) — never the one grammar-legal trailing comma `","?` admits, which the loop's own `eatPunct(",")` consumes before the loop exits on `}`. Such a slot draws `theta/parse/malformed-schema-field` when a `Field` already derived earlier in the interior (one line per slot), or `theta/parse/empty-schema-body` when none did (`{,}`, `{,,}`), at most once per interior; an entry immediately behind the slot that itself qualifies for this section's keyless-entry refusal draws that refusal alone, in the slot's place ([bug 0257](../bugs/0257-empty-inline-object-entry-slot-silently-tolerated.md) §Fix). A missing entry separator no longer strands the entries behind it — the field loop resynchronises depth-aware to the interior's next top-level `,` at that failure, exactly as it already does at a colon-gate failure, so an entry standing behind an earlier entry whose type text carries a junk tail (`{a: b c, d e}`) is reached and refused when it is keyless ([bug 0256](../bugs/0256-generic-argument-stranded-entry-registers-permissive.md) §Fix, the operator ruling's OPTION 1 — resync-and-tolerate); the stranding entry itself keeps the colon-present disposition stated above and draws no line of its own. A key that repeats within one inline object type is `theta/parse/duplicate-inline-field-name`, judged over the entries the body spells between its top-level commas, on the text before each entry's own top-level colon, taken as written, and raised once per repeated key in source order before the body is lowered; a key reused between an outer inline object and one nested inside it is two field lists rather than a repeat. All four raw-key rules — duplicate, quoted, renamed and non-identifier — hold in every `Type` position, including a generic type argument's interior and any depth beneath it: the lowering never dividing that interior into fields bounds what reaches the wire, not whether the source key is judged. A non-repeating key of that same text whose first character is `"` or `'` is `theta/parse/quoted-inline-field-name`: the field-name position admits an identifier only ([Object schema](./schemas.md#object-schema) fixes a field name as one), so a quoted spelling is refused there exactly as it is refused in a named schema's declaration; a key that repeats keeps `theta/parse/duplicate-inline-field-name` alone. The `Type` reference inside each field is recursive, so nested inline objects and `array<{ ... }>` parse. At lowering, an inline object type is hoisted into `$defs` under a synthesised `__inline_<slug>` name — see [Schema Subset — Lowering Algorithm](./schema-subset.md#lowering-algorithm).
...
**Contextual keywords.** `subagent` and `with` are contextual keywords (theta 1.2), as is `par` before `for` (theta 1.1): each is recognised as a keyword only in its enumerated syntactic position(s) and is an ordinary identifier everywhere else. `subagent` is recognised only immediately before a top-level `fn` (the `subagent fn` modifier; the same word as the `mode: subagent` frontmatter value is unaffected); `with` is recognised in two positions — between a `subagent fn`'s parameter list — or its optional `: ReturnType` — and its body block (the session-config clause), and, since theta 1.3, immediately after a call expression's argument list and only when the next token is `{` (the call-site options clause; see [Call-site `with` clause](#call-site-with-clause) below). `par`, `subagent`, and `with` are the three contextual keywords in theta 1.2.0; theta 1.3 adds `with`'s second recognition position, not a keyword, so the set is unchanged. The reserved-keyword set is owned by [Lexical Structure — Reserved keywords](./lexical.md).
...
A `CallWithClause` is a postfix clause on the three clause-bearing call surfaces — a `.theta`-callable call through `tools:`, an `invoke(path, args…)` / `invoke<T>(path, args…)`, and a `subagent fn` call ([Functions — FN-6](./functions.md#fn-6); theta 1.4). It attaches immediately after the call's closing `)`, before any other postfix operator: in `f(a) with { cwd: t }?` the clause binds to the call and `?` then applies to the call's `Result`; the same before-any-postfix rule holds for `.`, `[`, and method-call postfixes. `with` here is the second recognition position of the contextual keyword defined under [Contextual keywords](#fn-declarations) above — recognised only immediately after a call expression's argument list and only when the next token is `{`, an ordinary identifier everywhere else. Recognition is positional, not callee-kinded: the clause parses after any call expression's argument list, and a clause on a call outside the three surfaces above is rejected by callee classification at the static pass that resolves callees, never by a failure to parse — default-reject: a callee the frozen callable set classifies as a Pi tool draws `theta/parse/with-clause-pi-tool`, and every other bare-identifier callee (a plain same-file `fn`, an imported non-`subagent` `fn` — re-export chains included — or any callee the set does not classify) draws `theta/parse/with-clause-in-process-callee`, the callee running in-process with no child working directory for the clause to set. Each `CallWithField` value is a **full expression**, evaluated at call time; the value semantics, evaluation order, mode gating, and failure arms are owned by [Invocation — Options surface](./invocation.md#options-surface), which also owns the default-reject callee classification ([INV-8](./invocation.md#inv-8)); the Pi-tool rejection is owned by [Tool Calls — Argument shape](./tool-calls.md).

The clause is deliberately distinct from the declaration-site `WithClause` of [`fn` declarations](#fn-declarations) above, and the two productions do not share nonterminals. Three differences carry the distinction: the key sets differ (`CallWithKey` is the closed per-call options set, first key `cwd`; `WithKey` is the five session-config keys); the value grammars differ (`CallWithField` admits any `Expr`, where a `WithValue` obeys the like-named frontmatter field's literal shape); and the unknown-key severities differ — a `CallWithKey` outside the closed set is a parse **error** `theta/parse/with-clause-unknown-key`, where an unknown declaration-site `WithKey` keeps the `theta/load/unknown-frontmatter-field` **warning**, because the declaration-site keys mirror frontmatter fields (forward-compatible unknown-key handling by design) while the call-site keys are per-call runtime options closed in theta 1.3 (see [Invocation — Options surface](./invocation.md#options-surface) for the divergence rationale). Declaration vs call clause is carried by syntactic position alone, exactly how the grammar already disambiguates contextual keywords.
