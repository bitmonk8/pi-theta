...
**Why the transcript bytes are pinned.** The enclosing [System-prompt structure (normative)](#system-prompt-structure-normative) sub-section opens by allowing the prompt wording to vary, but the transcript rendering is stricter on purpose: it is a hard reproducibility contract, not revisable illustration. The [Determinism](#determinism) section fixes `temperature: 0` and a per-loom FNV-1a seed so the same session yields the same binder decision on every run and across processes. That guarantee holds only when the binder model sees byte-identical input; wobble in the role tags, the `: ` separator, the turn delimiter, the `[tool-call …]` line form, or the trailing-space contract on the empty-content user message changes the prompt bytes and can change the sampled output even at `temperature: 0`. Reference renderings A–D below are therefore the exact bytes the determinism guarantee depends on, which is why they are pinned as MUST-reproduce-exactly rather than left as examples revisable for clarity.
