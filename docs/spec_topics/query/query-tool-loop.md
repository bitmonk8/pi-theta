...
<a id="qry-14"></a> **QRY-14.** A typed query is an ordinary tool-loop conversation whose *final* response is structured. The runtime presents the loom's frontmatter `tools:` to the model alongside a synthesised one-shot respond tool (`__loom_respond_<slug>`, see [Implementation Notes — Runtime](../implementation-notes.md#runtime)) and runs a **two-phase** loop:
