...
<a id="qry-13"></a> **QRY-13.** If the model responds with tool-use, the runtime executes the requested tool against the theta's callable set, feeds the result back to the model, and loops until the model produces a final (non-tool-call) response or the round cap fires (see **Tool-call loop bound** below). That final response is what the query returns. A response schema, if given, is enforced against the final response only — not against intermediate tool-call payloads. The lifetime and visibility of the theta's callable set (including the typed-query `__theta_respond_<slug>` tool when a schema is in force) is governed by [Pi Integration Contract — Tool-registration lifetime and visibility](../pi-integration-contract.md): subagent-mode queries see the callable set's host-tool half via the spawned child `pi` process's `--tools` allowlist (its model reaches the tools registered natively in the child; `.theta` callables ride the presented-name + closure-hash carrier instead — see [Subagent — `--tools` carries HOST-tool names only](../pi-integration-contract/subagent.md#subagent-tools-host-names-only)); prompt-mode queries see it via a `pi.setActiveTools` snapshot/restore around the turn, so the user's bare session never inherits the theta's callable set.

When a tool the model invoked inside this loop fails — `execute()` throws, or resolves `{ content, isError: true }` — the runtime lowers that outcome to the failing `tool_use` block's tool-result (an `isError: true` result carrying the tool's text content, lowered as for code-side calls in [Pi Integration Contract — Tool execution from theta code](../pi-integration-contract/host-interfaces-core.md#tool-execution-from-theta-code)) and feeds it back to the model alongside any sibling results; the round counts against `tool_loop.max_rounds` per [CIO-4](../hard-ceilings/ceilings-3-and-4.md#cio-4) and the loop continues. Such an in-loop tool failure does **not** raise `ModelToolError` — it is fed back as a tool-result and the loop continues; `ModelToolError` is a reserved variant with no theta 1.0-reachable producer (see [Errors and Results — `ModelToolError`](../errors-and-results/queryerror-variants.md#queryerror-variants)). Cancellation of a model-driven `execute()` continues to surface as `CancelledError` and is never fed back as a tool-result (see [Cancellation](../cancellation.md)).
...
The technique used to obtain the structured payload is provider-specific (synthesised one-shot tool + forced tool choice for the theta 1.0 reference; native structured output where supported in future revisions); the *behavioural* contract above is what authors and tests rely on. Provider compatibility is bounded by **Provider compatibility for typed queries** in [Pi Integration Contract](../pi-integration-contract.md). A model that rejects the forced tool choice at runtime — a MODEL-level refusal inside a supported api — does not terminate the query on that rejection alone: the runtime re-issues the dispatch once with the tool choice left to the provider default (the one-shot **degraded dispatch**; the single respond tool and the template's instruction still stand), per the bug-0481 pin clarification under [Pi Integration Contract — `complete()` forced-tool behavioural presupposition](../pi-integration-contract/conversation-drive.md#complete-forced-tool-presupposition); a degraded reply that still fails to call the tool lands in ERR-17 non-compliance and respond-repair exactly as any other non-compliant reply.

### Respond-tool wire schema

<a id="respond-tool-wire-schema"></a>

The respond tool's `parameters` document — and, through it, the `<schema-json>` the [initial forced respond turn template](#initial-forced-respond-turn-template-normative) below and the [respond-repair follow-up templates](./query-failure-and-repair.md#follow-up-turn-templates-normative) interpolate — is the **wire schema** derived from the lowered response schema:

- A lowered schema whose root admits a JSON object is its own wire schema, byte for byte. This covers every object-rooted lowering and a root that constrains nothing (the `{}` residual).
- Every other lowered root — a `type` excluding `object`, an `enum` or `const` literal set, an `anyOf` / `oneOf` union, a root `$ref` — is carried one level down under the **single-property envelope** `{ "type": "object", "properties": { "value": <lowered root> }, "required": ["value"] }`. The lowered document's root `$defs` table moves to the envelope root, because a `#/$defs/<Name>` pointer resolves against the document root alone: left under `properties.value`, a lowered `array<Item>` root's own pointers dangle and the document fails to compile.

The envelope is a wire artifact forced by the wire's shape. A tool call's `arguments` are a JSON object, so a `parameters` document rooted anywhere else rejects every call the model can emit — the host validates a registered tool's arguments against `parameters` before `execute` runs, and on the off-session forced-respond dispatch `parameters` is the provider's tool input schema, which must be object-rooted — the same constraint the [binder attachment wrapper](../pi-integration-contract/binder-inference.md#binder-inference-call) exists for. Left unenveloped, a non-object root rejects every call and spends the model's repair budget on a shape no argument object satisfies. The envelope therefore applies to unsatisfiable roots alone: an object-rooted typed query registers and conveys the same bytes it would without this rule.

Everything keyed to the schema stays keyed to the **lowered** schema, never to the envelope: `<slug>` (the registered `__theta_respond_<slug>` name, the tool reference both templates carry, and the canonical-form bytes the registration cache byte-compares under [PIC-44](../pi-integration-contract/tool-registration-lifetime.md#pic-44)), the depth walk of [CIO-3](../hard-ceilings/ceilings-3-and-4.md#cio-3), and the AJV validation [QRY-22](./query-failure-and-repair.md#qry-22) obliges. Under an enveloped registration the candidate response payload is the `value` member of the call's arguments, recovered before the depth walk and the validation that follows it, so both run over the payload against the lowered schema exactly as they do for an object root. A call whose arguments carry no `value` member is taken as the payload verbatim: it fails validation and enters respond-repair as the non-conforming response it is, instead of being rewritten to an absent value.
