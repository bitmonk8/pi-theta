...
- `BNDR-5`: number echo is shortest round-trip fixed-point — no scientific notation (including the `±1e21` and `|value| < 1e-7` switches expanded to full fixed-point); a non-integral value carries at least one fractional digit; an integral value carries no `.0`; `-0` renders `0`.
