...
**Ships when.** `npm test` fires the argument codes, asserts the closed `CodeToolError` enum and off-surface outcomes, asserts both return-type rows lower on the accepted path (Pi tool → `Ok(string)`, subagent-mode `.loom` → `Ok(T)`), asserts the host-denial → `Err{code_tool,execution}` mapping (never silent `Ok`), and asserts the code-side `execute()` Promise's three-channel swallowing-handler suppression (no `unhandledRejection`, no second `RuntimeEvent`, no diagnostic) at the `Checkpoint` seam (`V8a`).
