# `V3e` — Expression stdlib members

**Spec.** [`../spec_topics/expressions.md`](../spec_topics/expressions.md).

**Adds.** The `string`/`array<T>`/`object` standard-library member surface evaluated on top of the [`V3a`](./V3a-expression-evaluator.md) expression interpreter: the `String.replace` reference vectors and `concat`, the `string` members (`length`, `toLowerCase`/`toUpperCase`/`trim`, `startsWith`/`endsWith`/`includes`, `split`), the `array<T>` members (`length`, `join`, `includes`/`indexOf`, `slice`), and the `object` members (`keys`, `values`, `has`).
...
**Ships when.** `npm test` reproduces the five `replace` vectors and exercises every loom-1.0 `string`/`array`/`object` stdlib member against its normative behaviour and return type.
