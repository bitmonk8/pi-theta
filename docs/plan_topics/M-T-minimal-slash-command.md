...
**Adds.** The failing tests for the narrowest end-to-end pipeline: discover one `.loom` file from a single source, register it as a slash command, dispatch it, parse its `mode:` frontmatter, and issue a single untyped `@`-query whose assistant response appends as one prompt-mode turn to the caller's conversation.
