...
**Adds.** The `LoomRegistry` drain-state contract (two fields, three methods, two-arm `readDrainState`, idempotent predicate, read-failure fail-safe, superseded-entry dispatch) over the registry [`V9b`](./V9b-registration-reload-wiring.md) builds and swaps. The closed two-arm dispatch (dispatch / shutting-down note), the no-third-arm / no-alternative-gating-field prohibition, and the `readDrainState` read-failure fail-safe MUSTs are owned here.
...
**Ships when.** `npm test` exercises the two drain-state arms (incl. the `readDrainState` read-failure fail-safe) and the superseded-entry dispatch returning the fixed system note.
