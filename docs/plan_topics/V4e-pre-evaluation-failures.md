...
**Deps.** `V4e-T`, `V9a`, `V6a`, `V11f`, `V10a`, `V16a`, `V9b`, `V10c` (the latter two own the **watcher-time reload failure-injection seam** — registry-swap and `.loom`/`.warp` re-parse in `V9b`, settings re-merge in `V10c` — against which `ERR-7`'s synthetic failure is injected)
