...
thinking: high              # thinking level for every query (off | minimal | low | medium | high | xhigh | max)
...
Fields not listed here are not part of the theta vocabulary and surface as
...
still loads and registers in both cases. An unrecognised sub-key inside a
`tool_loop:` or `respond_repair:` mapping draws the same
`theta/load/unknown-frontmatter-field` warning, rendered with the dotted
`<block>.<sub-key>` form (e.g. `tool_loop.max_round`), and the theta still
registers.
...
| `thinking` | no | no pin: the host's own thinking-level resolution | (theta 1.6, bug 0491.) The thinking level every query of the theta runs at — one of `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max` (pi's `ThinkingLevel`), clamped by the host to the levels the resolved model supports. Absent, nothing is pinned: a prompt-mode turn runs at the session's level — or, under a `model:` swap, at the level the host's model switch derives for that model — and the thinking window restores the session's own level afterwards; a subagent child resolves its level from its own settings (the per-model level on hosts that have one, pi ≥ 0.84.3, else the default). Any other present value (an unknown string, a non-string scalar, a non-scalar) is the load-time error `theta/load/unknown-thinking-value` and the theta is not registered. In `mode: subagent` the level is marshalled as `--thinking <level>`; in prompt mode it is applied by the thinking window. It does not affect the off-session forced respond turn. |
| `bind_model` | no | `theta.binderModel` setting (no further fallback) | Slash-command argument binding uses the fallback model. When neither `bind_model:` nor `theta.binderModel` resolves and the theta is not bypass-eligible, load fails with `theta/load/binder-model-unresolved`. The configured string is matched to a model per the binder-model parse rule. The resolved model is then run through the four-way strict-capability requirement: an explicit `false` strict-capability indicator fails the load with `theta/load/binder-model-not-strict-capable` (E); an absent indicator on the resolved model while at least one available model exposes one emits `theta/load/binder-model-strict-capability-unknown` (W) and the theta still registers; an absent indicator with NO available model exposing one (the shape under the theta 1.0 Pi-SDK pin, where `strictCapable` is absent on every Pi-supplied `Model<Api>`) admits the model silently and the theta registers with no diagnostic, host-wide absence being a documented constant rather than a per-theta event. The load-time refusal does not apply to the marked root theta of a spawned subagent child (the subagent-root exemption). |
...
| `bind_echo` | no | `true` | Bound args are echoed before execution, except auto-suppressed on the binder bypass. A present value that is neither `true` nor `false` (non-boolean scalars and non-scalar nodes included) is the separate `theta/load/unknown-bind-echo-value` load-time error and the theta is not registered — no truth-coercion. |
...
| `respond_repair` | no | `{ attempts: 3, methodology: validator_error }` | Typed queries get the default respond-repair budget. `respond_repair: {}` (block present, sub-keys absent) is equivalent to omitting `respond_repair:` entirely; the defaults apply. A present non-mapping value is `theta/load/malformed-respond-repair-field` (the theta does not register); a bare `respond_repair:` null scalar stays silent. |
| `tool_loop` | no | `{ max_rounds: 25 }` | Every query (untyped, typed, and any respond-repair follow-up) runs its tool-call loop under the default cap. `tool_loop: {}` (block present, `max_rounds` absent) is equivalent to omitting `tool_loop:` entirely; the default `25` applies. A present non-mapping value is `theta/load/malformed-tool-loop-field` (the theta does not register); a bare `tool_loop:` null scalar stays silent. |
...
documented, accepted cost of the zero-token code channel. A `subagent fn`
