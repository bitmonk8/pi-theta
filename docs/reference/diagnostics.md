...
| `theta/parse/inline-field-name-not-identifier` | E | parse | `field name '<field>' within one inline object type is not an identifier` |
...
| `theta/parse/stdlib-arg-type-mismatch` | E | type | `stdlib method '<method>' on type <type> argument <i> type mismatch: expected <expected>, got <actual>` |
...
| `theta/load/session-tool-unavailable` | E | load | `runtime tool '<name>' is unavailable on this host: '<member>' is not a function` |
