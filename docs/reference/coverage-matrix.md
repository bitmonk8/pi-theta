...
|---|---|---|---|
| [grammar.md](./grammar.md) | Lexical structure; grammar productions (literal sublanguage, `let`, blocks, `fn`, `match` arm body, `schema X by <field>`, `///`, type grammar, newline continuation, `array<T>` type-sink, call-site `with` clause); expression forms; operator precedence; built-in methods; control flow; `return`; bindings & mutability | `grammar.md`, `lexical.md`, `expressions.md`, `control-flow.md`, `functions.md`, `return.md`, `bindings.md` | complete |
| [type-system.md](./type-system.md) | Type expressions; compatibility relation `⊑` (TYPE-1…TYPE-11); array/ternary common-type; runtime-tool call typing (RFC 0011); runtime value model; equality; wire-name translation; engine assumptions; effects | `type-system.md`, `runtime-value-model.md`, `expressions.md`, `tool-calls.md` | complete |
| [frontmatter.md](./frontmatter.md) | Frontmatter field contract table (verbatim); naming convention; `params:` types/defaults; `tools:` (callable set, FRNT-2/3, resolution snapshot, the `theta_progress` extension tool incl. clamps and the model self-report consequence — EXST-13…EXST-15, PIC-74; runtime-tool entries incl. precedence and the load-time host-member probe — RFC 0011); `system:` interpolation; `respond_repair:`; `tool_loop:` (FRNT-1); template interpolation | `frontmatter.md`, `frontmatter/frontmatter-fields-a.md`, `frontmatter/frontmatter-fields-b-and-templates.md`, `execution-status.md`, `tool-calls.md` | complete |
...
| [errors-and-results.md](./errors-and-results.md) | Terminal outcomes; pre-evaluation failure surface (ERR-1…ERR-7, ERR-16); no-rollback (ERR-8…ERR-13); runtime panics + message templates; `QueryError` variant schemas (ERR-14, ERR-15, ERR-17, ERR-19); runtime-tool `CodeToolError` arms (RFC 0011); final value (FN-5) | `errors-and-results.md`, `errors-and-results/error-model.md`, `errors-and-results/queryerror-variants.md`, `functions.md`, `query/*`, `invocation.md`, `tool-calls.md` | complete |
...
| [discovery-cli.md](./discovery-cli.md) | Five discovery sources; priority; failure modes (DISC-2); filename validity; collisions (DISC-1, DISC-3, DISC-4); discovery roots; package discovery (DISC-5, DISC-6); settings (DISC-7, keys incl. `theta.progress`, `thetaPaths` schema); slash invocation (SLSH-1…SLSH-5, SNK-a…SNK-k, the SLSH-2 execution-status telemetry carve-out); execution-status operator surface (`theta.progress` ceiling incl. the class-2 `theta_progress` off-gate, `/theta-status` view override); `invoke` (resolution, typed return, arity, cross-mode, cycle detection, static resolution, INV-1) | `discovery.md`, `discovery/discovery-sources.md`, `discovery/package-and-settings.md`, `invocation.md`, `slash-invocation.md`, `execution-status.md` | complete |
