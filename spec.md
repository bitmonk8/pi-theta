...
1. **Node version floor.** Node `>=20.6.0`, matching `@mariozechner/pi-coding-agent`'s `engines.node` floor at the pinned peer-dep version. The literal `>=20.6.0` is a *floor*, not a range — there is no upper bound. A Pi minor bump that moves that floor requires re-validating the loom range in the same edit, alongside the surface re-validation called out under **Pi SDK and capabilities** above.
...
3. **Pi SDK named-capability surface.** The seven SDK capabilities enumerated under **Pi SDK and capabilities** above MUST be present in the host's `@mariozechner/pi-coding-agent` instance at extension-factory entry. The detection-and-failure contract for a missing capability is owned by [Pi Integration Contract — Extension entry point](./spec_topics/pi-integration-contract.md); V1 specifies no separate capability probe beyond what that section defines.
