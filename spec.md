...
- **Source-language stability.** A `.loom` or `.warp` file that loads cleanly under V1.0 is guaranteed to load and behave identically under every V1.x release. Substantive grammar or semantics changes follow the REQ-ID lifecycle in `GOV-8` above (split / merge / deletion-plus-add, never in-place rewording), so the user-facing observable — V1.x stability — is what `GOV-8`'s change discipline produces. Migration across major versions is out of V1 scope; see [Future Considerations](./spec_topics/future-considerations.md).
