...
- **Failure.** The body returned `Err` (whether via `?` propagation or an explicit `Err(...)` return), panicked (the closed list under **Runtime panics** below), or exhausted a hard runtime ceiling (per [`spec.md` — Hard runtime ceilings](../spec.md#orientation), with the per-ceiling routing class — panic / `Err` variant / load-time — named at that bullet). No final value flows; the caller observes only the `Err` envelope per the per-surface mappings below.
...
The trichotomy applies only once evaluation has begun. Failures that occur *before* evaluation begins — host-incompatibility detected by the capability probe, lex / parse / type batches, frontmatter rejection, binder-model resolution failure, `tools:` resolution failure, watcher-time reload failures — are NOT evaluation outcomes; they surface per [Diagnostics](./diagnostics.md) on the `loom-system-note` channel, never produce appended turns or a final value, and are not subject to cancellation. An `invoke` parent whose callee fails to load observes a separate evaluation-time failure of its own: `InvokeInfraError { reason: "load_failure", ... }` per [Invocation — Failures](./invocation.md), which IS an evaluation outcome of the *parent*.
...
| Hard-ceiling exhaustion | per the per-ceiling routing class named at [`spec.md` — Hard runtime ceilings](../spec.md#orientation) | same | same |
