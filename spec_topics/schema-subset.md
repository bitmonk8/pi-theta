...
| #2 Tool-call args, model-driven | the model | tool-error result fed back as the next user turn per [Query — Tool calls during a query](./query.md); the round counts against `tool_loop.max_iterations` and the loop continues. No `QueryError` reaches loom code unless the loop later exhausts under ceiling #2 — in particular, this row does **not** surface as `ModelToolError` (which covers tool-execution failures inside the loop, not validation failures the runtime synthesises before the tool body runs) |
