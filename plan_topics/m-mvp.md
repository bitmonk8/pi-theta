...
**Adds.** Lexer + parser limited to: frontmatter (`---` block with one line `mode: prompt`; `params:` absent or `params: {}`); body containing exactly one expression-statement of the form `` @`literal text` `` (no `${...}`, no `?`, no escapes other than `\``). Runtime: walks the body, calls `ConversationDriver.send` once, awaits `agent_end`. The MVP loom is no-params: `params:` absent and `params: {}` are equivalent, the binder is bypassed, and trailing slash text after the command name emits a single `customType: "loom-system-note"` formatted as `loom /<name>: ignoring extra arguments — this loom takes no parameters` before the loom runs (full rule in V3c). Slash command registration: the `description` field passed to `pi.registerCommand` carries `frontmatter.description` verbatim and only that — `argument-hint` is not concatenated and is not surfaced through the autocomplete dropdown (binder-grounding only; see [Slash-Command Argument Binding](../spec_topics/binder.md)). Discovery: `~/.pi/agent/looms/` and `.pi/looms/` only.
