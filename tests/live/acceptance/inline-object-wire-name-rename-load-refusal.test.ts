...
const CLEAN = [
  ...
  "@`What is 205 plus 384? Answer with the number only.`",
  "",
].join("\n");
...
describe("-- H9a live: bug 0160 renamed-inline-field-name refusal at the inline object field slot through the real `pi -p`, and the rename-free sibling end to end", () => {
  it(': refuses the theta whose annotation carries `{a as "w": string}`, still registers and drives the rename-free `{wire: string}` sibling, and measures whether the code reaches the H9a capture', async () => {
    ...
    await expectOffenderProbeClean({
      slug: "b0160",
      files: {
        "renoffender.theta": OFFENDER,
        "renprobe.theta": PROBE,
        "renclean.theta": CLEAN,
      },
      clean: {
        slashInvocation: "/renclean",
        expected: CLEAN_SENTINEL,
        message: (clean) => `clean: the rename-free sibling must register and DRIVE a real turn -- without this the refusal assertion below could pass vacuously (wrong root, no registration at all). stdout: ${clean.stdout} stderr: ${clean.stderr}`,
        codesMessage: "clean: the rename-free sibling must carry NO theta/{load,parse,runtime}/* code at all -- the refusal must emit nothing on the good path.",
      },
      probe: {
        slashInvocation: "/renprobe",
        expected: REFUSED,
        unexpected: LOADED,
        message: (probe) => `probe: the offending theta must NOT load, so the prober's invoke("./renoffender.theta") resolves Err(InvokeInfraError) and the match prints "${REFUSED}". Printing "${LOADED}" means bug 0160's inline rename is still admitted. stdout: ${probe.stdout} stderr: ${probe.stderr}`,
      },
