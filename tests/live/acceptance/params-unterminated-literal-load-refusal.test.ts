  ...
  it('refuses the theta whose params: field carries `{a as "w: integer, b: integer}`, still registers and drives the well-formed `p: string` sibling, and measures whether the code reaches the H9a capture', async () => {
    ...
    await expectOffenderProbeClean({
      ...
      clean: {
        ...
        message: (clean) => `clean: the well-formed sibling must register and DRIVE a real turn -- without this the refusal assertion below could pass vacuously (wrong root, no registration at all). stdout: ${clean.stdout} stderr: ${clean.stderr}`,
      ...
      probe: {
        ...
        expected: REFUSED,
        unexpected: LOADED,
        message: (probe) => `probe: the offending theta must NOT load, so the prober's invoke("./poffender.theta") resolves Err(InvokeInfraError) and the match prints "${REFUSED}". Printing "${LOADED}" means bug 0232's unterminated wire-name literal is still being admitted and lowered to the permissive \`{}\`. stdout: ${probe.stdout} stderr: ${probe.stderr}`,
      },
