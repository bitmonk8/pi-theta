...
const PROBE = [
  ...
  '  Err(e) => "846"',
  "}",
  "@`A load probe reported code ${verdict}. What is that code plus 100? Answer with the number only.`",
  "",
].join("\n");
...
describe("-- H9a live: bug 0154 binding-case-mismatch refusal at the inline object field-name slot through the real `pi -p`, and the lowercase-first sibling end to end", () => {
  it(": refuses the theta whose annotation carries `{Ys: string}`, still registers and drives the lowercase-first `{ys: string}` sibling, and measures whether the code reaches the H9a capture", async () => {
    ...
    await expectOffenderProbeClean({
      slug: "cellbcm",
      files: {
        "cellbcmoffender.theta": OFFENDER,
        "cellbcmprobe.theta": PROBE,
        "cellbcmclean.theta": CLEAN,
      },
      clean: {
        slashInvocation: "/cellbcmclean",
        expected: CLEAN_SENTINEL,
        message: (clean) => `clean: the lowercase-first sibling must register and DRIVE a real turn -- without this the refusal assertion below could pass vacuously (wrong root, no registration at all). stdout: ${clean.stdout} stderr: ${clean.stderr}`,
        codesMessage: "clean: the lowercase-first sibling must carry NO theta/{load,parse,runtime}/* code at all -- the fix must not emit anything on the good path.",
      },
      probe: {
        slashInvocation: "/cellbcmprobe",
        expected: REFUSED,
        unexpected: LOADED,
        message: (probe) => `probe: the offending theta must NOT load, so the prober's invoke("./cellbcmoffender.theta") resolves Err(InvokeInfraError) and the match prints "${REFUSED}". Printing "${LOADED}" means bug 0154 is unfixed. stdout: ${probe.stdout} stderr: ${probe.stderr}`,
      },
