...
const normativeMessagePattern = liveRegistryMessagePattern("bug-0275", "load");
    ...
    try {
      ...
      expect(
        ...
        `bug-0275 live cell precondition unmet: no ${INVOKE_PATH_ESCAPE_CODE} row is located ` +
          ...
          "this report extends — did not fire. Notes: " + JSON.stringify(collectSystemNotes(offender.sessionManager.getEntries())),
      ).not.toEqual([]);
      expect(
      ...
      expect(
        ...
        `the relocation spans exactly one level, so no ${INVOKE_PATH_ESCAPE_CODE} row belongs ` +
          "at the grandparent's file. Notes: " + JSON.stringify(collectSystemNotes(offender.sessionManager.getEntries())),
      ).toEqual([]);
      ...
      expect(
        ...
        "one escaping entry draws exactly one row at its immediate caller — the relocated " +
          ...
          "Notes: " + JSON.stringify(collectSystemNotes(offender.sessionManager.getEntries())),
      ).toEqual([]);
    } finally {
    ...
    try {
      ...
      expect(
        ...
        `an entry that resolves inside an active discovery root must draw no ` +
          ...
          "workspace too and is named by nothing. Notes: " + JSON.stringify(collectSystemNotes(control.sessionManager.getEntries())),
      ).toEqual([]);
