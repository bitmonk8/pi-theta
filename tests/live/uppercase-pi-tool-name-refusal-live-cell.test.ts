...
describe("bug 0108 live cell — an uppercase-first Pi-tool registry name is refused at live production load ", () => {
  it("un-registers the `- WebSearch` theta while its `as`-renamed sibling and the lowercase control both register", async () => {
    ...
    const thetas: PlantedTheta[] = [
      ...
      { source: "project", stem: "upperpitool", text: toolsTheta(["tools:", `  - ${UPPER_TOOL}`]) },
      // (b) The escape hatch the refusal Message points the author at.
      {
        source: "project",
        stem: "upperrenamed",
        text: toolsTheta(["tools:", `  - ${UPPER_TOOL} as ${LOWER_TOOL}`]),
      },
      ...
      { source: "project", stem: "lowercontrol", text: toolsTheta(["tools:", `  - ${LOWER_TOOL}`]) },
    ];
    ...
    try {
      ...
      expectRegisteredControlThenAbsentSubject(
        ...
        [
          {
            stem: "lowercontrol",
            message: "bug-0108 live cell precondition unmet: the lowercase extension-tool control " +
          ...
          {
            stem: "upperrenamed",
            message: "bug-0108 live cell: the `as`-renamed sibling " +
        ...
        {
          stem: "upperpitool",
          commandMessage: `bug-0108 live cell: a \`tools: ${UPPER_TOOL}\` theta registered through the real ` +
            ...
            "state bug 0108 reports. Registered: ",
          registeredNamesMessage: "bug-0108 live cell: the uppercase-first Pi-tool theta's slash name must not appear " +
