...
**Remaining documented deferrals/follow-ups (none are named-backlog bugs; each recorded with rationale):** QRY-20 discard-observability event (belongs with the whole operator-facing always-log `RuntimeEvent` layer — thread `SystemNoteChannelDeps`+`Clock`+`invocationId` into `ExecuteBodyDeps`, stop dropping `outcome.event` in `runQueryEffect`, + void-tail detection); the tool-return-shape DIAGNOSTIC is discarded in prod because the tool-lowering path threads `noopSink()` (behavioral correctness intact; a pre-existing sink-wiring limitation); subagent `dispose()`/`forwarding.detach()` are skipped when `executeBody` THROWS (pre-existing leak; B1's `finally` fixes only the registry-entry cleanup — move `dispose()`/`detach()` into a `finally` to close it); `dispatchSiteSetup`/`runPerInvocationFinally` remain unwired (B1 chose inline entry bookkeeping to avoid relocating the producer's session ownership); drain-state two-arm reconciliation (delete the dead-in-prod `degraded` arm end-to-end + rewrite `drain-state-contract.test.ts`); model-callable-`.loom` lowering (`TODO(SUBAG-2)` — a larger unimplemented feature, not a residue). Genuine 1.0 cuts kept: `tool-batch` (sequential safe per CLAUDE.md), `argument-hint` (no Pi SDK slot), binder forced-tool (`anyOf` not a valid Anthropic `input_schema`).
