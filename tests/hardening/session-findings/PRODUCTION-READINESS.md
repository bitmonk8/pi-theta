...
- **Increment B2 (forwarding list, sub-step 5). ⏭️ NEXT.** Thread a shared forwarding-listener list so sub-step 5 detaches the `ctx.signal`/tool/parent-invoke listeners on shutdown. Higher-risk: the live `forwardSlashCommandCancel`/`deriveChildLoomAbort` seams return no detach handle (only `attachSubagentAbortForwarding` does) and are pinned by CANCEL-2/V17b — must capture `(signal, listener)` or expose detach without disturbing those. `{once:true}` listeners self-remove post-fire, so the value case is the still-attached, not-yet-aborted invocation at shutdown; `ForwardingSignalSource.label` is a closed 3-member set matching the spec's `details.call` labels.
