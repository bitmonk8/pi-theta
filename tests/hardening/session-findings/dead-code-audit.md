...
- **`src/runtime/tool-registration.ts`** — V9f: the PIC-17 active-set snapshot/swap/restore gate **is superseded** (folded inline into `production-loom-producer.ts` `#driveUserVisibleTurn` and `driveStreamedUserTurn`: `getActiveTools()` → `setActiveTools([...])` → `finally setActiveTools(ambient)`), **but** the module also carries genuinely-unwired residues — PIC-8 restore-failure protocol (the production `finally` has no `try`/`catch`), PIC-19 setup-failure protocol, PIC-44 registration cache-collision, and `ToolDefinition.label` derivation (`factory.ts` registers commands with no label). KEPT because deletion would drop the only implementation of those obligations (mixed module). **⛔ ALL FOUR RESIDUES INVESTIGATED → ACCEPTED CUT / ALREADY-COVERED (2026-07-13; not wired, module + its 11 unit tests retained as the sole GOV-22-residue impl of cka-17/cka-60):**
  - **PIC-44 registration cache-collision (`registerToolInCache`/`createRegistrationCache`) → NOT REALIZABLE.** No live seam: production **never** calls `pi.registerTool` (it appears only in `capability-probe.ts` existence-probe + `sdk-inventory.ts` surface list). Subagent mode installs tools via `customTools` on `createAgentSession` (`production-loom-producer.ts:1391`); prompt-mode typed queries use the forced-respond **text-extraction** path (`renderTypedAwareQueryText` + `parseStructuredPayload`), registering no `__loom_respond_<slug>` tool; prompt-mode `.loom` callees are rejected at load time. With no registration cache, no slug collision can occur — the whole prompt-mode `pi.registerTool` mechanism the PIC-44 (`tool-registration-lifetime.md#pic-44`) spec describes was superseded by that architecture. Wiring would require inventing a registration mechanism the shipped runtime deliberately omits.
  - **`ToolDefinition.label` derivation (`deriveToolLabel`) → NOT REALIZABLE (no live consumer).** No production site materialises a `.loom`-as-tool `ToolDefinition` carrying a `label`: `factory.ts` registers slash commands via `pi.registerCommand` (no `label` field); subagent `customTools` are host built-ins from the SDK `create*ToolDefinition` factories (which carry their own labels); model-callable-`.loom` lowering is the explicit `TODO(SUBAG-2)` at `production-loom-producer.ts:1256`; the typed-query respond tool is not a registered `ToolDefinition`. cka-17 (`extension-bootstrap-and-per-loom.md` §Per-loom registration) has no live surface to attach to.
  - **PIC-8 restore-failure protocol (`withActiveSetGate`/`restoreActiveSet`) → DEFENSIVE-ONLY + WOULD-PERTURB-HOT-PATH.** The 3 live restore seams (`#driveUserVisibleTurn` 2483, `driveStreamedUserTurn` 2948, `runPromptSuspendInvoke`/`invoke-prompt-suspend.ts` live at 1998) each do a bare `finally { setActiveTools(ambient) }`. The only trigger is `pi.setActiveTools` throwing at step-4 restore after **succeeding** at step-2 install on the same turn — not realizable at the loom 1.0 Pi-SDK pin, where `setActiveTools` is a synchronous in-memory name-list swap (`tool-registration-lifetime.md#snapshot-restore-pi-behavioural-preconditions`) that cannot transiently fail; the probe validates the surface at bootstrap. Wiring the full protocol (single re-attempt + `active-set-restore-failed` (E) + verbatim `display:true` note + non-masking of the inner error) requires threading `emitDiagnostic`/`emitSystemNote`/`loomName`/`routeInternalError` sinks through 3 hot CANCEL/PIC-17 seams. `notes.md` (2026 V15d entry) already ratified that delegating those seams to `withActiveSetGate` forces no-op sinks (a latent defect) and that every real test path has the restore succeed. Do NOT perturb.
  - **PIC-19 setup-failure protocol → ALREADY-COVERED (outer runtime-defect boundary) + DEFENSIVE-ONLY.** Step-1 `getActiveTools()` / step-2 `setActiveTools([...])` sit **before** the `try` in the live seams; an unhandled throw there is NOT caught by the `mapPromptModeSyncThrow` guard (2453, `sendUserMessage`-only) and unwinds to the top-level runtime-defect surface (`surfaceUnexpectedThrow` → `loom/runtime/internal-error`, `runtime-panics.ts`) — exactly the PIC-19 (`tool-registration-lifetime.md#pic-19`) route — with no restore owed (the throw precedes the `finally`). The observable PIC-19 outcome is thus already achieved; the trigger is likewise defensive (post-probe SDK-shape drift, `capability-probe.md#post-probe-sdk-shape-drift`). An explicit seam-level route would perturb the hot path and duplicate the outer boundary.
  Net: WIRE NONE. Module retained solely as the cka-17/cka-60 GOV-22-residue impl; its 11 unit tests (`tests/tool-registration-lifetime.test.ts`) are the sole realization. Mirrors the V17b `forwarding-listener-trap.ts` accepted-cut precedent above.
