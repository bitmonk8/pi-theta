...
const { src, paramsDoc } = paramsDefaultFixture(BODY, "bug0175.theta");
 ...
 * register: the registration gate (src/parser/frontmatter.ts:1515–1517) withholds
...
function expectRefusedAsResidue(label: string, doc: ThetaDocument, residue: string): void {
  ...
  expectParamsDropGateShape(label, doc, diagnostic, loweredP, {
    severity: `the drop gate reads error severity, so a warning would leave the theta registered with a default whose value is a truncation of its own source`,
    frontmatter: `an error-severity params diagnostic withholds the frontmatter, which is what un-registers the theta — the observable that keeps the truncated value out of body scope`,
    lowered: `no lowered \`params:\` fragment may survive the refusal — a surviving one is what the post-default-merge AJV hook then judges the truncated value against`,
  });
}
...
const driveIfRegistered = makeDefaultBinderDrive(FIXTURE_SOURCES, scripted, JSON.stringify);
