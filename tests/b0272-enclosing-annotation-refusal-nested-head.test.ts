...
// ===========================================================================
//
// `LoadRow`, `registered`, `theta`'s FRONTMATTER-prefixed body and `msg`/`line`'s
// rendering are the shared harness in `tests/helpers/load-row-harness.ts` (also
// used by tests/b0273-query-result-error-side-unresolved-name.test.ts); this
// file's own `extents`/`expectCaptured`/`expectRows` diverge from that shared
// shape (extent tracking, a different precondition arity) and stay local.

/** A `mode: prompt` theta whose body is `body` verbatim, parsed once. */
function theta(label: string, body: string): LoadRow {
  return loadRowFromBody(label, body, "b0272.theta");
}
