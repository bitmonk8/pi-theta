...
// `checkImportMalformedSpecifierList` (src/parser/imports.ts:419–439) and
// `checkImportDanglingAlias` (:451–465) read. Left unrefused, the dangling `as`
...
//      'from' clause with a .thetalib path literal", src/parser/imports.ts:361–362)
...
// contract, src/parser/imports.ts:441–465). Group (d) fences the STATEMENT
...
describe("bug 0100 (f) — the dangling `as`'s downstream consequences, pinned with the refusal", () => {
  ...
  it("RED (f-collision): the collapsed local name still collides, and the specifier is now refused", async () => {
    ...
    // avoid (`checkImportNameCollisions`, src/parser/imports.ts:588, compares
  ...
  it("RED (f-export-set): the module still publishes the source name, and the lib specifier is now refused", async () => {
    ...
    // `computeThetaLibExports` publishes it (src/parser/imports.ts:791–796), so
