...
describe("RFC-0012 §5 — PlacementRegistry", () => {
  ...
  it("a duplicate name is dropped and the first registration stands", () => {
    const registry = new PlacementRegistry();
    const first = backend("herdr", 1);
    expect(registry.register(first)).toBeUndefined();
    const dropped = registry.register(backend("herdr", 99));
    expect(dropped?.message).toBe("ignoring subagent placement registration 'herdr': a backend with this name is already registered");
...
describe("RFC-0012 §5 — bindPlacementRegistration over pi.events", () => {
  it("DISCOVER: pi-theta emits { apiVersion: 1, register } and a listener registering synchronously lands in the registry", () => {
    ...
    // A second discover (a reload) re-offers the same name: the duplicate is
    // dropped with the W row and the first stands.
    binding.discover();
    expect(registry.snapshot()).toHaveLength(1);
    expect(diagnostics.map((d) => d.code)).toEqual([SUBAGENT_PLACEMENT_INVALID_CODE]);
  });
