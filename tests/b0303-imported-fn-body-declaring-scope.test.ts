...
function expectCleanLoad(row: Measured, label: string, expectedMaterialised: string[]): void {
  expectCleanImportLoad(
    row,
    label,
    "a well-formed `.thetalib` import is legal at every static gate; the load pass must report nothing (bug doc §Reproduction: `diags :: []`)",
    "imports.md §Visibility auto-exports a top-level `fn`/`enum`, so the imported symbol materialises under its local name",
