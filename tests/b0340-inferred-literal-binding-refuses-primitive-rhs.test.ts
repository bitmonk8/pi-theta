...
// Bug 0340 — an unannotated binding initialised from a literal records the
...
// (docs/bugs/0340-inferred-literal-binding-refuses-primitive-rhs.md)
//
// SETTLED FIX (release 0.305.0): record what the initialiser EXPRESSION types
...
describe("bug 0340 (a) — a legal write into an inferred binding is not refused", () => {
...
describe("bug 0340 (b) — a genuinely wrong write is still refused, with an actionable message", () => {
...
describe("bug 0340 (c) — the TYPE-2 narrowing route the literal target masked", () => {
...
describe("bug 0340 (d) — no reassignment message renders the same type on both sides", () => {
...
describe("bug 0340 (e) — the registration consequence", () => {
...
describe("bug 0340 (f) — neighbouring sinks do not move", () => {
...
describe("bug 0340 (g) — inferred and annotated bindings now agree", () => {
  it("G1: the array LUB renders the same for an inferred and an annotated integer binding", () => {
    ...
    // observable; the `integer | number` LUB itself is bug 0340 §Non-goals
