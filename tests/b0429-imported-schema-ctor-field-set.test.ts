...
function msg(code: string, field: string, schema: string): string {
  ...
  return registryMessageOf(REGISTRY, REGISTRY_PAGE, code, [["<field>", field], ["<schema>", schema]]);
}
...
function expectMaterialised(result: ComposeResult, expected: string, cell: string): void {
  expectMaterialisedImport(
    ...
    expected,
    cell,
    "the library resolved, parsed and exported the symbol, so the constructor walk was genuinely reachable",
  );
}
