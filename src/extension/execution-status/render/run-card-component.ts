...
/**
 * Build the live `theta-run` renderer over `deps`.
 *
 * The returned component's `render(width)` recomputes from the CURRENT bus
...
export function createRunCardRenderer(deps: RunCardComponentDeps): ThetaRunEntryRenderer {
  class RunCardComponent implements Component {
    ...
    #renderStatic(width: number): string[] {
      const component = deps.staticFallback(
    ...
    #renderLive(width: number): string[] {
      ...
      const now = clock.now();
      const state = deps.cardStateFor(this.#seed.invocationId);
