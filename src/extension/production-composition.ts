...
// no state.
//
// The discovered-`.theta` parse this walk depends on (`parseDiscoveredTheta`),
// plus the CLI-`--theta`-flag and pi-owned-command reads the same pass uses
// (`readThetaFlagPaths`, `readPiOwnedCommands`), live in
// `production-discovered-theta.ts` (PTQ-0322's Seam 0) and are imported back
// in below.
//
...
import {
  resolveCallablePiTool,
  resolvePiTool,
  type GetAllToolsSnapshot,
} from "./callable-pi-tool-resolver";
...
import { readCalleeDocument } from "./callee-load-parse";
import { resolveCallableClosurePath } from "./callable-closure-path";
...
async function resolveCallableClosureHash(
  ...
  const { cacheKey } = resolveCallableClosurePath(ctx, callerPath, calleePath);
...
async function collectCallableClosureSources(
  ...
  const { rootAbs, cacheKey } = resolveCallableClosurePath(ctx, callerPath, calleePath);
  ...
  return {
    ...
    pi: {
      sendMessage: (message, _options) => {
        pi.sendMessage(
          serializeSystemNote(SYSTEM_NOTE_CHANNEL, message.content, message.display, message.details),
          { triggerTurn: false },
        );
      },
    },
